...
<xsl:template match="OpTrim">
  RTRIM(LTRIM(<xsl:apply-templates select="./*[1]"/>))
	...
	<xsl:template match="OpCount">
	    (CASE WHEN COUNT(<xsl:apply-templates select="./*[1]"/> <xsl:apply-templates select="./*[2]"/>) = 0 THEN NULL ELSE COUNT(<xsl:apply-templates select="./*[1]"/> <xsl:apply-templates select="./*[2]"/>) END)
	</xsl:template>
...
<xsl:template match="OpDistinctCount">
	(CASE WHEN (COUNT_BIG(DISTINCT <xsl:apply-templates select="./*[1]"/>) + MAX(CASE WHEN <xsl:apply-templates select="./*[1]"/> IS NULL THEN 1 ELSE 0 END)) = 0 THEN NULL
	ELSE (COUNT_BIG(DISTINCT <xsl:apply-templates select="./*[1]"/>) + MAX(CASE WHEN <xsl:apply-templates select="./*[1]"/> IS NULL THEN 1 ELSE 0 END)) END)
</xsl:template>

<!-- FUTURE: Avoid the redundant COUNT_BIG(DISTINCT ), either by using a subquery, or doing a projection on the results -->
<xsl:template match="OpDistinctCountSkipBlank">
	(CASE WHEN COUNT_BIG(DISTINCT <xsl:apply-templates select="./*[1]"/>) = 0 THEN NULL
	ELSE COUNT_BIG(DISTINCT <xsl:apply-templates select="./*[1]"/>) END)
</xsl:template>
  ...
  <xsl:template match="OpStrictEqual">
	...
	<xsl:template match="OpAdd">
		...
		<xsl:choose>
			...
			<xsl:when test="@Op1Nullable = 'false'">
				(SELECT COALESCE(AuxTable.Op1 + AuxTable.Op2, AuxTable.Op1) FROM (SELECT <xsl:apply-templates select="./*[1]"/> AS Op1, <xsl:value-of select="$Op2"/> AS Op2) as AuxTable)
			</xsl:when>
			<xsl:when test="@Op2Nullable = 'false'">
				(SELECT COALESCE((<xsl:value-of select="$Op1"/>) + AuxTable.Op2, AuxTable.Op2) FROM (SELECT <xsl:apply-templates select="./*[2]"/> AS Op2) as AuxTable)
			</xsl:when>
			<xsl:when test="@Op1Type = 'Datetime' and @Op2Type = 'Datetime'">
	...
	<xsl:template match="OpSubtract">
		...
		<xsl:choose>
			...
			<xsl:when test="@Op2Nullable = 'false'">
				(SELECT COALESCE((<xsl:apply-templates select="./*[1]"/>) - AuxTable.Op2, -AuxTable.Op2) FROM (SELECT <xsl:value-of select="$Op2"/> AS Op2) as AuxTable)
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="OpConcatenate">
		<xsl:choose>
			...
			<xsl:when test="@Op1Nullable = 'false'">
				(SELECT COALESCE(CAST(AuxTable.Op1 AS nvarchar(max))+ AuxTable.Op2, AuxTable.Op1) FROM (SELECT (<xsl:apply-templates select="./*[1]"/>) AS Op1, (<xsl:apply-templates select="./*[2]"/>) AS Op2) as AuxTable)
			</xsl:when>
			<xsl:when test="@Op2Nullable = 'false'">
				(SELECT COALESCE(CAST(<xsl:apply-templates select="./*[1]"/> AS nvarchar(max)) + AuxTable.Op2, AuxTable.Op2) FROM (SELECT (<xsl:apply-templates select="./*[2]"/>) AS Op2) as AuxTable)
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:choose>
			...
			<xsl:when test="$target-type = 'Double'">
				<xsl:choose>
					...
					<xsl:when test="$param-type = 'SqlBool' or $param-type = 'SqlBit'">
			...
			<xsl:when test="$target-type = 'SqlBool'">
				<xsl:choose>
					...
					<xsl:when test="$param-type = 'String'">
						CAST(<xsl:apply-templates select="$value-node"/> AS boolean)
					</xsl:when>
					<xsl:otherwise>
