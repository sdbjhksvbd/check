...
:do_scheme
echo ^<scheme^>
echo     ^<title^>Powershell v3 Modular Input^</title^>
echo     ^<description^>Execute PowerShell scripts v3 with parameters as inputs.^</description^>
echo     ^<use_single_instance^>true^</use_single_instance^>
echo     ^<script^>splunk-powershell.path^</script^>
echo     ^<streaming_mode^>xml^</streaming_mode^>
echo     ^<endpoint^>
echo         ^<id^>powershell^</id^>
echo         ^<args^>
echo             ^<arg name="name"^>
