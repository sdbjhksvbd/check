...
STUDIO_CATCHALL_ERROR_MSG = 'Unable to render the dashboard %s to PDF. Check logs to verify the feature flag is set, the app is installed, the dashboard definition is valid, and no other errors occurred.'
...
class PDFGenHandler(splunk.rest.BaseRestHandler):
    ...
    def _handleConverter(self):
        convertedData = convertDashboardToPdfContent(self.sessionKey, self._namespace, self._owner, self._dashboardContent, self._dashboardName, self._studioErrors)
    ...
    def _handleStudioDashboardPdf(self):
        try:
            ...
            pu.logErrorAndTrace(errorMsg)
            self._studioErrors.append(STUDIO_CATCHALL_ERROR_MSG % self._dashboardName)
        return True
