...
class Moncocon():
    ...
    def _is_this_a_shc_instance(self):
        ...
        if mode in ['member', 'captain', 'dynamic_captain']:
            return True
        else:
            return False
    ...
    def is_auto_mc_config_enabled(self):
        ...
        content = mc_assets_conf['entry'][0]['content']
        if 'mc_auto_config' in content:
            are_we_enabled = content['mc_auto_config']
        else:
            are_we_enabled = 'disabled'
