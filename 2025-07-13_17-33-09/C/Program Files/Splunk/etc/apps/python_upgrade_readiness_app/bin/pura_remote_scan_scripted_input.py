...
logging = logger_manager.setup_logging('pura_remote_scan_scripted_input')
...
class RemoteScan:
    ...
    def add_scan_details_kvstore(self, endpoint, scan_details):
        ...
        try:
            ...
            if response["status"] not in success_codes:
                logging.error(
                    MESSAGE_ERROR_KVSTORE_DETAILS_WRITE.format(self.host)
                )
                return None
            return True
        except Exception as e:
            logging.exception(MESSAGE_EXCEPTION_KVSTORE_DETAILS_WRITE.format(self.host, str(e)))
            return None

    def get_local_scan_details(self):
        try:
            file_details = pura_storage_utils.create_dirs_if_not_exists(schedule_scan_collection, "", self.host)
            content = pura_storage_utils.read(file_details["file_path"])
            if content is None:
                logging.error(MESSAGE_ERROR_READ_SCHEDULE_SCAN_DETAILS.format("", self.host))
                return {}
            max_timestamp = None
            schedule_scan_details = dict()
            for schedule_scan_detail in content:
                if self.host == schedule_scan_detail["host"] and (max_timestamp is None or max_timestamp < int(
    ...
    def get_one_shot_str(self, host, scan_details):
        ...
        one_shot_str = "{} services/pura_manage_remote_scan".format(one_shot_str)
        return one_shot_str
    ...
    def save_schedule_scan_in_local(self, scan_details):
        try:
            ...
            if APP_DIR == OTHER_APPS_DIR:
                ...
                scripted_input_endpoint = "$SPLUNK_HOME%252Fetc%252Fapps%252Fpython_upgrade_readiness_app%252Fbin%252Fpura_scan_apps.py"
            else:
                scripted_input_endpoint = "$SPLUNK_HOME%252Fetc%252Fslave-apps%252Fpython_upgrade_readiness_app%252Fbin%252Fpura_scan_apps.py"
    ...
    def post(self):
        ...
        scan_details_local = self.get_local_scan_details()
        if (not scan_details_remote.get('is_updated', False)) and (not scan_details_local.get('is_updated', False)):
        ...
        for host in self.all_hosts:
            ...
            if hostname == self.host:
                if not scan_details_local.get("is_updated", False):
                    logging.info(
                        "Skipping to call pipe rest command on host: {} for saving in local.".format(hostname))
                else:
                    is_manage_scan_called = self.save_schedule_scan_in_local(scan_details_local)
                    if is_manage_scan_called:
                        file_details = pura_storage_utils.create_dirs_if_not_exists(schedule_scan_collection, "", self.host)
                        scan_details_local['is_updated'] = False
                        _ = pura_storage_utils.add(file_details["file_path"], scan_details_local, True)
