...
def get_dismiss_app_kvstore_details(session_key, scan_type="python"):
    ...
    try:
        ...
        for app in content:
            ...
            dismissed_app_details[app['app_path']]['dismissed_by'] = app.get('dismissed_by', DISMISSED_BY_USER)
    ...
    try:
        ...
        for app in content:
            ...
            if dismissed_file_details.get(app['app_path']):
                ...
                temp['dismissed_by'] = app.get('dismissed_by', DISMISSED_BY_USER)
                temp['dismiss_file_date'] = app.get('dismiss_file_date', "")
            else:
...
def get_dismiss_check_kvstore_details(session_key):
    ...
    try:
        ...
        for system_check in content:
            ...
            dismissed_check_details[system_check['check_name']]['dismissed_by'] = system_check.get('dismissed_by', DISMISSED_BY_USER)
        return dismissed_check_details
    except Exception as e:
        logging.exception("Could not fetch dismissed checks from kvstore: {}".format(str(e)))
        return {}

def get_export_report_kvstore_details(session_key, scan_type="python"):
    """
    ...
    """
    try:
        endpoint = pra_remote_export_report_endpoint
        if scan_type.lower() == "python":
            endpoint = pra_remote_export_report_endpoint
        elif scan_type.lower() == "jquery":
            endpoint = jra_remote_export_report_endpoint
        elif scan_type.lower() == "emerald":
            endpoint = era_remote_export_report_endpoint
        try:
            response, content = sr.simpleRequest('{}?output_mode=json'.format(endpoint),
                                            sessionKey=session_key, method='GET',
                                            raiseAllErrors=True)
        ...
        if response['status'] not in success_codes:
            return []
        content = json.loads(content)
        export_report_details = []
        for app in content:
            data = {'app_name': app['app'], 'user': app['exported_by'], 'time_stamp': app['export_report_date']}
            export_report_details.append(data)
        return export_report_details
    except Exception as e:
        logging.exception("Could not fetch exported reports from kvstore: {}".format(str(e)))
        return []

def get_send_email_kvstore_details(session_key, scan_type="python"):
    """
    ...
    """
    try:
        endpoint = pra_remote_send_email_endpoint
        if scan_type.lower() == "python":
            endpoint = pra_remote_send_email_endpoint
        elif scan_type.lower() == "jquery":
            endpoint = jra_remote_send_email_endpoint
        elif scan_type.lower() == "emerald":
            endpoint = era_remote_send_email_endpoint
        try:
            response, content = sr.simpleRequest('{}?output_mode=json'.format(endpoint),
                                            sessionKey=session_key, method='GET',
                                            raiseAllErrors=True)
        ...
        send_email_details = []
        for app in content:
            data = {'app_name': app['app'], 'user': app['emailed_by'], 'time_stamp': app['email_date']}
            send_email_details.append(data)
        return send_email_details
    except Exception as e:
        logging.exception("Could not fetch exported reports from kvstore: {}".format(str(e)))
        return []

def get_page_visits_kvstore_details(session_key, scan_type="python"):
    """
    ...
    try:
        ...
        page_visits_details = []
        for item in content:
            data = {'user': item['user'], 'time_stamp': item['time_stamp']}
            page_visits_details.append(data)
        return page_visits_details
    except Exception as e:
        logging.exception("Could not fetch exported reports from kvstore: {}".format(str(e)))
        return []
    ...
    try:
        for _app in report.get('apps', []):
            if kvstore_details.get(_app['app_path']):
                ...
                _app['summary']['dismissed_by'] = kvstore_details[_app['app_path']]['dismissed_by']
    ...
    try:
        for check in report.get('system', []):
            if kvstore_details.get(check['name']):
                ...
                check['summary']['dismissed_by'] = kvstore_details[check['name']]['dismissed_by']
                ...
                elif old_status == CHECK_CONST_WARNING:
                    system_warning = report["summary"]["system_warning"]
                    system_dismissed = report["summary"]["system_dismissed"]
                    report['summary']["system_warning"] = system_warning - 1
                    report['summary']["system_dismissed"] = system_dismissed + 1
                else:
                    ...
                    for _check in _app['checks']:
                        if _check['name'] == check:
                            if is_remote:
                                ...
                                dismissed_by = file_details["dismissed_by"]
                                dismiss_file_date = file_details['dismiss_file_date']
                                ...
                                _check['messages'], dismissed_message_count = update_check_messages_remote(_check['messages'],
                                                                                                    file_path, dismissed_instance, _app['name'], dismissed_by, dismiss_file_date)
                            else:
                                dismissed_by = file_details["dismissed_by"]
                                dismiss_file_date = file_details['dismiss_file_date']
                                _check['messages'], dismissed_message_count = update_check_messages(_check['messages'],
                                                                                                        file_path, dismissed_by, dismiss_file_date)
...
def update_check_messages(messages, file_path, dismissed_by, dismiss_file_date):
    dismissed_message_count = 0
    for i in messages:
        if i['message_filename'] == file_path:
            i['dismissed'] = 1
            i["dismissed_by"] = dismissed_by
            i['dismiss_file_date'] = dismiss_file_date
...
def update_check_messages_remote(messages, file_path, dismissed_instance, app_name, dismissed_by, dismiss_file_date):
    dismissed_message_count = 0
    for i in messages:
...
def delete_kvstore_details(session_key, endpoint):
    """
    ...
    """
    try:
        response, _ = sr.simpleRequest(endpoint, 
                                       sessionKey=session_key, 
                                       method='DELETE', 
                                       raiseAllErrors=True)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_KVSTORE_DELETE.format(endpoint, str(e)))
            return False
        return True
    except Exception as e:
        logging.exception(MESSAGE_EXCEPTION_KVSTORE_DELETE.format(endpoint, str(e)))
        return False
...
def check_scheduled_input_details(endpoint, post_args, session_key):
    try:
        response, content = sr.simpleRequest(
                get_schedule_scan_interval_endpoint.format(endpoint),
                sessionKey=session_key,
        ...
        if str(current_disabled) == updated_disabled:
            return True
        else:
            return False
    except Exception as e:
...
def save_jura_scripted_input(email_switch_details, session_key, user, host):
    ...
    try:
        ...
        scheduled_input_details = check_scheduled_input_details(scripted_input_endpoint, postargs, session_key)
        if not scheduled_input_details:
            logging.info("Updated input.conf in local for host: {}".format(host))
            is_jura_details_saved = add_details_to_scripted_input(endpoint, postargs, session_key, "", host)
            return is_jura_details_saved
        else:
            logging.info("Skipping input updation for host: {} as the data is same.".format(host))
            return True
    except Exception as e:
        logging.exception(str(e))
        return False
...
def one_shot_str_wrapper(path, service):
    ...
    try:
        if service is None:
            return []
        oneshot_job = service.jobs.oneshot(path, output_mode='json')
        response = results.JSONResultsReader(oneshot_job)
        return response
    except Exception as e:
        logging.exception(str(e))
        return []
...
def update_structure(scan_summary, scanType, is_config):
    ...
    try:
        ...
        for summary in summary_data:
            ...
            # Add user actions details for apps only
            if not is_config:
                export_report = {'data': [], 'count': 0}
                for item in scan_summary.get('export_report', {}).get('data', []):
                    if item['app_name'] == data['name']:
                        export_report['data'].append({'user': item['user'], 'time_stamp': item['time_stamp']})
                        export_report['count'] += 1
                data['export_report_details'] = export_report
                
                send_email = {'data': [], 'count': 0}
                for item in scan_summary.get('send_email', {}).get('data', []):
                    if item['app_name'] == data['name']:
                        send_email['data'].append({'user': item['user'], 'time_stamp': item['time_stamp']})
                        send_email['count'] += 1
                data['send_email_details'] = send_email
...
def update_summary(scan_summary):
    ...
    try:
        ...
        export_report = {'data': [], 'count': 0}
        for item in scan_summary.get('export_report', {}).get('data', []):
            if item['app_name'] == ALL_APPS_NAME:
                export_report['data'].append({'user': item['user'], 'time_stamp': item['time_stamp']})
                export_report['count'] += 1
        summary["data"]["all_apps_export_report_details"] = export_report
        summary["data"]["page_visits_details"] = scan_summary.get('page_visits', {})
        summary["data"]["email_switch_details"] = scan_summary.get('email_switch', {})
