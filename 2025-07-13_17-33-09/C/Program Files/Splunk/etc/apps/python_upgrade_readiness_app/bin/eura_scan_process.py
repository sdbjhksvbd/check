...
class ScanProcess(object):
    ...
    def post_scan_deployment(self):
        ...
        try:
            ...
            if not self.is_cloud and self.system_scan:
            ...
            if not self.is_cloud and self.system_scan:
                ...
                if search_peer_tls_config_report.get("summary", {}).get("Status") == CHECK_CONST_PASSED:
                    system_passed += 1
                elif search_peer_tls_config_report.get("summary", {}).get("Status") == CHECK_CONST_WARNING:
                    system_warning += 1
                else:
                    system_blocker += 1
                ...
                if mongodb_tls_dns_validation_report.get("summary", {}).get("Status") == CHECK_CONST_PASSED:
                    system_passed += 1
                elif mongodb_tls_dns_validation_report.get("summary", {}).get("Status") == CHECK_CONST_WARNING:
                    system_warning += 1
                else:
                    system_blocker += 1
            ...
            if not self.is_cloud:
                results['summary']['system_dismissed'] = 0
                results['summary']['system_passed'] = system_passed
                results['summary']['system_warning'] = system_warning

            results['summary']['scan_completion_time'] = scan_completion_time
            results['summary']['app_version'] = UPGRADE_READINESS_APP_VERSION
