...
def send_email_wrapper(session_key, email_body_details):
    ...
    if failed_apps > 5:
        ...
        string += "</ul>"
        email_body_text = BODY.format(failed_apps, "apps", "jQuery", stack_name, string)
    elif failed_apps > 1:
        string = "<ul>\n"
        for s in failed_apps_name:
            string += "<li>" + str(s) + "</li>\n"
        ...
        for s in failed_apps_name:
            string += "<li>" + str(s) + "</li>\n"
        string += "</ul>"
        email_body_text = BODY.format(failed_apps, "app", "jQuery", stack_name, string)  
...
def find_receivers(session_key):
    ...
    try:
        ...
        endpoint = "/services/jura_email_receivers_list"
        try:
