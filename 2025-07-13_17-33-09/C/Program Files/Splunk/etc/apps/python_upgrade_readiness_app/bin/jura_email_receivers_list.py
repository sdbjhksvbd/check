...
logging = logger_manager.setup_logging('jura_email_receivers_list')
...
class ReceiversListHandler(PersistentServerConnectionApplication):
    ...
    def get_email_receivers_list(self):
        ...
        try:
            ...
            try:
                kvstore_response, kvstore_content = sr.simpleRequest('{}?output_mode=json'.format(jra_email_receivers_list_endpoint),
    ...
    def post_email_receivers_list(self, request_body):
        utils.delete_kvstore_details(self.session_key, jra_email_receivers_list_endpoint)
        for item in request_body['receivers_list']:
            try:
                response, _ = sr.simpleRequest('{}?output_mode=json'.format(jra_email_receivers_list_endpoint),
