    ...
    def handle(self, in_string):
        ...
        try:
            ...
            pura_entry_lock = FileLock(
                os.path.join(os.path.dirname(__file__), "..", "local", "jura_entry.lock")
            )
            ...
            with pura_entry_lock.acquire(timeout=120, poll_intervall=5):
                ...
                if six.PY2:
                    command = "\"{}\" cmd python \"{}\"".format(SPLUNK_PATH, JQUERY_PROCESS_PATH)
                elif six.PY3:
                    command = "\"{}\" cmd python3 \"{}\"".format(SPLUNK_PATH, JQUERY_PROCESS_PATH)
                if os.name == "nt":
