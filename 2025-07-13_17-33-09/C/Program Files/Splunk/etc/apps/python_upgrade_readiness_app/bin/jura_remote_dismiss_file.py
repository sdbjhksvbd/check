...
logging = logger_manager.setup_logging('jura_remote_dismiss_file')
...
class RemoteDismissFileHandler(PersistentServerConnectionApplication):
    """
    ...
    For example, if a GET request is made to the endpoint is executed with the path /remote_dismiss_file,
    then this class will attempt to run a function named get_remote_dismiss_file().
    ...
    def post_remote_dismiss_file(self, query_params):
        """
        ...
        if 'instance' not in query_params or not query_params['instance']:
            logging.error(MESSAGE_DISMISS_INSTANCE_READ_ERROR)
            return utils.render_error_json(MESSAGE_DISMISS_INSTANCE_READ_ERROR, 404)
        self.remote_host = query_params['remote_host']
        entry = {
            "app": query_params["app"],
            "app_path": query_params["app_path"],
            "check": query_params["check"],
            "file_path": query_params["file_path"],
            "instance": query_params["instance"],
            "dismissed_by": self.user,
            "dismiss_file_date": int(time.time())
        }
    ...
    def write_entry(self, entry):
        ...
        try:
            response, _ = sr.simpleRequest('{}?output_mode=json'.format(jra_dismiss_remote_file_endpoint),
                                           sessionKey=self.session_key, jsonargs=json.dumps(entry), method='POST',
                                           raiseAllErrors=True)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_WRITING_REMOTE_DISMISS_ENTRY.format(self.remote_host))
            return False, MESSAGE_EXCEPTION_WRITING_REMOTE_DISMISS_ENTRY.format(self.remote_host)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_WRITING_REMOTE_DISMISS_ENTRY.format(self.remote_host))
            return False, MESSAGE_ERROR_WRITING_REMOTE_DISMISS_ENTRY.format(self.remote_host)

        return True, MESSAGE_REMOTE_DISMISS_ENTRY_SUCCESS.format(entry['file_path'], entry['check'],
                                                          entry['app'], self.remote_host)
