...
logging = logger_manager.setup_logging('eura_remote_dismiss_file')
...
class RemoteDismissFileHandler(PersistentServerConnectionApplication):
    ...
    def write_entry(self, entry):
        ...
        try:
            response, _ = sr.simpleRequest('{}?output_mode=json'.format(era_dismiss_remote_file_endpoint),
                                           sessionKey=self.session_key, jsonargs=json.dumps(entry), method='POST',
                                           raiseAllErrors=True)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_WRITING_REMOTE_DISMISS_ENTRY.format(self.host))
            return False, MESSAGE_EXCEPTION_WRITING_REMOTE_DISMISS_ENTRY.format(self.host)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_WRITING_REMOTE_DISMISS_ENTRY.format(self.host))
            return False, MESSAGE_ERROR_WRITING_REMOTE_DISMISS_ENTRY.format(self.host)
