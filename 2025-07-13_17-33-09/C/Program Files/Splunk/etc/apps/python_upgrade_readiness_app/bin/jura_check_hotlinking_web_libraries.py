...
class SplunkHotlinklingWebLibrary:
    ...
    def read_risky_imports_list(self):
        try:
            logging.info("Getting risky imports list.")
            dir_path = os.path.join(APP_DIR, SELF_DIR_NAME, "bin", self.libs, "pura_libs_utils")
            file_path = os.path.join(dir_path, "risky_imports.json")
            web_core_contents = []
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                web_core_contents = json.load(f)
            return list(set(web_core_contents))
        except Exception as e:
            logging.exception(str(e))
            return []
    ...
    def get_all_files(self, app_name, app_path):
        ...
        try:
            ...
            html_static_template_filepath = os.path.join(dir_path, "appserver", "static", "template")
            html_static_template_files = self.get_html_template_files(html_static_template_filepath)
            html_dashboard_filepath = os.path.join(dir_path, "default", "data", "ui", "html")
            html_dashboard_files = self.get_html_template_files(html_dashboard_filepath)
            html_files = html_template_files + html_static_template_files + html_dashboard_files
            return html_files
        except Exception as e:
            logging.exception(str(e))
            return []

    def find_hotlinking_web_library_in_html(self, html_files, disallowed_list, risky_imports_list):
        """
            ...
            try:
                ...
                with open(file_path, "r", encoding="utf-8", errors="ignore") as my_file:
                    ...
                    for match in matches:
                        ...
                        elif match in risky_imports_list:
                            risky_imports.add(match)
                    bad_imports.update(risky_imports)
                    if bad_imports:
    ...
    def check_splunk_web_library(self, app_name, app_path):
        ...
        try:
            ...
            risky_imports_list = self.read_risky_imports_list()
            self.find_hotlinking_web_library_in_html(html_files, disallowed_list, risky_imports_list)
