...
logging = logger_manager.setup_logging('jura_remote_export_report')
...
class RemoteExportReportHandler(PersistentServerConnectionApplication):
    ...
    def get_remote_export_report(self, is_remote):
        ...
        if not filepath:
            return utils.render_error_json(FILEPATH_NOT_FOUND.format(filepath), 404)
        ...
        are_dismissed_files_filtered = utils.filter_dismissed_files(results, dismiss_file_details, scan_type="jquery")
        ...
        entry = {
            ...
            "export_report_date": int(time.time())
        }
    ...
    def write_entry(self, entry):
        ...
        try:
            response, _ = sr.simpleRequest('{}?output_mode=json'.format(jra_remote_export_report_endpoint),
                                           sessionKey=self.session_key, jsonargs=json.dumps(entry), method='POST',
                                           raiseAllErrors=True)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.remote_host))
            return False, MESSAGE_EXCEPTION_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.remote_host)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.remote_host))
            return False, MESSAGE_ERROR_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.remote_host)

        return True, MESSAGE_REMOTE_EXPORT_REPORT_ENTRY_SUCCESS.format(self.remote_host)