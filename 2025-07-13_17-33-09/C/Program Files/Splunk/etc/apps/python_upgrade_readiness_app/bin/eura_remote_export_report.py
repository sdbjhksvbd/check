...
logging = logger_manager.setup_logging('eura_remote_export_report')
...
class RemoteExportReportHandler(PersistentServerConnectionApplication):
    ...
    def get_remote_export_report(self, is_remote):
        ...
        if self.app_name != ALL_APPS_NAME:
            ...
            if not results:
                return utils.render_error_json(MESSAGE_ERROR_REMOTE_EXPORT_APP_REPORT_.format(self.app_name, self.host), 404)
    ...
    def write_entry(self, entry):
        ...
        try:
            response, _ = sr.simpleRequest('{}?output_mode=json'.format(era_remote_export_report_endpoint),
                                           sessionKey=self.session_key, jsonargs=json.dumps(entry), method='POST',
                                           raiseAllErrors=True)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.host))
            return False, MESSAGE_EXCEPTION_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.host)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.host))
            return False, MESSAGE_ERROR_WRITING_REMOTE_EXPORT_REPORT_ENTRY.format(self.host)

        return True, MESSAGE_REMOTE_EXPORT_REPORT_ENTRY_SUCCESS.format(self.host)