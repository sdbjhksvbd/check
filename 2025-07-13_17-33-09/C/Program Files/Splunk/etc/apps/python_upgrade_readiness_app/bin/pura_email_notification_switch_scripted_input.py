...
logging = logger_manager.setup_logging('pura_email_notification_scripted_input')
...
class EmailSwitch:
    ...
    def get_one_shot_str(self, host, email_switch_details):
        ...
        if email_switch_details.get("is_pra_email_enabled") is not None:
            one_shot_str = "{} is_pra_email_enabled=\"{}\"".format(one_shot_str, email_switch_details['is_pra_email_enabled'])
        if email_switch_details.get("is_jra_email_enabled") is not None:
            one_shot_str = "{} is_jra_email_enabled=\"{}\"".format(one_shot_str, email_switch_details['is_jra_email_enabled'])
        one_shot_str = "{} services/pura_manage_email_notification_switch".format(one_shot_str)
        return one_shot_str
