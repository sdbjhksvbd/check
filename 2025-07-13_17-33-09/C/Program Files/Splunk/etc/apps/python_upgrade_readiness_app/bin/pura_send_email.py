...
def get_failed_apps_count(report):
    ...
    try:
        ...
        return {"failed_apps" : failed_apps, "failed_apps_name" : failed_apps_name}
    except Exception as e:
        logging.exception("Exception while getting failed apps count {}".format(str(e)))
        return {"failed_apps" : 0, "failed_apps_name" : []}
...
def send_email_wrapper(session_key, email_body_details):
    ...
    if failed_apps > 5:
        ...
        string += "</ul>"
        email_body_text = BODY.format(failed_apps, "apps", "Python", stack_name, string)
    elif failed_apps > 1:
        string = "<ul>\n"
        for s in failed_apps_name:
            string += "<li>" + str(s) + "</li>\n"
        ...
        for s in failed_apps_name:
            string += "<li>" + str(s) + "</li>\n"
        string += "</ul>"
        email_body_text = BODY.format(failed_apps, "app", "Python", stack_name, string)
...
def find_receivers(session_key):
    ...
    try:
        ...
        endpoint = "/services/pura_email_receivers_list"
        try:
