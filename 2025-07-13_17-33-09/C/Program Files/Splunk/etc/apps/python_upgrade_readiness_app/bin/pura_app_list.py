    ...
    def get_user_apps(self, response_role, response_apps):
        ...
        for app in applist.get('entry', []):
            ...
            try:
                ...
                read_permission = [] if read_permission is None else read_permission
                write_permission = app['acl'].get('perms').get('write')
                write_permission = [] if write_permission is None else write_permission
                app_permission.update(read_permission)
                app_permission.update(write_permission)
                app_permission = list(app_permission)
            ...
            elif '*' in app_permission:
                user_apps.append((app['name'], app['content']['label'], version, visible))
            elif set(user_roles).intersection(set(app_permission)):
                user_apps.append((app['name'], app['content']['label'], version, visible))
            else:
