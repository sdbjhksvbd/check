...
logging = logger_manager.setup_logging('eura_remote_sendemail')
...
class RemoteSendEmailHandler(PersistentServerConnectionApplication):
    ...
    def post_remote_sendemail(self, is_remote):
        ...
        are_dismissed_files_filtered = utils.filter_dismissed_files(report, dismiss_file_details, scan_type="emerald")
        if not are_dismissed_files_filtered:
            logging.error(MESSAGE_SKIP_DISMISS_FILES)
            report = results_dismissed_files_copy
        results_dismissed_checks_copy = copy.deepcopy(report)
        are_dismissed_checks_filtered = utils.filter_dismissed_checks(report, dismiss_check_details)
        if not are_dismissed_checks_filtered:
            logging.error(MESSAGE_SKIP_DISMISS_SYSTEM_CHECK)
            report = results_dismissed_checks_copy
        ...
        attachment_name = EMERALD_EMAIL_ATTACHMENT_NAME.format(self.app_name, str(self.host))
        ...
        if is_email_sent:
            entry = {
                "app": self.app_name,
                "emailed_by": self.user,
                "email_date": int(time.time())
            }
    ...
    def write_entry(self, entry):
        ...
        try:
            response, _ = sr.simpleRequest('{}?output_mode=json'.format(era_remote_send_email_endpoint),
                                           sessionKey=self.session_key, jsonargs=json.dumps(entry), method='POST',
                                           raiseAllErrors=True)
        except Exception:
            logging.exception(MESSAGE_EXCEPTION_WRITING_REMOTE_SEND_EMAIL_ENTRY.format(self.remote_host))
            return False, MESSAGE_EXCEPTION_WRITING_REMOTE_SEND_EMAIL_ENTRY.format(self.remote_host)
        if response['status'] not in success_codes:
            logging.error(MESSAGE_ERROR_WRITING_REMOTE_SEND_EMAIL_ENTRY.format(self.remote_host))
            return False, MESSAGE_ERROR_WRITING_REMOTE_SEND_EMAIL_ENTRY.format(self.remote_host)

        return True, MESSAGE_REMOTE_SEND_EMAIL_ENTRY_SUCCESS.format(entry['app'], self.remote_host)