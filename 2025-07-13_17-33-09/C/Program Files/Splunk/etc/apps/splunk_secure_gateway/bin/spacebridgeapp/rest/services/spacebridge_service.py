...
def authentication_query_request(log: logging.Logger, auth_code, encryption_context, system_authtoken):
    ...
    try:
        ...
        log.exception("Exception contacting spacebridge")
...
def delete_device_from_spacebridge(log: logging.Logger, device_id, system_authtoken, key_bundle=None):
    """
    Deletes device from spacebridge
    :return: response from spacebridge
    """
    headers, unregister_proto = _process_unregister_request(log, device_id, system_authtoken)
    config = load_config(system_authtoken)
    ...
    with requests_ssl_context(key_bundle):
        try:
            ...
            log.exception("Exception attempting sending delete device request to Spacebridge")
            raise Errors.SpacebridgeServerError('Unable to reach Spacebridge', HTTPStatus.SERVICE_UNAVAILABLE)

    log.info("Received response=%s on delete device from Spacebridge request" % response.status_code)

    _process_unregister_response(log, response.content)

    return response


async def async_delete_device_from_spacebridge(log: logging.Logger, async_spacebridge_client, device_id, system_authtoken):
    """
    ...
    if response.code != HTTPStatus.OK:
        ...
        log.exception("Exception attempting sending delete device request to Spacebridge, with code=%s, error_msg=%s"
                         % (response.code, response))
        raise Errors.SpacebridgeServerError('Unable to reach Spacebridge for device=%s'
                                            % device_id, HTTPStatus.SERVICE_UNAVAILABLE)

    log.info("Received response=%s on delete device from Spacebridge request" % response.code)

    _process_unregister_response(log, response._body)

    return response


def _process_unregister_response(log: logging.Logger, response):
    spacebridge_response = http_pb2.DeviceUnregistrationResponse()
    spacebridge_response.ParseFromString(response)
    log.info('Spacebridge response: %s' % str(spacebridge_response))
...
def _process_unregister_request(log: logging.Logger, device_id, system_authtoken):
...
def device_pairing_confirmation_request(log: logging.Logger,
                                        auth_header, auth_code, username, device_id,
                                        encrypt_public_key, sign_public_key,
                                        session_token_encrypted, encrypt, deployment_friendly_name, system_authtoken):
    """ Abstraction layer for the spacebridge request. This function:
    ...
    log.info("Registration Bundle deploymentPublicKey= %s" % str(sb_request_proto.deploymentPublicKey))
    log.info("Registration Bundle deviceId= %s" % str(sb_request_proto.deviceId))
...
def send_mdm_signing_key_to_spacebridge(log: logging.Logger, system_authtoken, authtoken, mdm_public_signing_key, key_bundle=None):
...
def send_notification_request(system_authtoken, auth_header, notification_proto) -> None:
    config = load_config(system_authtoken)
    try:
        response = requests.post(
            f'{config.get_spacebridge_domain()}/api/notifications', 
            headers={
                'Content-Type': 'application/x-protobuf',
                'Authorization': str(auth_header)
            },
            data=notification_proto.SerializeToString(),
            proxies=config.get_proxies()
        )
    except Exception:
        raise Errors.SpacebridgeServerError('Unable to reach Spacebridge', 503)
