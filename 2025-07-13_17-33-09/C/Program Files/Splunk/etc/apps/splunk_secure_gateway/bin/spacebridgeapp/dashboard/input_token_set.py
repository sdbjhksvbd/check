...
class InputTokenSet(object):
    ...
    def add_input_element(self, input_element):
        ...
        :return:
        """
        input_token = to_input_token(self.log, input_element)
...
def to_input_token(log: logging.Logger, input_element):
    """
    ...
    input_type = parse_input_type_from_token(log, input_element)
    ...
    if change_element:
        ...
        change = to_change_condition(log, change_element)
...
def parse_input_type_from_token(log: logging.Logger, input_element):
    ...
    if input_type == DROPDOWN_INPUT:
        return Dropdown(label=label, default_value=extract_default_value(input_element),
                        ...
                        dynamic_options=extract_dynamic_options(log, input_element),
...
def extract_dynamic_options(log: logging.Logger, input_element) -> Optional[DynamicOptions]:
    """
    ...
    if field_for_label and field_for_value and search_element is not None:
        v, search = build_dashboard_visualization_search(log, search_element=search_element)
