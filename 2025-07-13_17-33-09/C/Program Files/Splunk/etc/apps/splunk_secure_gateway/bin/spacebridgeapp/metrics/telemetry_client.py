...
def post_event(log: logging.Logger, event_jsn, session_token):
    payload = create_telemetry_payload(event_jsn)
    log.debug("attempting to post metrics")
    params = {
        'output_mode': 'json'
    }
    ...
    log.info(F"Posted metrics data to telemetry with response={str(r.status)}")
    return r


async def get_telemetry_instance_id(log: logging.Logger, async_client, auth_header):
    rest_uri = rest.makeSplunkdUri()
    ...
    if r.code != HTTPStatus.OK:
        ...
        log.info("Could not get telemetry instance id with error={} and status={}".format(error_text, r.code))
    instance_id = await r.text()
    return instance_id


async def get_installation_environment(log: logging.Logger, async_client, auth_header):
    rest_uri = rest.makeSplunkdUri()
...
async def get_splunk_version(log: logging.Logger, async_client, auth_header):
    """
    Gets splunk version
    :param async_client:
    :param auth_header
    ...
    if r.code != HTTPStatus.OK:
        error_text = await r.text()
        log.info("Could not get splunk version with error={} and status={}".format(error_text, r.code))
    jsn = await r.json()
...
class AsyncTelemetryClient(AsyncClient):
    ...
    def __init__(self, log: logging.Logger, config: SecureGatewayConfig):
        super(AsyncTelemetryClient, self).__init__(log, config, client=AioHttpClient(verify_ssl=False))
        self.log = log
    ...
    async def set_telemetry_instance_id(self, auth_header):
        self.telemetry_instance_id = await get_telemetry_instance_id(self.log, self, auth_header=auth_header)
    ...
    async def set_splunk_version(self, auth_header):
        self.splunk_version = await get_splunk_version(self.log, self, auth_header=auth_header)
    ...
    async def set_installation_environment(self, auth_header):
        self.installation_environment = await get_installation_environment(self.log, self, auth_header=auth_header)

    async def post_metrics(self, log: logging.Logger, event, auth_header):
        ...
        if not self.telemetry_instance_id:
            self.telemetry_instance_id = await get_telemetry_instance_id(self.log, self, auth_header)
        event.update({constants.INSTANCEID: self.telemetry_instance_id})
