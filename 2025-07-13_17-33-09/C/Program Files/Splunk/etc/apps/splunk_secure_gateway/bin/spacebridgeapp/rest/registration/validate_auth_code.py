...
class ValidateAuthCodeHandler(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def __init__(self, command_line, command_arg):
        BaseRestHandler.__init__(self, logname="rest_registration_query")
    ...
    def get(self, request):
        ...
        return handle_query(config, self.log,
                            auth_code, device_name, user, system_authtoken, encryption_context, kvstore_access_object)


def handle_query(config: SecureGatewayConfig,
                 log: logging.Logger,
                 auth_code, device_name, user, system_authtoken, encryption_context,
    ...
    deployment_info = get_deployment_info(log, system_authtoken)
    ...
    log.info(f"Authenticating code with enforce_mdm={mdm_signing_public_key}")
    ...
    # Broad exception clause as we can't be certain what error an air-gapped system firewall returns
        ...
        log.error(f"Error when querying Discovery for list of instances. Error={e}")
    ...
    try:
        ...
        if instance_id:
            ...
            log.debug("Updating auth_code = %s in Spacebridge Discovery", hashed_auth.hex())
            update_hashcode_instance(hashed_auth=hashed_auth,
                                     instance_id=instance_id,
                                     encryption_context=encryption_context,
                                     config=config)

        log.debug("Spacebridge Discovery Successful, spacebridge_server = %s using Registration v2",
        ...
        if instance_id and not config.is_spacebridge_server_v1_compatible():
            log.warning("Auth_code = %s had a Registration_V2 error. Spacebridge_server = %s does not fall-back.",
        ...
        log.warning("Auth_code = %s failed Registration_V2 with error = %s. Spacebridge_server falling-back to RegV1.",
    ...
    log.debug("Successfully validated auth_code using registration_version = %s", registration_version)
    ...
    log.info("client_device_info={}".format(client_device_info))

    user_devices = get_devices_for_user(log, user, system_authtoken)
    log.info("user_devices=%s" % user_devices)
