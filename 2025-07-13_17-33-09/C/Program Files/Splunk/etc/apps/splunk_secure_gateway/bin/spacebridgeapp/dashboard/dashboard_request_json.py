...
async def fetch_dashboard_list_for_app(log: logging.Logger,
                                       request_context, app_name, params, async_splunk_client):
    response = await async_splunk_client.async_get_dashboard_list_request(auth_header=request_context.auth_header,
                                                                          app_name=app_name,
                                                                          params=params)

    log.debug('fetch_dashboard_list_for_app response=%s', response.code)
    ...
    log.debug('fetch_dashboard_list_for_app app_name=%s, total=%s', app_name, total)
...
async def fetch_dashboard_list_json(log: logging.Logger,
                                    request_context,
                                    offset=0,
                                    max_results=0,
                                    app_names=None,
    ...
    for app in app_names:
        ...
        total_app, dashboards_app = await fetch_dashboard_list_for_app(log,
