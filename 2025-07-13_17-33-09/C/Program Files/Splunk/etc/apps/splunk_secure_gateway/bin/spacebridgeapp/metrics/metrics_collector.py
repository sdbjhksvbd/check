...
class SpacebridgeAppMetric:
    ...
    def __init__(self, session_token, logger: logging.Logger):
    ...
    def send_to_telemetry(self):
        ...
        telemetry_client.post_event(self.logger, metric, self.session_token)
...
class NumRegisteredDevicesMetric(SpacebridgeAppMetric):
    ...
    def calculate(self):
        devices = get_devices_for_registered_users(self.logger, self.session_token)
...
class OptInPageMetric(SpacebridgeAppMetric):
    ...
    3. After user clicks on "Get Started" button in UI (admin users) //User opted in

    ####OLD VERSION - Where user has to check box on legal page and manually opt in#####
    ...
    OPT_IN = 'opt_in'
    GET_STARTED_AND_OPT_IN = 'get_started_and_opt_in'
    ...
    def __init__(self, session_token, logger, user, option):
        ...
        self.user = user
        if option not in(self.NON_ADMIN_INTRO, self.ADMIN_INTRO, self.GET_STARTED_AND_OPT_IN,
    ...
    def calculate(self):
        ...
        deployment_info = get_deployment_info(self.logger, self.session_token)
