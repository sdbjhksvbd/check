...
class AsyncITSIClient(AsyncNonSslClient):
    def __init__(self, log: logging.Logger, config: SecureGatewayConfig, uri):
        ...
        self.log = log
        self.uri = uri
        super(AsyncITSIClient, self).__init__(self.log, config)
    ...
    async def async_get_glass_table_count(self, auth_header, params=None):
        ...
        if response.code == HTTPStatus.OK:
            ...
            self.log.debug(f"Unable to find ITSI API. code={response.code}")
        else:
            self.log.debug(f"Unable to retrieve glass_table count params={params} code={response.code}")
        return count
