...
from spacebridgeapp.util.constants import USER_META_COLLECTION_NAME, DASHBOARD_APP_LIST, KEY, APP_NAMES



async def process_dashboard_app_list_set_request(log: logging.Logger,
                                                 request_context,
    ...
    if app_names:
        ...
        all_app_names = await fetch_app_names(log,
                                              request_context=request_context, async_splunk_client=async_splunk_client)
    ...
    dashboard_app_list = await set_dashboard_app_list(log,
...
async def fetch_dashboard_app_list(log: logging.Logger,
                                   request_context,
    ...
    if async_kvstore_client and async_splunk_client:
        ...
        if response.code == HTTPStatus.OK:
            ...
            if response_json:
                ...
                valid_app_names = await fetch_app_names(log, request_context=request_context, async_splunk_client=async_splunk_client)
...
async def set_dashboard_app_list(log: logging.Logger,
                                 request_context, app_names=None, async_kvstore_client=None, async_splunk_client=None):
    """
    Helper to set dashboard_app_list object in user_meta
    """
    # Get dashboard_meta collection if key exists
    existing_app_names = await fetch_dashboard_app_list(log,
                                                        request_context=request_context,
                                                        async_kvstore_client=async_kvstore_client,
                                                        async_splunk_client=async_splunk_client)
    ...
    log.info("Successful Dashboard App List Set Request")
    return dashboard_app_list


async def process_dashboard_app_list_get_request(log: logging.Logger,
                                                 request_context,
                                                 client_single_request,
                                                 single_server_response,
                                                 async_client_factory):
...
async def fetch_dashboard_app_list_with_default(log: logging.Logger,
                                                request_context,
    ...
    # Get dashboard_meta collection if key exists
    app_names = await fetch_dashboard_app_list(log,
                                               request_context=request_context,
                                               async_kvstore_client=async_kvstore_client,
                                               async_splunk_client=async_splunk_client)
...
async def process_app_list_request(log: logging.Logger,
                                   request_context,
                                   client_single_request,
                                   single_server_response,
                                   async_client_factory):
    ...
    # fetch app lists
    app_list = await fetch_app_names(log,
                                     request_context=request_context, async_splunk_client=async_splunk_client)
    ...
    log.debug("Finished populating response for app list request")


def fetch_app_details(log: logging.Logger, request_context, app_id, async_splunk_client):
    return _fetch_app_names(log, request_context, app_id, async_splunk_client)


def fetch_app_names(log: logging.Logger,
                    request_context,
                    async_splunk_client=None):
    return _fetch_app_names(log, request_context, async_splunk_client=async_splunk_client)


async def _fetch_app_names(log: logging.Logger,
                           request_context,
                           app_id=None,
                           async_splunk_client=None):
    """
    ...
    log.debug('fetch_app_names response={}, code={}'
