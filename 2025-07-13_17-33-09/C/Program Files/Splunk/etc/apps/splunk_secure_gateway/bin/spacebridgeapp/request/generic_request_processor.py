...
__CACHED_COMPANION_APPS: Optional[cache.CachedResult] = None
...
async def fetch_registered_apps(log: logging.Logger, auth_header, async_splunk_client):
    await cache_companion_app_list(log, auth_header, async_splunk_client)
    return __CACHED_COMPANION_APPS.value


async def cache_companion_app_list(log: logging.Logger, auth_header, async_splunk_client):
    ...
    if not __CACHED_COMPANION_APPS or not __CACHED_COMPANION_APPS.is_valid():
        log.info("Refreshing cached companion app list")
...
async def process_generic_message_request(log: logging.Logger,
                                          request_context, client_request, server_response, async_client_factory):
    """
    ...
    """
    log.info(f"Processing generic message request, request_id={request_context.request_id}, "
    ...
    app_list = await fetch_registered_apps(log, request_context.auth_header, async_splunk_client)

    log.debug(f"Number of registered apps={len(app_list)}")
    ...
    if response.code != HTTPStatus.OK:
        ...
        log.error(f"Error forwarding request to app={app_bundle[APP_NAME_FIELD]}, "
                  f"base_url={app_bundle[BASE_URL_FIELD]}, error={msg}, request_id={request_context.request_id}")
    ...
    log.info("Finished processing generic message request")
