...
class DeleteTokensModularInput(BaseModularInput):
    ...
    log_mod_input_name = "ssg_delete_tokens_modular_input.app"
    ...
    def do_run(self, input_config):
        ...
        if not modular_input_should_run(self.session_key, logger=log):
            log.debug("Modular input will not run on this node.")
            return

        log.info("Running Delete tokens modular input on search captain node")
        delete_tokens_sync = DeleteTokensSync(log, self.session_key)

        try:
            delete_tokens_sync.run()
        except:
            log.exception("Failure encountered while running Delete Tokens sync")
...
class DeleteTokensSync(object):
    ...
    def run(self):
        ...
        """
        self.log.info("Attempting Deleting Invalid Tokens")
        try:
            self.sync()
        except splunk.RESTException as e:
            if e.statusCode == HTTPStatus.SERVICE_UNAVAILABLE:
                self.log.info(f"KVStore is unavailable with status = {e.statusCode}, ssg_delete_tokens modular input "
    ...
    def sync(self):
        ...
        try:
            ...
            self.log.exception("Exception performing DeleteTokensSync")
    ...
    def remove_tokens_slated_for_deletion(self):
        ...
        if response_header.status != HTTPStatus.OK:
            self.log.error(f"Unable to retrieve any tokens from {constants.DELETE_TOKENS_COLLECTION_NAME}. Status: {response_header.status}, Message: {response_content}")
            return
        ...
        if delete_count > 0 or skip_count > 0 or error_count > 0:
            self.log.info(f"Token deletion queue status: {delete_count} deleted, {skip_count} skipped, {error_count} errors")
            if error_count > 0:
                errors_str = "\n".join(delete_token_errors)
                self.log.error(f"Failed to delete {error_count} tokens. Details:\n{errors_str}")

        kv_error_count = len(kv_errors)
        if kv_error_count > 0:
            errors_str = "\n".join(kv_error_count)
            self.log.error(f"Failed to remove {kv_error_count} from kv store. Details:\n{errors_str}")
    ...
    def remove_all_expired_tokens(self):
        ...
        tokens = get_all_secure_gateway_tokens(self.log, self.session_key)
        current_time = get_current_timestamp()
        delete_count = 0
        delete_token_errors = []
        ...
        if delete_count > 0:
            self.log.info(f"Deleted {delete_count} expired tokens")

        delete_errors_count = len(delete_token_errors)
        if delete_errors_count > 0:
            errors_str = "\n".join(delete_token_errors)
            self.log.error(f"Failed to delete {delete_errors_count} tokens. Details:\n{errors_str}")
    ...
    def delete_token_from_splunk(self, username, token_id) -> Tuple[bool, int, str]:
        ...
        """
        try:
            response = delete_token_by_id(self.log, self.session_key, username, token_id)
