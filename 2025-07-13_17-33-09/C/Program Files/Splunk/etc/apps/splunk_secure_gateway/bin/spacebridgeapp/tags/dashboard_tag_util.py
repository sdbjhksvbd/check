...
from spacebridgeapp.util.constants import REGISTERED_DEVICES_COLLECTION_NAME, \
...
async def get_app_id_from_device_id(log: logging.Logger,
                                    auth_header, device_id, user, async_kvstore_client):
    ...
    log.error(f"Unable to fetch app_id for device={device_id}, code={response.code}")
    return None
...
async def get_dashboard_tags(log: logging.Logger,
                             request_context, async_splunk_client, async_kvstore_client):
    """
    ...
    if not is_dashboard_tagging_supported(splunk_version):
        log.debug(f"Dashboard Tagging is not supported for Splunk version={splunk_version}")
        return None

    app_id = await get_app_id_from_device_id(log,
                                             auth_header=request_context.auth_header,
                                             device_id=request_context.device_id,
                                             user=request_context.current_user,
                                             async_kvstore_client=async_kvstore_client)
    if not app_id:
        log.debug("Unable to fetch APP_ID for Dashboard Tag Filtering.")
        return None
...
async def edit_dashboard_xml(request_context, async_splunk_client, edit_link, root_element):
    ...
    :return:
    """
    eai_data = "eai:data=" + urllib.quote(defused_et.tostring(root_element, encoding='unicode', method='xml'))
