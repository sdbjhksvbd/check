...
from spacebridgeapp.util.constants import ITSI, ITSI_GLASS_TABLE
...
def get_glass_table_api_params(dashboard_ids=None, limit=0, offset=0, minimal=False):
    """
...
async def append_glass_table_descriptions(log: logging.Logger,
                                          request_context, dashboard_list, total, continuation_available, offset=0,
            ...
            if limit_avail <= 0 and use_pagination:
                ...
                if response.code == HTTPStatus.OK:
                    ...
                    for glass_table in response_json:
                        dashboard_definition = await to_dashboard_description(log=log,
                                                                              json_object=glass_table,
                                                                              minimal=minimal)
                    ...
                    log.warning(f"Unable to query {ITSI} glass_table api with params={params}")

    return dashboard_list, total, continuation_available


async def fetch_glass_table_dashboard_description(log: logging.Logger,
    ...
    response_json = await response.json()
    dashboard_definition = await to_dashboard_description(log=log, json_object=response_json)
