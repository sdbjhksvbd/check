...
def validate_user(log: logging.Logger, webhook_url, username, verify_ssl):
    """
    ...
    try:
        ...
        log.info("Received status_code={} for user={} from registration webhook".format(r.status_code, username))
...
async def aio_validate_user(log: logging.Logger, webhook_url, username, verify_ssl, async_client: AsyncClient):
    """
    Validate whether a user is allowed to register a device using a webhook. Returns true or raises an exception
    ...
    log.info("Received status_code={} for user={} from registration webhook".format(r.code, username))
