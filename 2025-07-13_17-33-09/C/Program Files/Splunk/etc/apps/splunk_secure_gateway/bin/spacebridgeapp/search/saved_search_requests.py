...
async def fetch_saved_search(log: logging.Logger,
                             auth_header, owner, ref, app, async_splunk_client):
    """
    ...
    if response.code != HTTPStatus.OK:
        ...
        log.error(f"Failed to query saved search. status_code={response.code}, error={error}, ref={ref}, "
                     f"app_name={app_name}")
...
async def fetch_saved_searches(log: logging.Logger,
                               auth_header, owner, app, async_splunk_client):
    """
    ...
    if response.code != HTTPStatus.OK:
        error = await response.text()
        log.error(f"Failed to get all saved searches. status_code={response.code}, error={error}, "
                     f"app_name={app_name}")
        raise SpacebridgeApiRequestError(format_splunk_error(response.code, error), status_code=response.code)
...
async def fetch_saved_search_history(log: logging.Logger,
                                     auth_header, owner, ref, app, async_splunk_client):
    """
    ...
    if response.code != HTTPStatus.OK:
        error = await response.text()
        log.error(f"Failed to query saved search history. status_code={response.code}, error={error}, ref={ref}, "
                     f"app_name={app_name}")
        raise SpacebridgeApiRequestError(format_splunk_error(response.code, error), status_code=response.code)
...
async def dispatch_saved_search(log: logging.Logger,
                                auth_header, owner, ref, app, data, async_splunk_client):
    """
    ...
    if response.code != HTTPStatus.OK and response.code != HTTPStatus.CREATED:
        error = await response.text()
        log.error(f"Failed to create dispatch job saved search. status_code={response.code}, error={error}, "
