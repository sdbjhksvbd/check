...
class AsyncSplunkClient(AsyncNonSslClient):
    ...
    def __init__(self, log: logging.Logger, config: SecureGatewayConfig, uri):
        """
        :param uri: string representing uri to make request to
        """
        self.uri = uri
        super(AsyncSplunkClient, self).__init__(log, config)
    ...
    def get_search_job_uri(self, owner, app_name, sid):
        ...
        https://docs.splunk.com/Documentation/Splunk/9.0.1/RESTREF/RESTsearch#search.2Fjobs.2F.7Bsearch_id.7D
        """
        return self.get_namespaced_rest_endpoint_uri(f'{owner}/{app_name}/search/v2/jobs/{sid}')

    def async_search_job_results_preview_request(self, auth_header, owner='admin', app_name='search', search_id='',
                                                     params=None):
        """
        Make async call to get search job results (or POST if search query specified)
        """
        uri = self.get_search_job_results_preview_uri(owner, app_name, search_id)

        #Must use POST if search param specific (Post processing required)
        if params and params.get('search') is not None:
            return self.async_post_request(uri=uri, data=params, auth_header=auth_header)
        else:
            return self.async_get_request(uri=uri, params=params, auth_header=auth_header)
    ...
    def get_search_job_results_preview_uri(self, owner, app_name, sid):
        ...
        https://docs.splunk.com/Documentation/Splunk/9.0.1/RESTREF/RESTsearch#search.2Fv2.2Fjobs.2F.7Bsearch_id.7D.2Fresults_preview
        """
        return self.get_namespaced_rest_endpoint_uri(f'{owner}/{app_name}/search/v2/jobs/{sid}/results_preview')
    ...
    def async_get_saved_searches(self, auth_header, owner, app_name, ref, count=0, offset=0, search=None):
        """
        Make async get request to Splunk /saved/searches/{ref} to retrieve saved search metadata. If a specific
        app_name is not specified, uses the non-namespaced Splunk URI
        """
        params = {'output_mode': 'json', 'count': count, 'offset': offset}

        # Only add search param if explicitly supplied
        if search:
    ...
    def async_get_namespaced_saved_searches(self, auth_header, owner, app_name, ref, count=0, offset=0, search=None):
        """
        Make async get request to Splunk /saved/searches/{ref} to retrieve saved search metadata. Always uses the
        namespaced rest endpoint URI, regardless of supplied app_name and owner
        """
        params = {'output_mode': 'json', 'count': count, 'offset': offset}
        ...
        rest_uri = self.get_namespaced_rest_endpoint_uri(f'{owner}/{app_name}/saved/searches/{ref}')
        return self.async_get_request(uri=rest_uri, params=params, auth_header=auth_header)

    def async_get_reports(self, auth_header, owner, max_results=0, offset=0, app_name='search'):
        """
        Fetch list of reports as it shows up in splunk web

        """

        # Query used by Splunk Web to fetch reports list
        search = 'NOT ((federated.provider!="")) AND NOT ' + \
        '((is_scheduled=1 AND (alert_type!=always OR alert.track=1 OR ' + \
                 '(dispatch.earliest_time="rt*" AND dispatch.latest_time="rt*" AND actions="*" AND actions!=""))))' + \
                 f' AND ((eai:acl.sharing="user" AND eai:acl.owner="{owner}") OR (eai:acl.sharing!="user"))' + \
                  ' AND is_visible=1'
        ...
        rest_uri = self.get_namespaced_rest_endpoint_uri('-/-/saved/searches')
        return self.async_get_request(uri=rest_uri, params=params, auth_header=auth_header)
