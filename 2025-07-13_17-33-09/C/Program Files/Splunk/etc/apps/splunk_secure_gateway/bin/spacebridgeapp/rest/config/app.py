...
from spacebridgeapp.util.constants import NOBODY, ALERTS_IOS, AR_PLUS, APPLE_TV, SPLUNK_TV, VR, DRONE_MODE, SYSTEM_AUTHTOKEN
...
class AppState(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def __init__(self, command_line, command_arg):
        BaseRestHandler.__init__(self, logname="rest_app_type_configuration")
    ...
    def get(self, request):
        ...
        return get_state_of_app(self.log, app_name, authtoken)
    ...
    def post(self, request):
        ...
        return set_state_of_app(self.log, app_name, authtoken, system_authtoken, new_state)
...
def get_state_of_app(log: logging.Logger, app_name, authtoken):
    """
    ...
    log.info('Retrieved state of app for app_type=%s, application_state=%s' % (app_name, app_state))
...
def set_state_of_app(log: logging.Logger, app_name, authtoken, system_authtoken, new_state):
    """
    ...
    if not new_state:
        delete_all_devices_of_type(log, app_name, authtoken, system_authtoken)
...
def delete_all_devices_of_type(log: logging.Logger, app_type, authtoken, system_authtoken):
    """
    Removes all devices of a given type from all users in the kvstore
    """
    users = get_all_users(authtoken) + [NOBODY]
    for user in users:
        delete_all_devices_of_type_for_user(log, user, app_type, authtoken, system_authtoken)


def delete_all_devices_of_type_for_user(log: logging.Logger, user, app_type, authtoken, system_authtoken):
    """
    ...
    for device in devices:
        if device['device_type'] == app_type:
            delete_device_from_spacebridge(log, device['device_id'], system_authtoken)
