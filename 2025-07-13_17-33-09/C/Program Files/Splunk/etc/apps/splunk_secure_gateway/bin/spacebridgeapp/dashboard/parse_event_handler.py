...
from spacebridgeapp.data.event_handler import SearchHandler, SearchCondition, Init
...
def build_event_actions(log: logging.Logger,  parent_element):
    link = None
    link_element = parent_element.find('link')
    if link_element is not None:
        link = build_link(log, link_element=link_element)
...
def build_link(log: logging.Logger, link_element=None):
    """
    ...
    dashboard_id, input_map = parse_dashboard_link(log, url=url)
...
def to_change_condition(log: logging.Logger, change_element):
    link, sets, evals, unsets = build_event_actions(log, parent_element=change_element)

    condition_elements = change_element.findall("condition")
    conditions = [to_form_condition(log, condition_element) for condition_element in condition_elements]
...
def to_init(log: logging.Logger, init_element):
    link, sets, evals, unsets = build_event_actions(log, parent_element=init_element)

    condition_elements = init_element.findall("condition")
    conditions = [to_form_condition(log, condition_element) for condition_element in condition_elements]

    init = Init(link=link, sets=sets, evals=evals,
                unsets=unsets, conditions=conditions)
    return init


def to_form_condition(log: logging.Logger, condition_element):
    match_attribute = {
    ...
    link, sets, evals, unsets = build_event_actions(log, parent_element=condition_element)
...
def to_drilldown_condition(log: logging.Logger, condition_element):
...
def build_drill_down(log: logging.Logger, drill_down_element):
    """
    ...
    link, list_set, list_eval, list_unset = build_event_actions(log, parent_element=drill_down_element)

    condition_elements = drill_down_element.findall("condition")
    conditions = [to_drilldown_condition(log, condition_element) for condition_element in condition_elements]
...
def build_search_handler(log: logging.Logger, search_handler_element, search_state):
    """Parse a search handler element into Search Handler object"""

    if search_handler_element is None:
        return None

    link, list_set, list_eval, list_unset = build_event_actions(log, parent_element=search_handler_element)

    condition_elements = search_handler_element.findall("condition")

    conditions = [to_search_condition(log, condition_element) for condition_element in condition_elements]
...
def to_search_condition(log: logging.Logger, condition_element):
    match = condition_element.get("match", "")
