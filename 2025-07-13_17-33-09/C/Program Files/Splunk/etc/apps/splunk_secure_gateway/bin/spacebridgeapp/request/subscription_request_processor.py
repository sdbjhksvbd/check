...
from spacebridgeapp.request.report_request_processor import _fetch_single_report
...
from spacebridgeapp.search.input_token_support import set_default_token_values, inject_tokens_into_string, \
    map_default_token_values, EARLIEST, LATEST
...
async def _spawn_search_job(log: logging.Logger, request_context, async_splunk_client, subscription_search, input_tokens, sid):
    owner, app_name, dashboard_name = parse_dashboard_id(subscription_search.dashboard_id)
    sid = await create_job_from_search(log,
                                       request_context.auth_header, subscription_search, app_name,
                                       request_context.current_user, input_tokens, async_splunk_client, sid)

    log.info("Created search job sid={}".format(sid))
    return sid


def log_dashboard_source_subscription(log: logging.Logger,
                                      dashboard_id,
                                      search_type_id,
                                      search_key,
                                      subscription_id):
    ...
    """
    log.info(f"Dashboard Source Subscription Created: "
                f"dashboard_id={dashboard_id}, search_type_id={search_type_id}, search_key={search_key}, "
                f"subscription_id={subscription_id}")


async def process_subscribe_request(log: logging.Logger,
    ...
    if client_subscription_message.clientSubscribeRequest.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_REQUEST):
        log.info(f"Processing subscription request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_REQUEST")
        await process_subscribe_dashboard_visualization_request(log,
                                                                request_context,
        ...
        log.info(f"Processing subscription request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_SAVED_SEARCH_REQUEST")
        await process_subscribe_saved_search_request(log,
                                                     request_context,
                                                     client_subscription_message,
                                                     server_subscription_response,
                                                     async_client_factory)
        ...
        log.info(f"Processing subscription request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_DASHBOARD_INPUT_SEARCH_REQUEST")
        await process_subscribe_dashboard_input_search_request(log,
                                                               request_context,
                                                               client_subscription_message,
                                                               server_subscription_response,
                                                               async_client_factory)
        ...
        log.info(f"Processing subscription request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_UDF_DATASOURCE")
        await process_subscribe_udf_datasource(log,
                                               request_context,
                                               client_subscription_message,
                                               server_subscription_response,
                                               async_client_factory)
        ...
        log.info(f"Processing subscription request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_GENERIC_MESSAGE")
        response = await process_generic_message_request(log,
                                                         request_context,
                                                         client_subscription_message,
                                                         server_subscription_response,
                                                         async_client_factory)
...
async def process_subscribe_update_request(log: logging.Logger,
                                           request_context,
                                           client_subscription_message=None,
                                           server_subscription_response=None,
                                           async_client_factory=None):
    ...
    if client_subscription_update.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP):
        log.info(f"Processing subscription update request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP")
        await process_dashboard_visualization_map_update(log,
        ...
        log.info(f"Processing subscription update request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CHOROPLETH_MAP")
        await process_dashboard_visualization_map_update(log,
                                                         request_context=request_context,
        ...
        log.info(f"Processing subscription update request: "
                 f"request_id={request_context.request_id}, "
                 f"device_id={request_context.device_id}, "
                 f"type=CLIENT_SUBSCRIBE_UPDATE_GENERIC_MESSAGE")
        await process_generic_message_request(log,
                                              request_context,
                                              client_subscription_message,
                                              server_subscription_response,
                                              async_client_factory)
...
async def process_dashboard_visualization_map_update(log: logging.Logger,
                                                     request_context: RequestContext,
    ...
    log.debug(f"Start process_subscription_update subscription_id={subscription_id}, "
              f"request_id={request_context.request_id}, device_id={request_context.device_id}")
    ...
    if get_response.code != HTTPStatus.OK:
        ...
        error_message = f"Update failed. Subscription ID Not Found! " \
                        f"status_code={get_response.code}, error={error}, subscription_id={subscription_id}"
        raise SpacebridgeApiRequestError(error_message, status_code=get_response.code)

    await process_post_search(log,
                              request_context=request_context,
...
async def process_trellis_split_by(log: logging.Logger,
                                   request_context,
    ...
    await process_subscription_update(log,
                                      request_context,
                                      subscription_update,
                                      server_subscription_response,
...
async def process_post_search(log: logging.Logger,
                              request_context: RequestContext,
                              validate_map_update=None,
                              construct_post_search_string=None,
                              dashboard_visualization_map=None,
                              ...
                              server_subscription_response=None,
                              async_kvstore_client=None):
    log.debug(f"Processing post search for request_id={request_context.request_id}, "
              f"device_id={request_context.device_id}")
...
async def process_subscribe_dashboard_input_search_request(log: logging.Logger,
                                                           request_context: RequestContext,
                                                           client_subscription_message=None,
                                                           server_subscription_response=None,
                                                           async_client_factory=None):
    ...
    dashboard_description = await validate_dashboard_search(log,
                                                            request_context=request_context,
    ...
    spawn_search_job = partial(_spawn_search_job, log, request_context, async_splunk_client)

    await lazy_load_subscription_search(log,
                                        request_context=request_context,
                                        dashboard_id=dashboard_id,
    ...
    # Create Subscription
    subscription_id = await create_subscription(log,
                                                request_context=request_context,
                                                search_key=search_key,
    ...
    log_dashboard_source_subscription(log,
                                      dashboard_id=dashboard_id, search_type_id=query_id, search_key=search_key,
                                      subscription_id=subscription_id)


async def process_subscribe_saved_search_request(log: logging.Logger,
                                                 request_context: RequestContext,
                                                 client_subscription_message=None,
                                                 server_subscription_response=None,
                                                 async_client_factory=None):
    ...
    report_id = saved_search_subscribe.savedSearchId
    input_tokens = dict(saved_search_subscribe.inputTokens)

    parsed_report_id = ParsedReportId(report_id)
    earliest = input_tokens[EARLIEST] if EARLIEST in input_tokens else ""
    latest = input_tokens[LATEST] if LATEST in input_tokens else ""

    trellis_enabled = saved_search_subscribe.trellisEnabled
    if trellis_enabled:
        trellis_split_by = saved_search_subscribe.trellisSplitBy
        if not trellis_split_by:
            log.debug("SavedSearch: Trellis split by field not provided by client device, calculating trellis split by field")
            # retrieve more information on the report
            ...
            trellis_split_by = await _get_default_trellis_split_by(log,
                                                                   request_context, async_splunk_client, content.get("qualifiedSearch"))
    else:
        trellis_split_by = None

    search = Search(ref=parsed_report_id.report_name, app=parsed_report_id.app_name,
                    earliest=earliest,
                    latest=latest)

    search_key = generate_search_hash(request_context.shard_id, parsed_report_id.report_name, input_tokens,
                                      user=request_context.current_user, refresh_interval=None)
    ...
    if saved_search_subscribe.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP):
        await process_post_search(log,
                                  request_context=request_context,
                                  construct_post_search_string=construct_cluster_map_post_search,
                                  dashboard_visualization_map=saved_search_subscribe.dashboardVisualizationClusterMap,
                                  subscription_id=subscription_id,
                                  server_subscription_response=server_subscription_response,
                                  async_kvstore_client=async_kvstore_client)

    elif saved_search_subscribe.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CHOROPLETH_MAP):
        await process_post_search(log,
                                  request_context=request_context,
                                  validate_map_update=validate_choropleth_map_params,
                                  construct_post_search_string=construct_choropleth_map_post_search,
                                  dashboard_visualization_map=saved_search_subscribe.dashboardVisualizationChoroplethMap,
                                  subscription_id=subscription_id,
                                  server_subscription_response=server_subscription_response,
                                  async_kvstore_client=async_kvstore_client)


async def process_subscribe_udf_datasource(log: logging.Logger,
                                           request_context: RequestContext,
                                           client_subscription_message,
                                           server_subscription_response,
                                           async_client_factory):
    ...
    if search_defn:
        ...
        refresh_interval = refresh_to_seconds(log, search_defn.refresh)
    ...
    if search_defn:
        ...
        await load_udf_ds_test_subscription_search(log,
                                                   request_context=request_context,
                                                   dashboard_id=dashboard_id,
                                                   search_key=search_key,
    ...
    # Log the Dashboard Source Subscription
    log_dashboard_source_subscription(log,
                                      dashboard_id=dashboard_id, search_type_id=data_source_id, search_key=search_key,
                                      subscription_id=subscription_id)


async def process_subscribe_dashboard_visualization_request(log: logging.Logger,
                                                            request_context: RequestContext,
                                                            client_subscription_message,
                                                            server_subscription_response,
                                                            async_client_factory):
    ...
    will return subscription_id in successful subscription
    """
    log.debug("process_subscribe_dashboard_visualization_request start")
    # async clients
    async_splunk_client = async_client_factory.splunk_client()
    async_kvstore_client = async_client_factory.kvstore_client()
    async_itsi_client = async_client_factory.itsi_client()
    ...
    search_key = generate_search_hash(dashboard_id, search_id, input_tokens,
                                      user=request_context.current_user,
                                      refresh_interval=refresh_to_seconds(log, visualization.search.refresh))
    ...
    if trellis_enabled:
        ...
        if not trellis_split_by:
            log.debug("Trellis split by field not provided by client device, calculating trellis split by field")
            search_query = create_search_query(visualization.search.query)
            trellis_split_by = await _get_default_trellis_split_by(log,
                                                                   request_context, async_splunk_client, search_query)
    else:
    ...
    # Log the Dashboard Source Subscription
    log_dashboard_source_subscription(log,
                                      dashboard_id=dashboard_id, search_type_id=visualization_id, search_key=search_key,
                                      subscription_id=subscription_id)

    if dashboard_visualization_subscribe.HasField(CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP):
        log.info(f"Processing post search for request_id={request_context.request_id}, "
                 f"subscription_id={subscription_id}, type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CLUSTER_MAP")
        await process_post_search(log,
                                  request_context=request_context,
                                  construct_post_search_string=construct_cluster_map_post_search,
        ...
        log.info(f"Processing post search for request_id={request_context.request_id}, "
                 f"subscription_id={subscription_id}, type=CLIENT_SUBSCRIBE_DASHBOARD_VISUALIZATION_CHOROPLETH_MAP")
        await process_post_search(log,
                                  request_context=request_context,
                                  validate_map_update=validate_choropleth_map_params,
                                  construct_post_search_string=construct_choropleth_map_post_search,
...
async def _get_default_trellis_split_by(log: logging.Logger,
                                        request_context, async_splunk_client, search_query):
    """
    Private helper to get the default trellis split by column for a given search query
    :param request_context:
    ...
    if response.code == HTTPStatus.OK:
        ...
        return default_split_by_field
    else:
        message = await response.text()
        log.error(f"Could not get search AST for query={search_query} with error={message}. "
                     f"Calculating default trellis split by")
        return None

async def load_udf_ds_test_subscription_search(log: logging.Logger,
                                               request_context, input_tokens, search_key, data_source_id, dashboard_id,
    ...
    log.info(f"Creating subscription search object in KVStore from load_udf_ds_test_subscription_search, "
             f"request_id={request_context.request_id}")
    http_result = await create_search(log,
                                      request_context,
                                      subscription_search,
                                      async_kvstore_client)
...
async def lazy_load_subscription_search(log: logging.Logger,
                                        request_context: RequestContext, input_tokens, search_key,
    ...
    if search_defn.base:
        ...
        if parent_search_defn:
            ...
            refresh_interval = refresh_to_seconds(log, parent_search_defn.refresh)
        ...
        log.debug(f"Base search exists, base={parent_search_defn}, request_id={request_context.request_id}")
        ...
        parent = await lazy_load_subscription_search(log,
                                                     request_context=request_context,
                                                     input_tokens=input_tokens,
    ...
    subscription_search = build_subscription_search(log,
                                                    request_context, dashboard_id, search_defn, search_query,
    ...
    job_status = await get_search_job_content(log,
                                              request_context.auth_header, subscription_search.owner,
                                              SPACEBRIDGE_APP_NAME, subscription_search.sid, async_splunk_client,
                                              params=params)
    if not job_status:
        log.info(f"Start search job, sid={sid}, request_id={request_context.request_id}")
        await spawn_search_job(subscription_search, input_tokens, sid)

    log.info(f"Creating subscription search object in KVStore, sid={sid}, request_id={request_context.request_id}")
    http_result = await create_search(log,
                                      request_context,
                                      subscription_search,
                                      async_kvstore_client)
    ...
    if http_result not in [HTTPStatus.CREATED, HTTPStatus.CONFLICT]:
        log.info(f"Failed to create subscription search for request_id={request_context.request_id}, sid={sid}")
        raise SpacebridgeApiRequestError(message='Failed to create subscription search', status_code=http_result)

    return subscription_search


def build_subscription_search(log: logging.Logger,
                              request_context,
                              dashboard_id,
                              search_defn,
                              search_query,
    ...
    try:
        ...
        refresh_interval_seconds = refresh_to_seconds(log, search_defn.refresh)
        ...
        log.exception("Failed to build subscription_search")
        raise e

    return subscription_search


async def create_search(log: logging.Logger,
                        request_context,
                        subscription_search,
                        async_kvstore_client):
    """
    ...
    http_result = await save_search(log,
                                    request_context=request_context,
...
async def save_search(log: logging.Logger,
                      request_context,
    ...
    if response.code == HTTPStatus.OK or response.code == HTTPStatus.CREATED:
        log.debug("Subscription Search Created. search_key=%s", subscription_search.key())
    ...
    if response.code == HTTPStatus.CONFLICT:
        log.debug("Subscription Search was already Created. search_key=%s", subscription_search.key())
...
async def _create_subscription(log: logging.Logger,
                               request_context, subscription, async_kvstore_client):
    ...
    if response.code == HTTPStatus.OK or response.code == HTTPStatus.CREATED or response.code == HTTPStatus.CONFLICT:
        log.debug("Subscription Created. subscription_id=%s, expired_time=%s",
...
async def _create_subscription_credentials(log: logging.Logger,
                                           request_context, auth, async_kvstore_client):
    # create subscription and return _key
    ...
    if response.code == HTTPStatus.OK or response.code == HTTPStatus.CREATED:
        log.debug("Subscription Created. subscription_id=%s", auth.subscription_id)
        return auth
    else:
        error = await response.text()
...
async def fetch_session_key_and_type(log: logging.Logger,
                                     request_context, async_splunk_client):
    """
    Function that returns session_key and session_key type
    """
    if isinstance(request_context.auth_header, JWTAuthHeader):
        log.debug("JWTAuthHeader detected. Setting session_key_type = {}".format(JWT_TOKEN_TYPE))
        session_key_type = JWT_TOKEN_TYPE
        session_key = request_context.auth_header.token
    else:
        log.debug("SplunkAuthHeader detected. Setting session_key_type = {}".format(SPLUNK_SESSION_TOKEN_TYPE))
...
async def create_subscription(log: logging.Logger,
                              request_context,
                              ttl_seconds,
                              search_key,
                              shard_id,
    ...
    if isinstance(request_context.auth_header, JWTAuthHeader):
        log.debug("JWTAuthHeader detected. Setting session_key_type=%s", JWT_TOKEN_TYPE)
        session_type = JWT_TOKEN_TYPE
        session_key = request_context.auth_header.token
    else:
        log.debug("SplunkAuthHeader detected. Setting session_key_type=%s", SPLUNK_SESSION_TOKEN_TYPE)
    ...
    await asyncio.gather(
        _create_subscription(log, request_context, subscription, async_kvstore_client),
        _create_subscription_credentials(log, request_context, auth, async_kvstore_client)
    )

    log.info(f"Subscription created. subscription_id={subscription_id}, "
             f"search_key={search_key}, request_id={request_context.request_id}")
    return subscription_id


async def process_unsubscribe_request(log: logging.Logger,
                                      request_context: RequestContext,
    ...
    server_subscription_response.subscriptionId = subscription_id
    log.debug(f"Start process_unsubscribe_request subscription_id={subscription_id}, "
              f"request_id={request_context.request_id}, device_id={request_context.device_id}")
    ...
    if response.code == HTTPStatus.NOT_FOUND:
        ...
        error = await response.text()
        log.debug(f"Subscription Id not found. status_code={response.code}, error={error}, "
                  f"subscription_id={subscription_id}, request_id={request_context.request_id}")
    elif response.code != HTTPStatus.OK:
        error = await response.text()
        log.error(f"Failed to unsubscribe. status_code={response.code}, error={error}, "
                  f"subscription_id={subscription_id}, request_id={request_context.request_id}")
        raise SpacebridgeApiRequestError(error, status_code=response.code)

    log.info(f"Finished process_unsubscribe_request subscription_id={subscription_id}, "
             f"request_id={request_context.request_id}")

    server_subscription_response.serverUnsubscribeResponse.SetInParent()


async def process_ping_request(log: logging.Logger,
                               request_context: RequestContext,
                               client_single_subscription=None,
    ...
    log.debug(f"Process subscription ping request, request_id={request_context.request_id}, "
              f"device_id={request_context.device_id}, subscription_id={subscription_id}")
...
async def process_subscription_update(log: logging.Logger,
                                      request_context: RequestContext,
                                      subscription,
                                      server_subscription_response=None,
                                      async_kvstore_client=None):
    ...
    log.debug(f"Start process_subscription_update subscription_key={subscription.key()}, "
              f"request_id={request_context.request_id}, device_id={request_context.device_id}")
    ...
    if post_response.code not in [HTTPStatus.OK, HTTPStatus.CREATED]:
        ...
        error_message = f"Failed to update subscription search_update. status_code={post_response.code}, error={error}, " \
                        f"subscription_key={search_update_key}, request_id={request_context.request_id}"
    ...
    log.debug(f"Subscription Post Search Updated subscription_key={search_update_key}, "
              f"post_search={subscription.post_search}, request_id={request_context.request_id}")
