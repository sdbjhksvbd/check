...
from spacebridgeapp.util.constants import SERVER_SUBSCRIPTION_RESPONSE, \
...
from spacebridgeapp.request.request_list import SINGLE_REQUESTS, SUBSCRIPTION_REQUESTS, ENCRYPTION_CONTEXT, ClientRequestDomain
...
async def post_process_message(log: logging.Logger,
                               request_context,
    ...
    if input_server_application_message.HasField(SERVER_SUBSCRIPTION_RESPONSE):
        try:
            ...
            if server_subscription_response.HasField(SERVER_SUBSCRIBE_RESPONSE):
                ...
                if not server_subscription_response.serverSubscribeResponse.WhichOneof("subscription_type"):
                    log.info(f"Start Post Process Single Subscription Update. subscription_id={subscription_id}, "
                    ...
                    await subscription_processor.process_subscription(log,
                                                                      request_context,
                                                                      subscription_id,
            ...
            log.exception("SpacebridgeError during post_process_message")
            e.set_proto(server_subscription_update)
        except Exception as e:
            log.exception("Unhandled exception during post_process_message")
        ...
        if not server_subscription_update.WhichOneof('subscription_update'):
            log.info(f"No Post Processing Response required for subscription_id={subscription_id}")
            return None
...
async def process_message(log: logging.Logger,
                          message_sender, client_application_message, server_application_message, async_client_factory,
    ...
    if client_application_message.HasField(CLIENT_SINGLE_REQUEST):
        ...
        log.warn("No suitable message type found client application request")
        return "device_id={}".format(device_id)

    response_object.requestId = server_response_id

    if request_object.HasField("runAs"):
        log.debug("run as credentials is present")
        auth_header = await parse_run_as_credentials(log,
                                                     encryption_context, system_auth_header,
                                                     async_client_factory, request_object.runAs)
    else:
        auth_header = parse_session_token(log, encryption_context, request_object.sessionToken)
    ...
    try:
        validate_client_version(log, request_object, response_object)
        await auth_header.validate(async_client_factory.splunk_client(), log, request_context)
        ...
        should_send_response = await ctx.run(processor, log, request_context, encryption_context, request_object,
        ...
        log.warning("CompanionAppError error=%s", e.message)
        e.set_proto(response_object)
    except SpacebridgeError as e:
        log.exception("SpacebridgeError during process_message")
        e.set_proto(response_object)
    except Exception as e:
        log.exception("Unhandled exception during process_message")
        response_object.error.code = common_pb2.Error.ERROR_UNKNOWN
        response_object.error.message = str(e)
    log.info('Finished processing message. {}'.format(request_context))
    return request_context


def validate_client_version(log: logging.Logger, request_object, response_object):
    user_agent = request_object.userAgent or "invalid"
    user_agent_parts = user_agent.split('|')
    app_id = user_agent_parts[0]
    log.debug("Checking {} version {}".format(app_id, request_object.clientVersion))
    if not is_version_ok(log, app_id, request_object.clientVersion):
...
async def handle_spacebridge_message(log: logging.Logger,
                                     auth_header, spacebridge_message, async_client_factory, encryption_context):
    """
    :param log:
    :param auth_header:
    ...
    if spacebridge_message.HasField(UNREGISTER_EVENT):
        log.info("Spacebridge unregister event")
        await unregister_device(log,
                                auth_header,
    ...
    if spacebridge_message.HasField(MDM_REGISTRATION_REQUEST):
        request_id = get_guid()
        log.info(f"message=RECEIVED_ENVELOPE type={MDM_REGISTRATION_REQUEST}, "
                    f"creating request_id={MDM_REGISTRATION_REQUEST}")
        await mdm_authentication_request(log,
                                         auth_header,
        ...
        log.info("Unknown spacebridge message received")


async def process_request(log: logging.Logger,
                          request_context,
                          encryption_context,
    ...
    """ Accepts the input client single request proto and a server
        ...
        Arguments:
         ...
         Return:
            N
    """
    should_send = await process_request_list(log,
                                             request_context,
                                             client_single_request,
                                             server_single_response,
                                             async_client_factory,
    ...
    if client_single_request.HasField(RequestType.CONNECTIVITY_TEST_REQUEST.value):
        log.info("type={}".format(RequestType.CONNECTIVITY_TEST_REQUEST.name))
        asyncio.create_task(send_websocket_metrics_to_telemetry(log,
            RequestType.CONNECTIVITY_TEST_REQUEST.name, request_context, async_client_factory.telemetry_client(),
            useragent=client_single_request.userAgent, params=get_params_for_metrics(client_single_request)))
...
async def process_request_list(log: logging.Logger,
                               request_context,
                               client_message,
                               server_response,
                               async_client_factory,
                               request_list: ClientRequestDomain,
                               encryption_context):
    """
    ...
    for request in request_list:
        enum, process_function, extra_args = request
        if client_message.HasField(enum.value):
            log.info("type={}".format(enum.name))
            if enum.name != "CLIENT_SUBSCRIPTION_PING":
                asyncio.create_task(send_websocket_metrics_to_telemetry(log,
                    enum.name, request_context, async_telemetry_client, useragent=useragent,
                    params=get_params_for_metrics(client_message)))

            # Build argument map
            kwargs = {arg: async_client_factory.from_value(arg) for arg in extra_args}

            if ENCRYPTION_CONTEXT in extra_args:
