...
from spacebridgeapp.util.constants import DEVICE_NAME, DEVICE_TYPE, DEVICE_ID, APP_ID, \
    ...
    AUTH_CODE, USERNAME, BODY, PASSWORD, TEMP_KEY, SELF_REGISTER, \
    QUERY, SESSION, USER, AUTHTOKEN, SYSTEM_AUTHTOKEN, PAYLOAD, \
...
from spacebridgeapp.rest.registration.util import RegistrationMethod, AuthMethod, \
...
from spacebridgeapp.rest.services.splunk_service import authenticate_splunk_credentials, get_tokens_enabled
from http import HTTPStatus
from spacebridgeapp.rest.util.errors import SSGHTTPStatus
...
class CompletePairingHandler(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def __init__(self, command_line, command_arg):
        BaseRestHandler.__init__(self, logname="rest_registration_auth_code")
    ...
    def post(self, request):
        ...
        return handle_confirmation(config, self.log,
                                   auth_code, auth_method, self_register, user, session_token,
                                   system_authtoken, body, self.async_client_factory)


def handle_confirmation(config: SecureGatewayConfig, log: logging.Logger,
                        auth_code, auth_method, self_register, user, session_token, system_authtoken, body,
    ...
    if register_with_jwt:
        validate_registration_via_webhook(config, log, user)
    else:
        username = extract_parameter(body, USERNAME, BODY)
        password = extract_parameter(body, PASSWORD, BODY)
        validate_registration_via_webhook(config, log, username)
        username = handle_local_auth(log, user, username, password)
    ...
    deployment_friendly_name = get_deployment_friendly_name(log, system_authtoken)
    ...
    if register_with_jwt:
        if is_valid_session_token(user, session_token):
            try:
                ...
                if not get_tokens_enabled(log, system_authtoken):
                    return token_disabled_error_response()
                log.error("Failed to fetch jwt token for user={} with message={}".format(user, str(e)))
                raise e
        else:
    ...
    try:
        version_get_response = version_get_response_sync_adapter(log,
                                                                 session_token,
                                                                 device_encryption_info.app_id,
                                                                 temp_record[DEVICE_NAME],
                                                                 async_client_factory,
                                                                 registration_info)
    except Exception as e:
    ...
    log.info(
        f"Device registration confirmed. Device for user={user}, with type={temp_record[DEVICE_TYPE]}, and app={temp_record[APP_NAME]} was recorded in kvstore")

    # Return success message
    return success_response(credentials, register_with_jwt)


def handle_local_auth(log: logging.Logger, user: str, username: str, password: str) -> str:
    """
    Method to get current-context from username and password for local auth.
    Returns username retrieved from current-context.
    """
    try:
        ...
        log.exception('Invalid credentials passed to login api')
        raise e

    log.info("Received new registration confirmation request by user={user} for device_owner={username}")
    return username


def token_disabled_error_response():
    """
    ...
    """
    return {
        'payload': 'Registration Error: Tokens are disabled. Please enable tokens to complete registration.',
        'status': SSGHTTPStatus.SPLUNK_TOKENS_DISABLED,
    }
