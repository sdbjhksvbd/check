...
class MigrationHandler(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def __init__(self, command_line, command_arg):
        BaseRestHandler.__init__(self, logname="ssg_migration_handler")

    def get(self, request):
        system_authtoken = request[SYSTEM_AUTHTOKEN]

        status, migration_data = get_migration_status(self.log, system_authtoken)
    ...
    def post(self, request):
        ...
        if RUN in body:
            worker = Migration(self.log, system_authtoken)
            worker.run()
            return {
                'payload': {
                    'RUN': "success",
...
def get_migration_status(log: logging.Logger, system_authtoken):
    """
    ...
    try:
        ...
        spacebridge_server_bundle = get_current_spacebridge_server_bundle(log, system_authtoken)
