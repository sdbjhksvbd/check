...
LOG_FILENAME = f"{constants.SPACEBRIDGE_APP_NAME}_kvstore_service.log"
LOG_NAME = "kvstore_service"
class KVStoreCollectionAccessObject(object):
    def __init__(self, collection=None, session_key=None, app=constants.SPACEBRIDGE_APP_NAME,
        ...
        if session_key is not None:
            ...
            self.log = setup_logging(LOG_FILENAME, LOG_NAME, config=config)
        else:
            self.log = setup_logging(LOG_FILENAME, LOG_NAME, level=logging.INFO)
    ...
    def get_item_by_key(self, key):
        ...
        self.log.debug("get_item_by_key url={}".format(request_url))
        return rest.simpleRequest(
            request_url,
            sessionKey=self.session_key,
            method='GET',
    ...
    def get_items_by_query(self, query, sort=None, limit=0):
        ...
        self.log.debug("get_items_by_query, query={}".format(query))
        return rest.simpleRequest(
            self.uri,
            sessionKey=self.session_key,
            method='GET',
    ...
    def get_item_by_keys(self, keys):
        ...
        return self.get_items_by_query(query=query)
    ...
    def delete_item_by_key(self, key):
        ...
        :return: response, content
        """
        self.log.debug('deleting from kvstore. uri:{}'.format(self.uri))
