...
class Migration(object):
    ...
    def _migrate(self, meta_collection, migration_info):
        ...
        for key in password_keys:
            try:
                value = fetch_sensitive_data(self.log, self.session_key, key, app=CLOUDGATEWAY_APP_NAME)
                update_or_create_sensitive_data(self.log, self.session_key, key, value)
            except splunk.ResourceNotFound:
                self.log.debug('key=%s not found in storage/passwords', key)
    ...
    def run(self):
        ...
        """
        self.log.info("Attempting Migration from Splunk Cloud Gateway to Splunk Secure Gateway")
        try:
            ...
            if migration_status.isdigit() and int(migration_status):
                self.log.debug("Migration will not run because migration from Splunk Cloud Gateway "
                             "is already done")
            else:
            ...
            if e.statusCode == HTTPStatus.SERVICE_UNAVAILABLE:
                self.log.info(f"KVStore is unavailable with status = {e.statusCode}, ssg migration "
