...
class ClientMessageHandler(AbstractMessageHandler):
    ...
    def __init__(self, encryption_context, server, logger, request, responses, websocket_ctx, is_subscription):
        ...
        if hasattr(encryption_context, 'session_key'):
            config = load_config(encryption_context.session_key)
...
def run_wss_test(log: logging.Logger,
                 request_jsn, credentials_bundle, client_encryption_ctx, server_info, request_id,
    ...
    client_message_handler = ClientMessageHandler(client_encryption_ctx, server_info, log, request, responses,
