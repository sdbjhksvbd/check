...
async def _fetch_credential_data(log: logging.Logger,
                                 user_auth_header,
    ...
    await _ensure_valid_response(log, credential_get_response, [HTTPStatus.OK], "Failed to refresh session token")
...
class SubscriptionClient(object):
    ...
    async def flush(self, auth_header):
        try:
            ...
            self.log.exception("failed to flush cache")
        except Exception as e:
            self.log.exception("Unhandled error flushing cache")
            raise e
    ...
    async def _update_credential(self, user, auth_header):
        ...
        credential = await _fetch_credential_data(self.log, auth_header, self.kvstore_client)
        ...
        self.log.debug("flush update credentials")
