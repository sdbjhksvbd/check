...
async def _load_many(log: logging.Logger,
                     auth_header, collection, params, async_kvstore_client,
...
async def _load_subscriptions(log: logging.Logger,
                              system_auth_header, searches, async_kvstore_client):
    if len(searches) == 0:
    ...
    subscriptions = await _load_many(log,
                                     system_auth_header, SUBSCRIPTIONS_COLLECTION_NAME, params, async_kvstore_client,
                                     mapper_fn=Subscription.from_json)

    return subscriptions


async def _load_subscription_credentials(log: logging.Logger,
                                         system_auth_header, subscriptions, async_kvstore_client):
    params = {}
    loaded_credentials = {}
    users = {subscription.user for subscription in subscriptions}

    for user in users:
        credentials = await _load_many(log,
                                       system_auth_header, SUBSCRIPTION_CREDENTIALS_COLLECTION_NAME, params,
        ...
        if credential.user not in loaded_credentials:
            log.warning("Did not find credentials for user=%s", user)

    return loaded_credentials


async def _load_search_updates(log: logging.Logger,
                               system_auth_header, subscriptions, async_kvstore_client):
    ...
    params = {QUERY: json.dumps(condition)}
    search_updates = await _load_many(log,
                                      system_auth_header, SEARCH_UPDATES_COLLECTION_NAME,
...
async def load_search_bundle(log: logging.Logger,
                             system_auth_header: str, shard_id: str, async_kvstore_client: AsyncKvStoreClient):
    searches_all = await fetch_searches(log, system_auth_header, async_kvstore_client)
    ...
    subscriptions_list = await _load_subscriptions(log, system_auth_header, searches_for_shard, async_kvstore_client)
    subscriptions_map = _collect(subscriptions_list, SUBSCRIPTION_KEY)

    subscription_credentials = await _load_subscription_credentials(log, system_auth_header, subscriptions_list, async_kvstore_client)
    search_updates = await _load_search_updates(log, system_auth_header, subscriptions_list, async_kvstore_client)
