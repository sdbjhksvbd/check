...
from spacebridgeapp.dashboard.parse_event_handler import build_drill_down, build_event_actions, to_init
...
from spacebridgeapp.util.constants import VALID_CHART_TYPES
from splapp_protocol import common_pb2


async def to_dashboard_definition(log: logging.Logger,
    ...
    for search_element in search_elements:
        ...
        v, search = build_dashboard_visualization_search(log,
                                                         search_element=search_element,
                                                         dashboard_refresh=dashboard_refresh,
                                                         show_refresh=show_refresh)

        dashboard_visualization = DashboardVisualization(visualization_type=common_pb2.DashboardVisualization.DASHBOARD_VISUALIZATION_SEARCH,
                                                         search=search,
                                                         visualization_id=v)
    ...
    input_token_set = InputTokenSet(log)
    ...
    for row_index, row_element in enumerate(row_elements):
        dashboard_row, ar_compatible_row = await to_dashboard_row(log,
...
async def to_dashboard_row(log: logging.Logger,
                           request_context, app_name, row_element, row_index, dashboard_refresh=None, show_refresh=True,
    ...
    for panel_index, panel_element in enumerate(panel_elements):
        dashboard_panel, ar_compatible_panel = await to_dashboard_panel(log,
                                                                        request_context=request_context,
                                                                        app_name=app_name,
...
async def to_dashboard_panel(log: logging.Logger,
                             request_context, app_name, panel_element, row_index, panel_index, dashboard_refresh=None,
...
async def to_dashboard_visualization(log: logging.Logger,
                                     request_context,
                                     app_name,
                                     element,
                                     row_index,
    ...
    for search_element in search_elements:
        ...
        if search_type != 'annotation':
            visualization_id, search = build_dashboard_visualization_search(log,
                                                                            search_element=search_element,
                                                                            row_index=row_index,
                                                                            panel_index=panel_index,
                                                                            dashboard_refresh=dashboard_refresh,
    ...
    drill_down = build_drill_down(log, drill_down_element=drill_down_element)
