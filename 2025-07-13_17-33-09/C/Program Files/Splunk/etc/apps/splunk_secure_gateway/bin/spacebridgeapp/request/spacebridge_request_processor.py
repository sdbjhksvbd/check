...
async def _find_owner(log: logging.Logger,
                      auth_header, device_id, all_users, async_kvstore_client):
    """
...
async def unregister_device(log: logging.Logger,
                            auth_header, unregister_event, async_splunk_client, async_kvstore_client):
    """
    :param log:
    :param auth_header: A system auth header
    ...
    if http_code == HTTPStatus.OK:
        user = await _find_owner(log, auth_header, event_device_id, all_users, async_kvstore_client)
    else:
        log.warning("Failed to list all users, status=%s" % http_code)
    ...
    if user:
        ...
        if len(devices_json) <= 0:
            log.info("Unregister device, user=%s has no remaining registered devices", format(user))
...
async def mdm_authentication_request(log: logging.Logger,
                                     auth_header, mdm_auth_request, async_client_factory, encryption_context,
                                     request_id):

    log.info("starting mdm_authentication_request, request_id={}".format(request_id))
    try:
        mdm_registration_ctx = CloudgatewayMdmRegistrationContext(log, auth_header, async_client_factory)

        config = load_config(auth_header.session_token)
        result = await handle_mdm_authentication_request(mdm_auth_request, encryption_context, mdm_registration_ctx,
                                                         log, config, request_id)
        log.info("completed mdm_authentication_request, request_id={}".format(request_id))
        return result
    except Exception as e:
        log.exception("Unexpected exception occured during mdm_authentication_request={}, request_id={}"
                         .format(e, request_id))
...
class CloudgatewayMdmRegistrationContext(ServerRegistrationContext):
    ...
    def __init__(self, log: logging.Logger,
                 system_auth_header, async_client_factory: AsyncClientFactory):
        self.log = log
    ...
    async def validate_username_password(self, username, password):
        ...
        self.log.info("valid_session_token=false with message={}, status_code={}".format(message, response.code))
    ...
    async def validate(self, username, password, device_info):
        ...
        :raises CloudgatewayMdmRegistrationError
        """
        config = load_config(self.system_auth_header.session_token)
        registration_webhook_url = config.get_registration_webhook_url()

        if registration_webhook_url:
            self.log.debug('Attempting to validate user via registration webhook')
            await aio_validate_user(self.log,
                                    registration_webhook_url,
                                    username,
                                    config.get_webhook_verify_ssl(),
                                    self.async_client)
            self.log.debug('Successfully validated that user via registration webhook')

        self.log.debug("validating username and password")
        await self.validate_username_password(username, password)
        self.log.debug("completed mdm validation")
        return True
    ...
    async def create_session_token(self, username, password):  # pylint: disable=no-self-use
        ...
        try:
            ...
            self.log.info("Successfully fetched jwt token")
        except Exception as e:
            self.log.info("Failed to fetch jwt token with message={}. Using basic session token instead.".format(e))
    ...
    async def get_deployment_name(self):
        ...
        meta_info = await async_get_meta_info(self.log, self.system_auth_header, self.async_kvstore_client)
    ...
    async def get_mdm_signing_key(self):
        ...
        """
        self.log.info("Fetching mdm signing key")
        ...
        if r.code == HTTPStatus.OK:
            ...
            self.log.info("successfully fetched mdm public key={}".format(mdm_signing_key))

            return b64decode(mdm_signing_key)

        self.log.error("Could not fetch mdm signing key with response={}".format(r.code))
        return ""
    ...
    async def persist_device_info(self, device_info, username):
        ...
        self.log.info("Received response_code={} back on add user to registered users collection".format(r.code))
    ...
    async def get_environment_meta(self, device_info, username, registration_info=None):
        ...
        try:
            ...
            # Companion Apps
            companion_app_list = await fetch_registered_apps(self.log, self.system_auth_header, self.async_splunk_client)
            ...
            self.log.exception("Exception fetching environment data")
