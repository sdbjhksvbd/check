...
class Alert(SpacebridgeAppBase):
    ...
    def set_protobuf(self, proto: common_pb2.Alert):
...
class Notification(SpacebridgeAppBase):
    ...
    def __init__(self, severity=0,
        ...
        self.display_app_name = str(display_app_name)
        self.notification_type = notification_type
        self.saved_search = saved_search
        self.was_snoozed = was_snoozed

    def set_protobuf(self, proto: common_pb2.Alert.Notification):
        ...
        proto.savedSearch = self.saved_search
        proto.wasSnoozed = self.was_snoozed
        if self.call_to_action:
            self.call_to_action.set_proto(proto.callToAction)

        proto.notificationType = self.notification_type
...
class Detail(SpacebridgeAppBase):
    ...
    def set_protobuf(self, proto: common_pb2.Alert.Detail):
    ...
    def __eq__(self, obj):
        ...
        if isinstance(obj, self.__class__):
            return obj.device_id == self.device_id and \
                   ...
                   obj.timestamp == self.timestamp and \
                   obj.was_snoozed == self.was_snoozed
        else:
            return False
