...
def parse_session_token(log: logging.Logger, encryption_context, encrypted_session_token):
    """
    :param log:
    ...
    try:
        ...
        log.exception("Exception encountered while parsing session token")
...
async def parse_run_as_credentials(log: logging.Logger,
                                   encryption_context, system_auth_header, async_client_factory, run_as_credentials):
    """
    :param log:
    :param encryption_context: An encryption context with encryption public and private keys
    ...
    verify_run_as_token_not_expired(log, session_token_with_expiry)
    ...
    return parse_session_token(log, encryption_context, session_token_with_expiry.sessionToken)


def verify_run_as_token_not_expired(log: logging.Logger, session_token_with_expiry):
...
def process_device_credentials_validate_request(log: logging.Logger,
                                                request_context,
    ...
    """
    log.debug("Processing process_device_credentials_validate_request")
...
async def process_complete_device_registration_request(log: logging.Logger,
                                                       request_context,
                                                       client_single_request,
                                                       server_single_response,
                                                       async_client_factory):
    ...
    if registration_query_response.code != HTTPStatus.OK:
        if registration_query_response.code in MESSAGE_VALUES:
            ...
            if response_text.find("Expired authentication code"):
                ...
                log.info("Response code: %s" % registration_query_response.code)
        else:
            server_single_response.completeDeviceRegistrationResponse.code = \
                request_pb2.CompleteDeviceRegistrationResponse.Code.Value('ERROR_VALIDATION_FAILED')
        log.info("Registration query failed. Response code: %s" % registration_query_response.code)
    else:
        ...
        if response_json:
            ...
            if response_confirmation_code != confirmation_code:
                ...
                log.info("Confirmation code mismatch. Received %s Expected %s" %
                ...
                if registration_confirmation_response.code in MESSAGE_VALUES:
                    ...
                    server_single_response.completeDeviceRegistrationResponse.code = \
                        request_pb2.CompleteDeviceRegistrationResponse.Code.Value('ERROR_VALIDATION_FAILED')
                    log.info("Response code: %s" % registration_confirmation_response.code)
        else:
            server_single_response.completeDeviceRegistrationResponse.code = \
                request_pb2.CompleteDeviceRegistrationResponse.Code.Value('ERROR_VALIDATION_FAILED')
            log.info("Registration query was missing body. Response code: %s" % registration_query_response.code)
