...
class SubscriptionHandler(BaseRestHandler, PersistentServerConnectionApplication):
    ...
    def get(self, request):
        ...
        if path:  # search using path as key
            self.log.debug('Fetching subscription with key %s', path)
            try:
            ...
            if query:
                self.log.debug('Fetching subscription(s) with query %s', query)
                response_header, response = kvstore_object.get_items_by_query(query)
            else:
                self.log.debug('Fetching all subscriptions')
    ...
    def post(self, request):
        ...
        if subscription.key():  # update case
            return update_subscription(self.log, kvstore_object=kvstore_object, subscription=subscription)
        else:  # create case
    ...
    def delete(self, request):
        ...
        if path:  # delete by path as key
            return delete_subscription_by_keys(self.log, kvstore_object=kvstore_object, keys=[path])
        ...
        return delete_subscription_by_keys(self.log, kvstore_object, keys)


def create_subscription(log: logging.Logger, kvstore_object, subscription, session_key):
    ...
    log.debug('Creating subscription with body: %s', subscription_json)
    ...
    try:
        ...
        log.debug('Created subscription with key %s', key)
        ...
        log.debug('Failed to create subscription with error: %s', e)
        return {
...
def update_subscription(log: logging.Logger, kvstore_object, subscription):
    key = subscription.key()
    subscription_json = jsonpickle.encode(subscription, unpicklable=False)  # to capture defaults
    log.debug('Updating subscription with key %s and body %s', key, subscription_json)
    ...
    try:
        ...
        log.debug('Failed to update subscription with error: %s', e)
        return {
...
def delete_subscription_by_keys(log: logging.Logger, kvstore_object, keys):
    query = build_containedin_clause(constants.KEY, keys)
    log.debug('Deleting subscriptions with query %s', query)
