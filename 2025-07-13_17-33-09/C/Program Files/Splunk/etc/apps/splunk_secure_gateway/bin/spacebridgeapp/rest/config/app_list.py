...
from spacebridgeapp.util.constants import SYSTEM_AUTHTOKEN, AUTHTOKEN, SESSION, USER, APP, DISPLAY_APP_NAME, \
...
class AppList(async_base_endpoint.AsyncBaseRestHandler):
    ...
    def __init__(self, command_line, command_arg):
        super().__init__(command_line, command_arg, logname="rest_app_list")
    ...
    async def get(self, request):
        ...
        try:
            selected_apps = await fetch_dashboard_app_list_with_default(self.log,
                                                                        request_context=request_context,
                                                                        async_kvstore_client=async_kvstore_client,
                                                                        async_splunk_client=async_splunk_client)
            app_list = await fetch_app_names(self.log, request_context, async_splunk_client)
        except SpacebridgeApiRequestError as e:
            self.log.warn("Failed to fetch app list, error=%s", e)
    ...
    async def post(self, request):
        ...
        try:
            total_app_list = await fetch_app_names(self.log, request_context, async_splunk_client)
            ...
            dashboard_app_list = await set_dashboard_app_list(self.log,
                                                              request_context=request_context,
