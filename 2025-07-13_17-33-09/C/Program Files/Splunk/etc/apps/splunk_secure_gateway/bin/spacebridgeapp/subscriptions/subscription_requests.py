...
from spacebridgeapp.util.constants import SEARCHES_COLLECTION_NAME, \
    SUBSCRIPTIONS_COLLECTION_NAME, QUERY, NOBODY, \
    SUBSCRIPTION_CREDENTIALS_COLLECTION_NAME, FIELDS, KEY, PARENT_SEARCH_KEY, VERSION, SUBSCRIPTION_VERSION_2, \
...
async def fetch_searches(log: logging.Logger,
    ...
    log.debug("Found active searches count=%d", len(searches))

    return searches


async def _fetch_searches(log: logging.Logger,
                          auth_header, params=None, async_kvstore_client=None):
    """
...
async def count_dependant_searches(log: logging.Logger,
                                   auth_header, search_key, async_kvstore_client):
    ...
    if r.code != HTTPStatus.OK:
        error = await r.text()
        log.warning("Search dependants fetch failed search_key=%s, code=%s, error=%s", search_key, r.code, error)
        return 0
    else:
...
async def fetch_subscriptions(log: logging.Logger,
                              auth_header, subscription_id=None, search_key=None, async_kvstore_client=None):
    """
...
async def delete_subscription(log: logging.Logger,
                              auth_header, owner=None, subscription_key=None, async_kvstore_client=None):
    """
    Delete subscription
    """
    ...
    if response.code == HTTPStatus.OK:
        log.info("Subscription Deleted. subscription_key=%s", subscription_key)
        return True

    # if not OK then log the error and return false
    error = await response.text()
    log.error("Unable to delete_subscription. status_code=%s, error=%s, subscription_key=%s",
                 response.code, error, subscription_key)
    return False


async def delete_search(log: logging.Logger,
                        auth_header, search_key=None, async_kvstore_client=None):
    """
    ...
    if response.code == HTTPStatus.OK:
        log.info("Search Deleted. search_key=%s", search_key)
        return True

    # if not OK then log the error and return false
    error = await response.text()
    log.error("Unable to delete_search. status_code=%s, error=%s, search_key=%s",
              response.code, error, search_key)
    return False
