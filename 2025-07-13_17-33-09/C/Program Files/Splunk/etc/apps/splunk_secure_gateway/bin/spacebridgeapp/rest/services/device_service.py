...
from spacebridgeapp.util.constants import NOBODY, USER_KEY, KEY, DEVICE_ID, CODE, \
...
def delete_device(log: logging.Logger, user, device_owner, device_key, system_authtoken, user_authtoken):
    """
    ...
    :return:
    """
    return delete_devices(log, user, device_owner, [device_key], system_authtoken, user_authtoken)


def delete_devices(log: logging.Logger, user, device_owner, device_keys, system_authtoken, user_authtoken):
    """
    ...
    :return:
    """
    devices = get_devices_for_registered_users(log, user_authtoken)
    devices_to_delete = [device for device in devices if device[KEY] in device_keys]

    return asyncio.run(_delete_devices(log, user, devices_to_delete, system_authtoken, user_authtoken))

def delete_all_devices(log: logging.Logger, user, system_authtoken, user_authtoken):
    """
    ...
    all_devices = [device for user in mobile_users for device in get_devices_for_user(log, user, user_authtoken)]
    return asyncio.run(_delete_devices(log, user, all_devices, system_authtoken, user_authtoken))


def _get_async_factory(log: logging.Logger, system_authtoken):
    mtls_spacebridge_client = None
    config = load_config(system_authtoken)
    if config.get_mtls_enabled():
        mtls_spacebridge_client = build_mtls_spacebridge_client(log, system_authtoken)
    uri = rest.makeSplunkdUri()
    return AsyncClientFactory(log, config, uri, spacebridge_client=mtls_spacebridge_client)


async def _delete_devices(log: logging.Logger, user, devices_to_delete, system_authtoken, user_authtoken):
    async_factory = _get_async_factory(log, user_authtoken)
    ...
    for device_owner in devices_by_owner:
        for batch in batch_records(devices_by_owner.get(device_owner), BATCH_SIZE):
            deferred_responses = [asyncio.create_task(_delete_device(log,
                                                                     user, device, user_header, system_header,
        ...
        if not user_has_registered_devices(log, device_owner, system_authtoken):
            kvs_users.delete_item_by_key(device_owner)
    return result


async def _delete_device(log: logging.Logger, user, device, user_header, system_header, system_authtoken, async_factory):
    ...
    try:
        ...
        log.error("public for device not found, device_id=%s" % device[KEY])

    log.info('device_key=%s (of device_owner=%s) deleted from kvstore by user=%s'
                % (device[KEY], device[USER_KEY], user))

    await async_delete_device_from_spacebridge(log, async_spacebridge_client, device[DEVICE_ID], system_authtoken)
    log.info('device key=%s (of device_owner=%s) deleted from spacebridge by user=%s'
                % (device[KEY], device[USER_KEY], user))
