...
def _run_cmd(log: logging.Logger, cmd: OnboardCmd, secrets: OnboardingSecrets, env: Dict) -> subprocess.CompletedProcess:
    ...
    timeout = int(load_config_setting(log, constants.CONF_ASSIST, constants.STANZA_CLOUD, constants.CONFIG_PROCESS_TIMEOUT_SECONDS))
    p = subprocess.run(exec_cmd, input=input, capture_output=True, env=env, timeout=timeout)
...
def _handle_onboarding(log: logging.Logger, session_key: str, registration_code: str, deployment_id: str, license_id: Optional[str]) -> HttpResponse:
    ...
    try:
        ...
        log.warning('Onboarding command timed out')
        return HttpResponse(HTTPStatus.INTERNAL_SERVER_ERROR, HttpResult.ERROR, {}, {"message": "command timed out"})
