...
from assist.util import get_platform, is_assist_path
...
def _test_supervisor_binary(full_path):
    ...
    if not is_assist_path(full_path):
        raise RuntimeError(f'assist binary not trusted, path={full_path}')
...
def binary_path(log: logging.Logger):
    ...
    _test_supervisor_binary(base_path)
