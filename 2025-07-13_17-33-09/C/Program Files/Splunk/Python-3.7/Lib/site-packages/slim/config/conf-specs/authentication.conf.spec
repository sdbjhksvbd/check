...
* OPTIONAL
* Comma separated list of splunk roles that should be excluded
...
scimEnabled = <boolean>
* Whether or not the System for Cross-domain Identity Management (SCIM) deletes users from
  the Splunk platform instance when you delete or deactivate them from the identity provider.
* A value of "true" means that the user from the Splunk platform will be automatically deleted
  when you deactivate the corresponding user from the IdP.
* Optional.
* Default: false

scimOauthIssuer = <string>
* Specifies the URL of the issuer of Open Authorization v 2.0 (OAuth2) tokens.
* This setting is applicable only when 'scimEnabled' setting has a value of "true".
* The Splunk platform uses this setting only when the identity provider uses
  OAuth2 as an authentication mode.
* Optional.
* No default.

scimOauthClientId = <string>
* Specifies the Client ID of the issuer of OAuth2 tokens.
* This setting is applicable only when 'scimEnabled' setting has a value of "true".
* The Splunk platform uses this setting only when the identity provider uses
  OAuth2 as an authentication mode.
* Optional.
