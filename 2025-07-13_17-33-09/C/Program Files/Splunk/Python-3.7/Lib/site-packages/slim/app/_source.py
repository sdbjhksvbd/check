...
from slim.utils import slim_configuration
from .. utils import *
...
class _AppSourceFactory(ABCMeta):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        if path.isdir(package_path):
            ...
            # Per SPL-119841, we are retaining the slim cache when slim_configuration.slim_cache_overwrite is false,
            # otherwise overwrite the slim cache.
            # This is to prevent the slim cache from being exploited by apps reusing the same
            # appid and app version...
            if not slim_configuration.slim_cache_overwrite:
                try:
                    app_source = slim_configuration.cache.get_sources[package]
                    app_source.package = package_path
                except KeyError:
                    app_source = super(_AppSourceFactory, cls).__call__(*args, **kwargs)
                    if slim_configuration.payload._messages and slim_configuration.payload._messages.get('ERROR'):
                        SlimLogger.error('Error in package: ', app_source.package)
                        return app_source
                    slim_configuration.cache.add_source(package, app_source)
                    SlimLogger.information('Added package {} to slim cache {} '.format(
                        app_source.package, slim_configuration.cache.cache_path))
            else:
                app_source = super(_AppSourceFactory, cls).__call__(*args, **kwargs)
                if slim_configuration.payload._messages and slim_configuration.payload._messages.get('ERROR'):
                    SlimLogger.error('Error in package: ', app_source.package)
...
class AppSource(with_metaclass(_AppSourceFactory, ObjectView)):
    ...
    def __init__(self, package, local_conf=None):
        ...
        if self.manifest is None:
            SlimLogger.error('Problem detected with package, can\'t get app manifest')
            return
