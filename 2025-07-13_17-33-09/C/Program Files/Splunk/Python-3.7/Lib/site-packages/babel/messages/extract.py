...
def extract_javascript(fileobj, keywords, comment_tags, options):
    ...
    template_type = options.get('template_type', 'none')
    last_token = None
    call_stack = -1
    ...
    for token in tokenize(
        ...
        dotted=dotted,
        matchRegExp=(template_type!='underscore')
    ):
        tokens.append(token)
    ...
    for token in tokens:
        ...
        elif call_stack == -1 and token.type in ('string', 'template_string'):
            new_value = unquote_string(token.value)
            if new_value:
                for new_token in tokenize(
                    new_value,
                    jsx=options.get("jsx", True),
                    template_string=options.get("template_string", True),
                    dotted=dotted,
