...
from splunk.splunk_internal_telemetry import post_telemetry_metric
...
SEARCH_LOG_RX = re.compile('^search\.log(\.\d+)?$')
...
class JobsController(BaseController):
    ...
    def streamJobExport(self, job, assetType, sessionKey=None, **kwargs):
        """
        ...
        # SPL-202832 export of search results to json or csv fails. Output file contains "414 Request-URI Too Long"
        ...
        if 'output_time_format' in kwargs:
            ...
            tzOffset = job.getTimeZoneOffset()
            request['output_time_format'] = i18n.get_ISO8609_MICROTIME(tzOffset)
        ...
        is_v2_enabled = is_v2_search_enabled(sessionKey)
        ...
        if job.reportSearch is None and ((job.eventIsTruncated and (count == 0 or count > job.eventAvailableCount)) or not job.isDone):
            ...
            endPoint = 'search/v2/jobs/export' if is_v2_enabled else 'search/jobs/export'
            logger.debug('Search V2 - endpoint = %s' %(endPoint))
            uri = en.buildEndpoint(endPoint, namespace=ns, owner=owner)
            ...
            uri = en.buildEndpoint('search/v2/jobs/%s/events/export' % job.sid, namespace=ns, owner=owner) if is_v2_enabled else en.buildEndpoint('search/jobs/%s/events/export' % job.sid, namespace=ns, owner=owner)
            logger.debug('Search V2 - endpoint in event assetType is = %s' %(uri))
            getargs = request
        else:
            logger.debug('No re-run reporting search')
            uri = en.buildEndpoint('search/v2/jobs/%s/results/export' % job.sid, namespace=ns, owner=owner) if is_v2_enabled else en.buildEndpoint('search/jobs/%s/results/export' % job.sid, namespace=ns, owner=owner)
            logger.debug('Search V2 - uri is = %s' %(uri))
            getargs = request
    ...
    def listJobs(self, restrictToSession=True, nocache=False, s=None, cachebuster=None, wait=True):
        ...
        an allow list provided by the client
        '''
    ...
    def sanitize_filename(self, filename, sid, extension):
        '''
        Returns a sanitized file name.

        SPL-185188 Force extension to be included in downloaded filename
        ...
        Returns:
        string
            Sanitized value of the filename
        '''
    ...
    def getJobAsset(self, sid, asset, compat_mode=True, **kwargs):
        ...
        self.sessionKey = cherrypy.session['sessionKey']

        job_lite = splunk.search.JobLite(sid, self.sessionKey)
        ...
        # content-type response header should be MIME_TEXT for search.log
        if SEARCH_LOG_RX.search(asset):
        ...
        if 'isDownload' in kwargs:
            ...
            clean_filename = self.sanitize_filename(filename, sid, extension)
            ...
            job = splunk.search.getJob(sid, sessionKey=self.sessionKey)
            # SPL-202832 export of search results to json or csv fails. Output file contains "414 Request-URI Too Long"
            url_len = len(urllib_parse.urlencode(kwargs))
            if url_len > 460800: # 450 * 1024 to leave some headroom as MAX_BYTES == 500k
                logger.warn('field_list argument larger than 450KB, removed in REST call to shorten URL')
                kwargs.pop('field_list', None)
                kwargs.pop('f', None)
            return self.streamJobExport(job, asset, sessionKey=self.sessionKey, **kwargs)
