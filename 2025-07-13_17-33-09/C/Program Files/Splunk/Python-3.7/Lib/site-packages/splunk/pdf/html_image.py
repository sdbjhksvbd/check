...
class HtmlImage(Flowable, HtmlMaxHeightMixIn):
    ...
    def __init__(self, imageFileName, width=None, height=None, hAlign='CENTER', mask='auto', mimetype=None, requestSettings=None, sessionKey=None, **kw):
        self.kw = kw
        ...
        self.mimetype = str(mimetypes.guess_type(imageFileName)[0])
    ...
    def getImage(self):
        if 'image/png' in self.mimetype:
            image = imageUtils.PngImageReader(self.fileName, requestSettings=self._requestSettings, sessionKey=self.sessionKey)
        elif 'image/jpeg' in self.mimetype:
            image = imageUtils.JpgImageReader(self.fileName, requestSettings=self._requestSettings, sessionKey=self.sessionKey)
        else:
            raise imageUtils.IllegalFormat(self.fileName, 'image')
        return image
