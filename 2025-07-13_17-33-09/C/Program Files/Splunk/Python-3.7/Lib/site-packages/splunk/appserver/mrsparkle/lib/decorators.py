...
"""
Sanitizes a field input to prevent CRLF Injection.

@type  field: dict
@param field: A dictionary of html attributes and corresponding values.

@rtype: dict
@return: A sanitized dictionary of html attributes and corresponding values.
"""
def _sanitize_field(field):
    for key in field:
        encoded_field = urllib_parse.quote(field[key])
        # spaces should be allowed
        field[key] = encoded_field.replace('%20', ' ')
    return field
...
def format_list_template():
    ...
    def decode_fields(fields):
        ...
        for field in fields:
            try:
                sanitized_field = _sanitize_field(json.loads(field))
                decoded_fields.append(sanitized_field)
