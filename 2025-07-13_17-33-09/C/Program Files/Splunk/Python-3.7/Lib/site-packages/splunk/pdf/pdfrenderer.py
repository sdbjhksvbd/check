...
class PDFRenderer(object):
    ...
    def __init__(self, namespace, title, description, outputFile, paperSize, timestamp="", includeSplunkLogo=None,
                 cidFontList=None, pdfSettings=None, requestSettings=None, sessionKey=None):
    ...
    def renderImage(self, imagePath, width=None, height=None, hAlign='CENTER'):
        ...
        if imagePath:
            ...
            except (imageUtils.UntrustedImageHost, imageUtils.RedirectIsNotAllowed, imageUtils.InsecureRemotePath) as e:
                self._story.append(Paragraph(self._fontManager.encodeTextForParagraph(su.escape(str(e)))))
    ...
    def renderSvgString(self, svgString, title = None):
        svgImageFlowable = pdfgen_svg.getSvgImageFromString(svgString, self._fontManager, sessionKey=self.sessionKey)
    ...
    def save(self):
        ...
        doc.build(self._story, onFirstPage=_firstFooterAndHeader, onLaterPages=_footerAndHeader)
...
def _firstFooterAndHeader(canvas, doc):
    canvas.setTitle(doc._title) # sets Title metadata in pdf
    _footerAndHeader(canvas, doc)
