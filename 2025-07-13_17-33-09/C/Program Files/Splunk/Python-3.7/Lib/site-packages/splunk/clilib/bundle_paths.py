...
def get_slaveapps_base_path():
    return join(etc(), comm.getAppDir())
...
def get_app_name_from_tarball(path, raise_error_if_multiple_apps = False):
    ...
    with closing(tarfile.open(path)) as tar:
        for i in tar.getmembers():
            ...
            # Added support for SSAI app installs when tarinfo object starts with './'
            # as Noah, AppInspect, Splunk Enterprise and puppet installs support this.
            # If the app package starts with ./, ignore the prefix.
            ignore_dot_slash_prefix = './'
            if i.name.startswith(ignore_dot_slash_prefix):
                i.name = i.name[len(ignore_dot_slash_prefix):]
            logger.debug("Examining file to install after removing ./ prefix: %s" % i.name)
...
class Bundle(object):
    ...
    @staticmethod
    def _merger(src, dst, dryRun, extension = ".conf"):
        try:
            if dryRun:
