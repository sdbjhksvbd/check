...
from splunk.appserver.mrsparkle.lib.util import redactSensitiveData
...
class AppInstallController(BaseController, Capabilities):
    ...
    def getLocalApp(self, appid, app_name=None, flush_cache=True):
        ...
        """
        logger.info(f"Fetching locally installed app with appid: {appid}, app_name: {app_name}")
        ...
        for id, app in local_apps.items(): # local_apps is not an instance of dictionary
            if self.getSbAppId(app).lower() == appid.lower() or app.get('label', appid) == app_name:
                return app
    ...
    def processAppUpload(self, f, force, **kwargs):
        """
        ...
        if current_app:
            ...
            if self.appUpgradeAvailable(current_app):
                ...
                return self.render_admin_template('/admin/appinstall/upgrade-available.html', {
                    'app': current_app,
                    'appid': appid,
                    'appname': current_app.get('label', app_name),
                    'breadcrumbs': breadcrumbs,
                    'error': error,
                    'state': state,
    ...
    def upload(self, appid, return_to=None, breadcrumbs=None, state=None, appfile=None, force=None, **kw):
        ...
        if isCloud():
            ...
            if appfile is not None and cherrypy.request.method == 'POST':
                try:
                    force = (force == '1')
                    sbAppId = self.processAppUpload(appfile, force, **kw)
    ...
    def checkstatus(self, appid, state=None, app_name=None, return_to=None, checkstatus=None, **kw):
        ...
        """
        logger.info(f"In checkstatus method with state: {json.dumps(state)}")
        if state:
            if not isinstance(state, StateDict):
                state = StateDict.unserialize(state)
        else:
        ...
        logger.info(f"App found while in checkstatus method: {app}")
        if not app:
    ...
    @route('/:appid=package_upload', methods=['POST'])
    @expose_page(must_login=True)
    def package_upload(self, **args):

        logger.info(f"In package_upload method with args: {redactSensitiveData(args, {'auth_token'})}")

        upload_app_file = args.get('upload-app-file', None)
        if not upload_app_file:
            cherrypy.response.status = 400
            return {"message": "missing parameter: upload-app-file",
                    "code": "BAD_REQUEST"}

        with tempfile.NamedTemporaryFile(mode="wb", delete=True) as app_temp_file:
            shutil.copyfileobj(upload_app_file.file, app_temp_file)
            app_temp_file.flush()

            if not tarfile.is_tarfile(app_temp_file.name):
                cherrypy.response.status = 400
                return {"message": "Uploaded file is not an acceptable tar file",
                        "code": "BAD_REQUEST"}

            postargs = {
                "app_path": app_temp_file.name,
                "_action": "upload",
                "auth_token": args.get('auth_token')
            }

            response, content = rest.simpleRequest('/uploaded-apps/package',
                                                   postargs=postargs,
