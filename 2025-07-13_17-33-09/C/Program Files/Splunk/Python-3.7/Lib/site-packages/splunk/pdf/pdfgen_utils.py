...
def dispatchSavedSearch(searchName, savedSearchModel=None, namespace=None, owner=None, sessionKey=None, hostPath=None,
                        forceHistoricSearch=False, overrideNowTime=None, earliestTime=None, latestTime=None, checkRiskyCommand=True):
    """
    ...
    args['dispatch.check_risky_command'] = checkRiskyCommand
...
def isRiskyCommandCheckDashboardEnabled(sessionKey=None):
    """
    read the enable_risky_command_check_dashboard flag from web.conf
    :param sessionKey:
    :return Boolean:
    """
    if cherrypy is not None and cherrypy.config is not None and 'enable_risky_command_check_dashboard' in cherrypy.config:
        enableRiskyCommandCheckDashboard = cherrypy.config.get('enable_risky_command_check_dashboard')
    else:
        try:
            settings = entity.getEntity(WEB_CONF_ENTITY, 'settings', sessionKey=sessionKey)
            enableRiskyCommandCheckDashboard = settings.get('enable_risky_command_check_dashboard', 1)
        except Exception:
            # failed to retrieve web.conf, we assume risky command check is enabled.
            enableRiskyCommandCheckDashboard = 1
    return enableRiskyCommandCheckDashboard
