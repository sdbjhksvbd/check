...
class PersistentServerConnectionApplicationServer(PersistentServerConnectionHandlingLoop):
    ...
    def _load_file(filename):
        ...
        modname = os.path.splitext(fileonly)[0]
        munged_name = PersistentServerConnectionApplicationServer._munge_module_name(modname, dirname)
        if sys.version_info >= (3, 1):
            # Python 3.1 and higher have the "importlib" module.
            try:
                # Import the module directly from the file.
                # See https://docs.python.org/3/library/importlib.html#importing-a-source-file-directly
                # for this recipe.
                spec = importlib.util.spec_from_file_location(munged_name, filename)
                module = importlib.util.module_from_spec(spec)
                sys.modules[munged_name] = module
                spec.loader.exec_module(module)
            except ImportError:
                return None
        else:
            # Python 3.0 and lower must use the deprecated "imp" module.
            try:
                (filehandle, filename, data) = imp.find_module(modname, [dirname])
            except ImportError:
                return None

            try:
                module = imp.load_module(munged_name, filehandle, filename, data)
            finally:
                if filehandle is not None:
                    filehandle.close()

        handler_class = PersistentServerConnectionApplicationServer._find_handler_in_module(module)
