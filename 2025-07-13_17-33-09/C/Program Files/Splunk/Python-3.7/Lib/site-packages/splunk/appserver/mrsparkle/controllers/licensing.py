...
class LicensingController(BaseController, Capabilities):
    ...
    def show_summary(self, **unused):
        ...
        for slave in slaves:
            ...
            if slave.name == self_config.slave_name:
                local_slave = slave
        if not local_slave:
            raise Exception('Could not retrieve peer information for peer name: %s' % self_config.slave_name)
        ...
        for stack in stacks:
            ...
            stack_table.append({
                ...
                'cle_active': stack.cle_active,
                'window_period': stack.window_period,
                'max_violations': stack.max_violations
    ...
    def edit_pool(self, 
        ...
        **unused
        ...
        if not len(pool_object.peers) and can_be_catch_all:
            slave_mode = 'catchall'
        else:
            slave_mode = 'explicit'
        ...
        assigned_slave_list = []
        for slave in pool_object.peers:
            # SPL-198154 - Avoid empty/none type strings
            label = slave_label_map.get(slave, slave)
            if label:
        ...
        # SPL-198154 handle none type strings
        assigned_slave_list.sort(key=lambda x:x[1] or "")
        ...
        if cherrypy.request.method == 'POST':
            ...
            if formset_slave_mode == 'catchall':
                pool_object.peers = CATCHALL_SLAVE_LIST
            
            else:
                if not formset_slaves:
                    pool_object.peers = []
                elif isinstance(formset_slaves, list):
                    pool_object.peers = formset_slaves
                else:
                    pool_object.peers = [formset_slaves]
        ...
        # doesn't show the *
        if pool_object.peers == CATCHALL_SLAVE_LIST:
            assigned_slave_list = []
    ...
    def edit_self(self, formset_master_server_uri=None, formset_master_server_mode=None, return_to=None, **unused):
        ...
        if self_config.master_uri == OWN_MASTER_URI:
            ...
            is_own_master = False
            master_server_uri = self_config.manager_uri or self_config.master_uri
        ...
        if cherrypy.request.method == 'POST':
            ...
            if formset_master_server_mode == 'this_server' or not formset_master_server_uri:
                self_config.manager_uri = OWN_MASTER_URI
            else:
                self_config.manager_uri = formset_master_server_uri
    ...
    def switch(self, license_group=None, return_to=None, **unused):
        ...
        # No endpoint is being accessed that will raise the AuthorizationFailed
