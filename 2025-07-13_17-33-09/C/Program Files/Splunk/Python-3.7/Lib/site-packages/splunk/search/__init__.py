...
#
# vide "enum class OrchestratorState"
DISPATCH_STATE_MAP = {
    'QUEUED': 1,
    'PARSING': 2,
    'RUNNING': 3,
...
def dispatch(search, hostPath=None, sessionKey=None, namespace=None, owner=None, waitForRunning=True, **kwargs):
    ...
    serverResponse, serverContent = rest.simpleRequest(
        uri, postargs=args, sessionKey=sessionKey, rawResult=True)
...
def listJobs(hostPath=None, sessionKey=None, normalizeValues=False, ignoreExpired=False, namespace=None, owner=None, **kw):
    ...
    '''
    is_v2_enabled = is_v2_search_enabled(session_key=sessionKey)
    uri = entity.buildEndpoint(JOBS_V2_ENDPOINT_ENTITY_PATH, '', namespace, owner) if is_v2_enabled else entity.buildEndpoint(JOBS_V1_ENDPOINT_ENTITY_PATH, '', namespace, owner)
    logger.debug('Search V2 - uri in listJobs is = %s' %(uri))

    if hostPath:
        uri = splunk.mergeHostPath(hostPath) + uri
    serverResponse, serverContent = rest.simpleRequest(uri, sessionKey, postargs=kw) if is_v2_enabled else rest.simpleRequest(uri, sessionKey, getargs=kw)
...
class SearchJob(object):
    ...
    def _getStatus(self, isInitial=False, force=False):
        ...
        is_v2_enabled = is_v2_search_enabled(self.sessionKey)
        uri = entity.buildEndpoint(JOBS_V2_ENDPOINT_ENTITY_PATH, self.id) if is_v2_enabled else entity.buildEndpoint(JOBS_V1_ENDPOINT_ENTITY_PATH, self.id)
        logger.debug('Search V2 - uri in search/init _getStatus is = %s' %(uri))
        ...
        while firstTime or ((time.time() - loopStartTime) < self._status_fetch_timeout):
            serverResponse, serverContent = rest.simpleRequest(uri, postargs=args, sessionKey=self.sessionKey, raiseAllErrors=True) if is_v2_enabled else rest.simpleRequest(uri, getargs=args, sessionKey=self.sessionKey, raiseAllErrors=True)
            firstTime = False
    ...
    def _getResultRange(self, mode='events', http_method='GET', **kwargs):
        ...
        http_method = 'POST' if 'search/v2/' in uri else http_method
        if http_method == 'POST':
        ...
        if serverResponse.status not in [200, 204]:
            ...
            try:
                ...
                # Raise generic exception later
                pass
            if rootXml is not None:
                extractedMessages = rest.extractMessages(rootXml)
                if extractedMessages is not None and len(extractedMessages) > 0:
                    raise splunk.SearchException(extractedMessages[0]['text'])
    ...
    def getTimeZoneOffset(self):
        if self.createTime is None:
            ...
            #why -6? e.g. createTime=2022-10-14T12:14:46.000-07:00,
            #the last 6 characters "-07:00" is timezone offset
            tzOffset = self.createTime[-6:]
        return tzOffset
...
class JobLite(object):
    def __init__(self, sid, sessionKey=None):
        self.sid = sid
        self.fetchOptions = {}
        self.sessionKey = sessionKey
    ...
    def get(self, asset):
        ...
        # Not sure if this change is needed because splunk lite code is no longer
        uri = '/services/search/v2/jobs/%s/%s' % (sid, asset) if is_v2_enabled else '/services/search/jobs/%s/%s' % (sid, asset)
        logger.debug('Search V2 - uri in search/init get method is = %s' %(uri))

        serverResponse, serverContent = splunk.rest.simpleRequest(
            uri, postargs=self.fetchOptions) if is_v2_enabled else splunk.rest.simpleRequest(
            uri, getargs=self.fetchOptions)

        if serverResponse.status == 404:
            raise splunk.ResourceNotFound(
                'Splunkd reported that the "%s" endpoint does not exist for sid=%s.  Either the job expired, or dispatch system is broken' % (uri, self.sid))
        if serverResponse.status not in [200, 204]:
            raise Exception('Server reported HTTP status=%s while getting uri=%s\n%s' % (
                serverResponse.status, uri, serverContent))
    ...
    def getResults(self, asset, offset=None, count=None):
        ...
        is_v2_enabled = is_v2_search_enabled(self.sessionKey)
        uri = '/services/search/v2/jobs/%s/%s' % (sid, asset) if is_v2_enabled else '/services/search/jobs/%s/%s' % (sid, asset)
        logger.debug('Search V2 - uri in search/init getResults method is = %s' %(uri))

        args = copy.copy(self.fetchOptions)

        args['output_mode'] = "xml"
        ...
        serverResponse, serverContent = splunk.rest.simpleRequest(
            uri, postargs=args) if is_v2_enabled else splunk.rest.simpleRequest(
            uri, getargs=args)
...
# All unit tests for this file have been moved to tests/search/test_search_init.py