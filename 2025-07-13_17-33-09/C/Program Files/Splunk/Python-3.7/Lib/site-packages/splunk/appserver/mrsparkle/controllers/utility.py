...
class UtilityController(BaseController):
    ...
    def log_js(self, log, js, data='[]', **args):
        ...
        successCount = 0
        for item in logs:
            secondary_logger = logger
            event = []
            event.append('name=javascript')
            event.append('class=%s' % item.get('class', 'undefined'))
            item_msg = item.get('message', '')
            # splunk.appserver.controllers.util is the default logger. If a type is specified, use that logger instead
            if item.get('type') is not None:
                secondary_logger = logging.getLogger(item.get('type'))
                # only format JSON data if using secondary logger ... we don't want to risk breaking anything with the regular logger.
                try:
                    item_msg = json.dumps(item_msg)
                except Exception as e:
                    pass
            event.append("%s" % item_msg)
            eventFormat = ", ".join(event)
            try:
                logLevel = item.get('level', 'info')
                if logLevel == 'log':
                    logLevel = 'info'
                getattr(secondary_logger, logLevel)(eventFormat)
