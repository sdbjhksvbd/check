...
import os
import sys
from splunk.acuif import get_acuif_pages
from splunk.appserver.mrsparkle.controllers.appinstall import AppInstallController, APP_INSTALL_TIMEOUT
...
from splunk.appserver.mrsparkle.lib.util import isFullUiSettingsMode
...
from splunk.rest.splunk_web_requests import get_manager_xml_pages_settings
...
def _unpack(root_str, search=None):
    ...
    root = et.fromstring(root_str, parser=parser)
...
def _unpackManagerXmlElementFromStr(xml_str):
    ...
    """
    parser = et.XMLParser(remove_blank_text=True, remove_comments=True)
    element = et.fromstring(xml_str, parser=parser)
...
def _sanitizeEvalInputs(evalStr):
    """
    SPL-235737: Sanitize eval str to prevent RCE vulnerability using python dunder methods
    """
    disallowList = ['__mro__']
    c = compile(evalStr.strip(), '<string>', 'eval')
    for name in c.co_names:
        if name in disallowList:
            raise NameError("Use of {} is not allowed".format(name))
    return c
...
class AdminController(BaseController, Capabilities):
    ...
    def generateUIHelperFromEntity(self, entity, namespace='search'):
        ...
        for block in block_nodes:
            ...
            fieldsetElement = {
                ...
                "showInView"    : [ "edit" ],
                "type"          : "fieldset",
                "legend"        : block.get("title", "Unable to render setup. Most likely, the cause is that the setup.xml file for this app is not configured correctly. For example, it may not specify task and type attributes. Contact the application developer to resolve this issue."),
                "elements"      : []
            }
    ...
    def saveUploadLookupTableFile(self, args, namespace, entity_name=''):
        """
        Write a lookup file into $SPLUNK_HOME/var/run/splunk/lookup_tmp and
        forward that file on to splunkd. If we're overwriting an existing file,
        get the name from that
        """
        if not entity_name or entity_name == en.NEW_EAI_ENTITY_NAME:
        ...
        if not re.match(r'.+\.(csv|csv\.gz|kmz|kml|mmdb|mmdb\.gz)$', filename):
            return False, _('Filename must end with .csv, .csv.gz, .kmz, .kml, .mmdb, or .mmdb.gz')
        ...
        """
        isBinary = False
        if filename.endswith(".kmz") or filename.endswith(".gz") or filename.endswith(".mmdb"):
            isBinary = True
        # Unique-ify filename.
    ...
    def saveEntity(self, endpoint_path, entity_name, args, namespace, entity_owner, entity_uri):
        ...
        if endpoint_path=='apps/local':
            ...
            setEntityTimeout = APP_INSTALL_TIMEOUT
        if endpoint_path=='data/lookup-table-files':
            uploaded, upload_err = self.saveUploadLookupTableFile(args, namespace, entity_name)
            if not uploaded:
        ...
        for arg in args:
            ...
            # for backward compatibility with cherrypy 3 we mimic the multi-part upload. This was never documented
            if (isinstance(args[arg], cherrypy._cpreqbody.Part) and args[arg].file):
                file_buffer = args[arg].file.read()
                args[arg] = "FieldStorage('pcap_file', '" + args[arg].filename + "', '" + str(file_buffer)[2:-1]+ "')"


            # if ( (not arg.startswith('spl-ctrl_')) and (arg!='splunk_session_id') and (arg!=None) and (arg!='confirm_password') and (args[arg] != "") ):
            if ((not arg.startswith('spl-ctrl_')) and (arg != 'splunk_form_key') and (arg != None) and (
        ...
        try:
            ...
            bolSaved = en.setEntity(updatedEnt, uri=_extractPathFromURI(entity_uri), msgObj=msgObj, timeout=setEntityTimeout)
    ...
    def getSingleEntity(self, endpoint_path, uri=None, namespace=splunk.getDefault('namespace'), viewFilter="edit"):
        ...
        manager_pages_config = cli_common.getConfStanza("server", "manager_pages")
        sanitize_uri_param = splunk.util.normalizeBoolean(manager_pages_config.get("sanitize_uri_param", True))

        if sanitize_uri_param:
            logger.info("uri before sanitizing:: %s." % uri)
            if (uri and endpoint_path.startswith('data/inputs')):
                # Discard query string inside the uri as it's not used.
                uri = uri.split('?')[0]

                # validate uri - Only allow uri that match splunkd endpoint format
                search_with_ns = re.match(rf"^\/servicesNS\/[^.<>\\%<>\"']*?\/[^.<>\\%<>\"']*?\/{re.escape(endpoint_base)}\/.*$", uri)
                if not search_with_ns:
                    search_without_ns = re.match(rf"^\/services\/{re.escape(endpoint_base)}\/.*$", uri)
                    if not search_without_ns:
                        uri = ''
            logger.info("updated uri:: %s." % uri)
                        ...
                        if self.getElementPermissions(endpoint_path, entity_name, element['elementName'], entity) == NOACCESS:
                            ...
                            if processValue in element:
                                try:
                                    ...
                                    sanitizedEvalstr = _sanitizeEvalInputs(element[processValue])
                                    value = eval(sanitizedEvalstr, safe_globals, safe_locals)
    ...
    def render_admin_template(self, template_name, template_args=None, hide_back=False):
        ...
        use_app_bar_performance_optimizations = util.should_use_app_bar_performance_optimizations(namespace)
        # set build_nav=False below as manager doesn't display any modules requiring nav/saved searches
        args =  root.app.buildViewTemplate(namespace, ADMIN_VIEW_NAME, render_invisible=True, build_nav=False, include_app_css_assets=False, use_app_bar_performance_optimizations=use_app_bar_performance_optimizations)
    ...
    def listEntities(self, namespace, endpoint_path, **kwargs):
        ...
        # Intercept admin config ui pages and set endpoint path = config, so that downstream
        # config page component can parse url
        if endpoint_path.startswith('manage_system_config'):
            ep_arr = endpoint_path.split('/')
            endpoint_path = ep_arr[0]
        ...
        if action == "edit":
            ...
            try:
                ...
                args.update( dict([(k[4:], _extractPathFromURI(kwargs[k])) for k in kwargs if k.lower().startswith('api.') and k[4:] not in args]) )
    ...
    def fetch_element(self, namespace, endpoint_base, element, element_name, form_defaults=None, element_overlay=None, entity_name=None, eai_attributes=None):
        # SPL-229456 The endpoint has been deprecated as it was insecure.
        logger.error("attempt to access the deprecated endpoint - fetch_element namespace={} endpoint_base={} element={} element_name={}".format(namespace, endpoint_base, element, element_name))
        raise cherrypy.HTTPError(404)
    ...
    def permissions(self, namespace, action, endpoint_path, sharing='user', owner=None, perms_read=None, perms_write=None, **kwargs):
        ...
        if entity['eai:acl'].get('sharing') == 'app' or entity['eai:acl'].get('can_share_app', None) == '1':
            if not util.isLite():
                app_label = _('This app only (%s)') % (entity['label'] if 'label' in entity and app == 'system' else app)
    ...
    def _permissionsGetRoles(self, perms):
        ...
        return sorted(set(filter(None, unique_rolls)))
    ...
    def renderSHCErrorifEnabled(self, page=None):
        ...
        if isFree:
            return None
        #SPL-202826 for stacks with full ui settings mode, don't render a wall page.
        if isFullUiSettingsMode():
            return None
        ...
        if self.isSHCMode():
            ...
            return self.render_admin_template('admin/notavailable.html', {
                'sectionTitle'        : _('Action is not available'),
    ...
    def moveObject(self, namespace, endpoint_base, action, **kwargs):
        ...
        try:
            # See SPL-176513 for details.
            enable_secure_entity_move = normalizeBoolean(cherrypy.config.get('enable_secure_entity_move'))
            if cherrypy.request.method == 'GET' and enable_secure_entity_move:
                msg = ("error", "GET for moving '%s' is not allowed." % (entname))
            else:
            ...
            for key, value in list(form_data.items()): #  We wrapped the call to items() in a list() for py3's dictionary changed size during iteration.
                ...
                if key in uiHelper_elements:
                    ...
                    if p:
                        try:
                            safe_globals = {'__builtins__': {}}
                            safe_locals = {'key': key, 'value': value, 'form_data': form_data, 'len': len}
                            c = _sanitizeEvalInputs(p)
                            nv = eval(c, safe_globals, safe_locals)
                            if nv is None:
    ...
    def datainputstats(self, namespace, action, operation=None, **kwargs):
        ...
        for (name, endPoint) in endPointList.items():
            ...
            # If entities can be fetched, it cannot be None (doesn't enter exception block) and assume user has permission.
        ...
        try:
            # set count=0 so all mod input results are returned
            modInputTypes = en.getEntities('data/modular-inputs', search=search, namespace=namespace, count=count, offset=offset)
            totalResults = modInputTypes.totalResults

        except splunk.AuthorizationFailed as e:
            logger.warning("Splunk.AuthorizationFailed: %s", str(e))

        # filter out inputs that will not be shown
    ...
    def systemsettings(self, namespace, action, operation=None, **kwargs):
        ...
        showGlobalBanner = self.can_access_manager_xml('global_banner')
        showInternalLibrarySettings = self.can_access_manager_xml('internal_library_settings')
        ...
        # Load Admin Config UI Framework Links if instance type is cloud
        if util.isCloud():
            acuif_pages = get_acuif_pages()

        if not (showGeneralSettings or
                showGlobalBanner or
                showInternalLibrarySettings or
                showEmail or
                showLogger or
        ...
        return self.render_admin_template('admin/systemsettings.html', {
            ...
            'showInternalLibrarySettings' : showInternalLibrarySettings,
            'showEmail'                   : showEmail,
            'showLogger'                  : showLogger,
            'showDeploymentClient'        : showDeploymentClient,
            'showSearchPrefs'             : showSearchPrefs,
            'acuif_pages'                 : acuif_pages
        })
    ...
    def uploadbgimg(self, **kargs):
        ...
        if image is not None :
            ...
            content = image.file.read(32)
            image.file.seek(0)
            if not (content.startswith(b'\xFF\xD8\xFF') or content.startswith(b'\x89\x50\x4E\x47\x0D\x0A\x1A\x0A')):
    ...
    def controlpage(self, namespace, action, operation=None, **kwargs):
        ...
        return self.render_admin_template('admin/control.html', {
            ...
            'displaySHCRollingRestartButton' : util.isCloud() and self.isSHCMode(),
            'msgid'                     : msgid,
...
# Unit tests
import unittest
class MainAdminTest(unittest.TestCase):
