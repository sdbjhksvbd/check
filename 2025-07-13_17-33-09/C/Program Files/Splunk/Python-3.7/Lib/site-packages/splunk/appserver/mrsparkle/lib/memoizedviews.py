...
class MemoizedViews(object):
    ...
    def getAvailableViews(self, namespace, refresh, output, flash_ok=True, quarantine_files_settings=None):
        time1 = time.time()
        ...
        for item, digest in digests.items():
            parsed_view = self.getParsedView(
                namespace,
                rawxml,
                item,
                digest.get('eai:digest'),
                flash_ok=flash_ok,
    ...
    def getParsedView(self, namespace, rawxml, viewid, viewdigest, flash_ok=True, quarantine_files_settings=None):
        ...
        v1_p0_dashboard_exists = util.v1p0_dashboard_exists()
        viewdigest = '%s_v1_p0_dashboard_exists=%s' % (viewdigest, v1_p0_dashboard_exists)
        ...
        if quarantine_files_settings is not None:
            ...
            viewdigest = '%s_enable_unsupported_hotlinked_imports=%s' % (viewdigest, enable_unsupported_hotlinked_imports)
            viewdigest = '%s_enable_jQuery2=%s' % (viewdigest, enable_jquery2)
        ...
        if len(rawxml) == 0:
            ...
            rawxml.update(en.getEntities(VIEW_ENTITY_CLASS,
                                         ...
                                         count=-1))
            time2 = time.time()
            logger.info('PERF - getParsedView getEntities=%ss' % (round(time2-time1, 4)))
        ...
        try:
            ...
            try:
                if viewobj.get('eai:type') == "views":
                    ...
                    html_template = 'view/dashboard_escaped_render.html' if v1_p0_dashboard_exists is True else 'view/html_dashboards_removed.html'
        ...
        time4 = time.time()
        logger.info('PERF - parseViewAndCache=%ss' % (round(time4-time3, 4)))
    ...
    def _generateViewRoster(self, viewName, viewConfig, urlPath):
        ...
        output = {
            ...
            'is_dashboard_redirected': viewConfig.get('is_dashboard_redirected', False)
        }
