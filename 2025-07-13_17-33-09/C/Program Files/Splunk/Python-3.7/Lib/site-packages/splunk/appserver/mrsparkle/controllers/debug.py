...
class DebugController(BaseController):
    ...
    def reset(self, **kwargs):
        ...
        if has_perms and cherrypy.request.method=='POST':
            jobs = se.listJobs(sessionKey=cherrypy.session['sessionKey'])
            for job in jobs:
                try:
                    j = se.getJob(job['sid'])
                    j.cancel()
    ...
    def refresh(self, entity=None, **kwargs):
        ...
        endpointExclusionList = ['admin/remote_indexes', 'admin/fshpasswords', 'admin/riq-config', 'admin/roq-config']
