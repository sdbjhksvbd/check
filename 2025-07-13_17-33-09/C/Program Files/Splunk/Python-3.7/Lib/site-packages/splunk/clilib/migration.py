...
PATH_SPLUNK_LAUNCH_CONF = make_splunkhome_path(["etc", "splunk-launch.conf"])
PATH_SPLUNK_LAUNCH_CONF_BAK = make_splunkhome_path(["etc", "splunk-launch.conf.bak"])
...
def find_webhooks(path):
  """
  path: path to $SPLUNK_HOME
  retval: dictionary of allowlist entries
  """
  allowlist_dict = {}
  filenames = []
  count = 1
  ...
  for candidate in ["savedsearches.conf", "alert_actions.conf"]:
    filenames.extend(glob.glob(os.path.join(path, "etc", "apps", "*", "local", candidate)))
    filenames.extend(glob.glob(os.path.join(path, "etc", "system", "local", candidate)))
    # etc/users/<username>/<app>/local
    filenames.extend(glob.glob(os.path.join(path, "etc", "users", "*", "*", "local", candidate)))
  
  for filename in filenames:
    config = comm.readConfFile(filename)
    for _, stanza  in config.items():
      url = stanza.get("action.webhook.param.url", stanza.get("param.webhook.url", ""))
      url_parts = None
      if url:
        try:
          ...
          if not url_parts.scheme or not url_parts.netloc:
            msg = "Malformed URL: {} with path: {}. Please take a look and change it to fit specifications".format(url, filename) 
            logger.warning(msg)
            continue
        except Exception as e:
          logger.error("find_webhooks: The following error \"{}\" came from URL \"{}\" at {}".format(e, url, filename))
          continue
      else:
        continue
      entry = "^{}:\/\/{}".format(url_parts.scheme, url_parts.netloc)
      split_entry = entry.split('.')
      entry = "\.".join(split_entry) + ".*"
      if entry not in seen_urls:
        seen_urls.add(entry)
        allowlist_dict["allowlist.entry{}".format(count)] = entry
        count += 1

  if allowlist_dict == {}:
      logger.info("No webhooks found in {}, nothing to migrate".format(path))
  else:
      logger.info("List of webhook URLs found in conf files:")
      logger.info("\n".join(allowlist_dict.values()))
  return allowlist_dict

def write_webhook_allowlist(path, allowlist_dict):
  """
  path: path to app home directory for which the allowlist will exist
  allowlist_dict: a dictionary of domain regexes populated in find_webhooks
  """
  path_to_conf = path[:]

  if not os.path.isdir(path_to_conf):
    raise cex.ParsingError("app does not exist")
  path_to_conf = os.path.join(path_to_conf, "local")

  if not os.path.isdir(path_to_conf):
    logger.info("Created {}".format(path_to_conf))
    os.mkdir(path_to_conf)

  path_to_conf = os.path.join(path_to_conf, "alert_actions.conf")

  if not os.path.isfile(path_to_conf):
    with open(path_to_conf, 'w') as _:
      pass
    logger.info("Created {}".format(path_to_conf))

  config = comm.readConfFile(path_to_conf)
  config.pop("default")

  webhook_stanza = "webhook"
  if webhook_stanza in config.keys():
    for key in config[webhook_stanza].keys():
      if "allowlist." in key:
        logger.info("Allowlist already exists in {}, exiting...\n".format(path_to_conf))
        return
  else:
    config[webhook_stanza] = {}
  config[webhook_stanza].update(allowlist_dict)

  comm.writeConfFile(path_to_conf, config)
  logger.info("Done writing webhook URLs to allowlist in {}...\n".format(path_to_conf))

def find_dashboards_csp_settings(path):
  """
  path: path to $SPLUNK_HOME/etc
  retval: dictionary of dashboard csp settings 
  """
  dashboards_csp_settings_dict = {} 
  try:
    count = 1
    seen_domains = set()
    for dirpath, _, files in os.walk(path):
      for filename in files:
        if filename == "web.conf":
          config = comm.readConfFile(os.path.join(dirpath, filename))
          for _, stanza in config.items():
            csp_allowed_domains = stanza.get("dashboards_csp_allowed_domains") 
            if csp_allowed_domains:
              domain_list = csp_allowed_domains.split(",")
              for domain in domain_list:
                sanitized_domain = domain.strip()
                if (sanitized_domain != "*.api.splkmobile.com" and sanitized_domain not in seen_domains):
                  seen_domains.add(sanitized_domain)
                  dashboards_csp_settings_dict["dashboards_trusted_domain.domain{}".format(count)] = sanitized_domain
                  count += 1
              
    if dashboards_csp_settings_dict == {}:
      logger.info("No dashboard csp settings found in {}, nothing to migrate".format(path))
    else:
      logger.info("List of dashboard csp settings found in conf files:")
      logger.info("\n".join(dashboards_csp_settings_dict.values()))
    return dashboards_csp_settings_dict

  except Exception as e: 
    raise cex.ParsingError("The following error occured while finding dashboards csp settings: {}".format(e))

def write_dashboards_csp_settings(path, settings_dict):
  """
  path: path to app home directory where the dashboards csp settings exist
  settings_dict: a dictionary of dashboard csp settings 
  """
  path_to_conf = path[:]
  ...
  path_to_conf = os.path.join(path_to_conf, "web-features.conf")
  ...
  dashboards_stanza = "feature:dashboards_csp"
  if dashboards_stanza in config.keys():
    for key in config[dashboards_stanza].keys():
      if "dashboards_trusted_domain." in key:
        logger.info("Dashboards settings already exists in {}, exiting...\n".format(path_to_conf))
        return
  else:
    config[dashboards_stanza] = {}
  config[dashboards_stanza].update(settings_dict)

  comm.writeConfFile(path_to_conf, config)
  logger.info("Done writing dashboards csp settings in {}...\n".format(path_to_conf))
...
def migSampleIndex_4_2_1(system_indexes_conf, dryRun):
  ...
  a startup failure during splunk start reviews, including splunkd rest,
...
def remove_GeoLite_mmdb(dryRun):
  '''
  SPL-211932: remove outdated/noncompliant MMDB library
  '''
  old_mmdb_file = make_splunkhome_path(["share", "GeoLite2-City.mmdb"])
  if os.path.exists(old_mmdb_file):
    logger.info('Removing legacy mmdb library...')
    comm.removeItem(old_mmdb_file, dryRun)
...
def migrateLatencyTrackerLogIntervalSec(dryRun):
  """
  Rename 'latency_tracker_log_interval_sec' to 'latency_tracker_log_interval'
  """
  oldKeyName = 'latency_tracker_log_interval_sec'
  newKeyName = 'latency_tracker_log_interval'
  stanzaName = 'health_reporter'
  kvSep      = '='
  def migrateHealthConf(healthConf, dryRun):
    if not os.path.exists(healthConf):
      return
    wasChanged = False
    ## Process file content here to preserve comments
    with open(healthConf, 'r') as f:
    ...
    for line in oldLines:
      ...
      if 0 == line.count(kvSep):
        ...
        key, val = line.split(kvSep, 1)
        key = key.strip()
        # if this is key with old name then rename it
        if oldKeyName == key:
          newLines.append('%s =%s' % (newKeyName, val))
          wasChanged = True
        else:
          newLines.append(line)
    if not wasChanged:
      return
    if dryRun:
      logger.info("Would migrate file %s" % healthConf)
    else:
      logger.info("Migrating file %s" % healthConf)
      # write out the new content
      with open(healthConf, 'w+') as f: # w+ = write/truncate.
        f.write(str.join("", newLines))
    return

  for bundle in bundle_paths.bundles_iterator(unmanaged=True):
      defaultHealthConf = os.path.join(bundle.location(), 'default', 'health.conf')
      localHealthConf = os.path.join(bundle.location(), 'local', 'health.conf')
      migrateHealthConf(defaultHealthConf, dryRun)
      migrateHealthConf(localHealthConf, dryRun)

def migrate_webhooks_to_allowlist(dryrun):
  logger.info("Running webhook alert_action URL migration to webhook allowlist...")
  allowlist = find_webhooks(comm.splunk_home)

  if dryrun:
    logger.info("DRYRUN: no webhooks migrated to allowlist.")
  else:
    write_webhook_allowlist(bundle_paths.get_bundle("alert_webhook").location(), allowlist)

def migrate_dashboards_trusted_domains(dryrun):
  logger.info("Running URL migration from web to web-features dashbards trusted domains...")
  dashboards_csp_settings = find_dashboards_csp_settings(make_splunkhome_path(['etc']))

  if dryrun:
    logger.info("DRYRUN: no dashboards trusted domains migrated to web-features.")
  else: 
    write_dashboards_csp_settings(bundle_paths.get_system_bundle_path(), dashboards_csp_settings)

def migrate_add_default_dashboard_xml_version_1_1(dryrun):
  """
  SPL-226836: Make sure dashboards have a version. If they don't, write "1.1" as
  the version. This only checks user dashboards and the search app dashboards.

  This implementation tries to be efficient by using xml et.iterparse() to only
  read the root node rather than the whole document. See the Python docs on
  xml.etree.ElementTree.iterparse for details.

  TODO: FIXME(hamesg): Use f-strings in Py3 once Py2 unit testing is off
  """

  logger.info("Running XML migration to add v=1.1 to dashboards that don't specify a version")
  dashboards = []
  try:
    dashboards = [x for path in ['etc/apps', 'etc/users'] for x in comm.findFiles(os.path.join(comm.splunk_home, *path.split('/')), r'data[\\/]ui[\\/]views[\\/].*\.xml$', caseSens=False, minBytes=10)]
  except Exception as e:
    logger.error('Failed to collect files matching **/data/ui/views/*.xml: `{e}`'.format(e=e))
    return

  logger.info('Checking {len_dashboards} dashboards for "v=1.1" attribute'.format(len_dashboards=len(dashboards)))
  count = 0
  for dashboard in dashboards:
    log = ''
    log += dashboard.replace(comm.splunk_home, '$SPLUNK_HOME', 1)
    context = None
    root = None
    try:
      context = et.iterparse(dashboard, events=("start", "end"))
      _, root = next(context)
    except et.ParseError as e:
      logger.error('{log} parse error: `{e}`'.format(log=log, e=e))
      continue
    if not (root.tag == 'dashboard' or root.tag == 'form'):
      log = '{log} is <{root_tag} ...> skipping'.format(log=log,root_tag=root.tag)
      # Free memory (might be unnecessary but I want to close the iterator)
      root.clear()
      logger.info(log)
      continue
    # We have a dashboard/form
    if 'version' in root.attrib:
      version = root.attrib['version']
      if version == '1.0':
        log = '{log} has legacy version=1.0 explicitly set - skipping'.format(log=log)
      elif version == '1.1' or version == '2':
        log = '{log} has version={version} - ok'.format(log=log, version=version)
      else:
        log = '{log} warning unknown version `version={version}` - skipping'.format(log=log, version=version)
      logger.info(log)
      continue

    # We have a null version dashboard
    try:
      # Force the iterator to completion
      list(context)

      # read dashboard and add version number
      
      # FIXME: SPL-244554 - We may not get UTF-8 here. Could be other encodings (eg. UTF-16), see SPL-244133
      # Enforcing utf-8 encoding here to ensure we don't silently break non-utf-8 encoded dashboards
      # More context at https://cd.splunkdev.com/splcore/main/-/merge_requests/62756/diffs#note_10640215
      with open(dashboard, 'r', encoding='utf-8') as f:
        dashboard_contents = f.read()
      dashboard_contents = re.sub(r'^(<form|<dashboard)(.*>)', r'\1 version="1.1"\2', dashboard_contents)

      if dryrun:
        log = '{log} would be upgraded to version=1.1 - DRYRUN MIGRATE'.format(log=log)
        count += 1
      else:
        try:
          with open(dashboard, 'wb') as f:
            f.write(dashboard_contents.encode('utf-8'))
        except Exception as e:
          logger.error('{log} serialize error or write error: `{e}`'.format(log=log, e=e))
          continue
        log = '{log} upgraded to version=1.1 - MIGRATED'.format(log=log)
        count += 1
        logger.info(log)
    except Exception as e:
      logger.error('{log} failed to migrate the dashboard: `{e}`'.format(log=log, e=e))
      continue
  if dryrun:
    print('Would migrate {count} dashboards - running as dryrun'.format(count=count))
  else:
    print('Migrated {count} dashboards'.format(count=count))

def migrate_splunk_launcher_conf(dryRun):

  # if we don't have etc/splunk-launch.conf, there's nothing to do; this should never happen!
  if not os.path.exists(PATH_SPLUNK_LAUNCH_CONF):
    logger.info("Ummm... {} not found".format(PATH_SPLUNK_LAUNCH_CONF))
    return

  if not dryRun:
    # copy to backup version
    logger.info("backup {}".format(PATH_SPLUNK_LAUNCH_CONF))
    shutil.copy(PATH_SPLUNK_LAUNCH_CONF, PATH_SPLUNK_LAUNCH_CONF_BAK)

  stanzaDict = comm.readConfFile(PATH_SPLUNK_LAUNCH_CONF)
  # default stanza is all there is
  default = stanzaDict.get('default')

  if default is None:
    logger.info("default stanza not found, PYTHONUTF8 not set in {}".format(PATH_SPLUNK_LAUNCH_CONF))
    return

  if default.get('PYTHONUTF8') is not None:
    logger.info("PYTHONUTF8 setting is set to '{}', not changing".format(default['PYTHONUTF8']))
    return

  logger.info("PYTHONUTF8 setting is missing from {} ".format(PATH_SPLUNK_LAUNCH_CONF))
  default['PYTHONUTF8'] = '1'
  stanzaDict['default'] = default;
  # done, write out the new splunk-launcher.conf
  if not dryRun:
    logger.info("updating {} with PYTHONUTF8=1".format(PATH_SPLUNK_LAUNCH_CONF))
    comm.writeConfFile(PATH_SPLUNK_LAUNCH_CONF, stanzaDict)
...
def autoMigrate(logPath, dryRun = False):
  ...
  # SPL-207130 Rename 'latency_tracker_log_interval_sec' to 'latency_tracker_log_interval'
  migrateLatencyTrackerLogIntervalSec(dryRun)

  # SPL-213371 allowlist webhook URL endpoints found in savedsearches.conf and alert_actions.conf
  migrate_webhooks_to_allowlist(dryRun)
 
  # SPL-219618 Dashboard CSP domains in web-features.conf
  migrate_dashboards_trusted_domains(dryRun)

  # SPL-226836 Update XML dashboards that are v=null to v=1.1 (but leave v=1.0 as is)
  migrate_add_default_dashboard_xml_version_1_1(dryRun)

  #
  # escalation 8213-japanese-windows-release
  # and other associated JIRA tickets:
  # SPL-244559, SPL-244865, SPL-245880, SPL-246151.
  #
  # migrate splunk-launch.conf for all platforms, to conditionally set PYTHONUTF8=1 
  #
  migrate_splunk_launcher_conf(dryRun)
