...
from splunk.rest.splunk_web_requests import get_page_migration_settings
import logging
...
class TemplateLookup(mako.lookup.TemplateLookup):
    ...
    def get_template(self, uri):
        ...
        if not uri.endswith('.html'):
            raise TypeError(_("Could not load template: '%s'. Templates must end with the file type '.html'." % uri))
        ...
        try:
            ...
            dir_and_file = template_path.split(os.path.sep)[-2:]
            if len(dir_and_file) == 2 and dir_and_file[0] == 'lookups':
                # lookups are static data and not templates. do not render.
...
class BaseController(with_metaclass(routes.RoutableType, object)):
    ...
    def render_template(self, template_name, template_args=None):
        ...
        template_args['get_page_migration_settings'] = get_page_migration_settings
