...
try:
    ...
    def mount_static(ctrl, global_cfg, cfg):
        ...
        def static_resolver(section, branch, dir_path):
            ...
            # overrides the 'dir_path' param with where applications are stored.
            if branch.startswith('app/'):
                return static_app_resolver(section, branch, static_app_dir)

            fn = os.path.normpath(os.path.join(dir_path, branch))
            splunk_home_base_dir = os.path.dirname(os.environ['SPLUNK_HOME'])
            if branch == 'js/i18n.js':
                ...
                return False
            # SPL-229055 restrict file path to be present inside $SPLUNK_HOME
            elif is_cacheable_js(branch) and os.path.exists(fn) and (splunk_home_base_dir == os.path.commonpath((splunk_home_base_dir, fn))):
    ...
    def run(blocking=True):
        ...
        # bailing with a message since python's ssl itself stops responding with encrypted keys
