...
def checkForSendEmail(search):
    """
    Throws a SearchException if search contains any 'sendemail' commands
    """
    if re.search('^(.*\|)?\s*sendemail\s', search):
        raise SearchException('During export, Splunk blocks searches containing "| sendemail".')
...
class InlineSearchManager(BaseSearchManager):
    ...
    def dispatch_internal(self, **kwargs):
        ...
        if 'checkRiskyCommand' in kwargs:
            options['check_risky_command'] = kwargs['checkRiskyCommand']
...
class PostProcessSearchJob(splunk.search.SearchJob):
    ...
    def setFetchOption(self, **kwargs):
        ...
        if 'check_risky_command' in self.request:
            newargs['check_risky_command'] = self.request['check_risky_command']
...
def createPostProcessJob(job, postprocessSearch):
    checkForSendEmail(postprocessSearch)
    jobArgs = dict(
...
class SavedSearchManager(BaseSearchManager):
    ...
    def dispatch_internal(self, overrideNowTime=None, **kwargs):
        ...
        checkForSendEmail(self.model().search)
        if 'checkRiskyCommand' in kwargs:
            dispatchArgs['checkRiskyCommand'] = kwargs['checkRiskyCommand']
