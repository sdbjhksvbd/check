...
class DynamicUIHelper(dict):
    ...
    def _eval_str(self, cfgstr, additional_locals=None):
        ...
        # SPL-226673 - To prevent remote code execution, limit the __globals & __locals to only the ones we actively use
        # The list of variables referenced below is compiled after walking through all the existing XML files that leverage "dynamicOptions" & have properties that use an expression.
        # All the variables used within those expressions have been exposed via __locals below.
        use_locals = {
            'entry': locals().get('entry', {}),
            'title': locals().get('title', ''),
            'entity_name': locals().get('entity_name', ''),
            'form_defaults': locals().get('form_defaults', {}),
            'str': str,
        }

        return eval(cfgstr, { '__builtins__': {}}, use_locals)
