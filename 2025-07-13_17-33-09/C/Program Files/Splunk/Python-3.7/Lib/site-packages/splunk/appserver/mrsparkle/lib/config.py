...
def _get_active_config():
    ...
    if cherrypy.request.headers.get('X-Splunkd') is not None:
        ...
        remote_user_header = cherrypy.request.config.get('remoteUser') or 'REMOTE_USER'
    ...
    return {
        ...
        'BUILD_PUSH_NUMBER': util.push_version(),
        ...
        'JOB_DEFAULT_AUTO_CANCEL': int(float(cherrypy.config.get('job_default_auto_cancel', 62))),
        ...
        'ENABLE_RISKY_COMMAND_CHECK_DASHBOARD': cherrypy.config.get('enable_risky_command_check_dashboard', True),
        'DISABLE_HIGHCHARTS_ACCESSIBILITY': cherrypy.config.get('disable_highcharts_accessibility', True)
    }
