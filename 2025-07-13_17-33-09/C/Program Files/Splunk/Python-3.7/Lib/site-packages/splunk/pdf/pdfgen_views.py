...
class AbstractViewType(object):
    ...
    def dispatchSearch(self, overrideNowTime=None, maxRowsPerTable=None, stripLeadingSearchCommand=False):
        ...
        if self.search is not None:
            try:
                kwargs = dict(overrideNowTime=overrideNowTime, stripLeadingSearchCommand=stripLeadingSearchCommand, checkRiskyCommand=self._checkRiskyCommand)
...
def getStudioDashboardContent(dashboard_name, namespace, owner, sessionKey):
...
def getDashboardTitleAndPanelsFromXml(dashboardXml, namespace, owner, sessionKey):
    ...
    panels = []
    checkRiskyCommand = pu.isRiskyCommandCheckDashboardEnabled(sessionKey)
    ...
    def add_element(props, search, error=None):
        ...
        panelElement = DashboardPanel(props, search, len(panelList), namespace, owner, sessionKey, checkRiskyCommand)
        if error:
...
class DashboardPanel(AbstractViewType):
    ...
    def __init__(self, panelDict, search, sequenceNum, namespace, owner, sessionKey, checkRiskyCommand):
        ...
        self._checkRiskyCommand = checkRiskyCommand
    ...
    def getSingleValueProps(self):
        ...
        if 'options' in self._panelDict:
            ...
            if 'rangeColors' in singleValueOptions and 'rangeValues' not in singleValueOptions:
                # rangeValues default values from savedsearches.conf.in
                singleValueOptions['rangeValues'] = '[0,30,70,100]'
...
class Report(AbstractViewType):
    ...
    def _isTransformingSearch(self):
        ...
        try:
            ...
            logger.error(f"{str(searchStr)} cannot be parsed by the splunk.search.Parser: {e}")
            return False
