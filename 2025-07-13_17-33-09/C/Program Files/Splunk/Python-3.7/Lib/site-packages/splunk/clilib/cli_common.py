...
def bom_aware_readline(fileobj, do_not_fold_pattern = None):
    ...
    if sys.version_info >= (3, 0):
        # SPL-222543 - Unable to generate diag - "UnicodeDecodeError: 'utf-8' codec can't
        # decode byte XxXX in position YY: invalid start byte"
        try:
            return line.decode()
        except UnicodeDecodeError as ex:
            # non-ASCII/UTF-8 characters detected in config file.
            # We can ignore these characters instead of completely bailing out
            logging.warn("Invalid characters found in config file '" + fileobj.name + "', will ignore and continue....")
            logging.debug("Exception raised : '%s'\n" % ex)
            return line.decode(errors='backslashreplace')
    return line
...
def getServerInstanceType():
  return getConfKeyValue("server", "general", "instanceType", None)

def isCloudInstanceType():
  if getServerInstanceType() == "cloud":
    return True
  return False
...
def decrypt(value, setEnv=False):
  ...
  cmd = [launcher_path, 'show-decrypted', '--value']
  if (setEnv):
      cmd.append('')
      os.environ["ENCRYPTED_PASSWORD"] = value
...
def checkValidFileEAICLI(path):
   """
   version of this function that does not use invokeAPI stuff...for the new CLI
   """
