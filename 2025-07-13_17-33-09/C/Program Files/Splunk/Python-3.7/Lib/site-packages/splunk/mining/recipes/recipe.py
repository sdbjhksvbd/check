...
def log(msg):
    log_file = '/tmp/recipe.txt'
    if DEBUG:
        flog = open(log_file, "a")
        flog.write(msg)
        flog.close()
    else:
        print("WARNING: DEBUG set to False. Not writing logs to %s" % log_file)
...
def test2():
    ...
    print("FIND: 2 %s" % str(Chef.find(obj, obj, 2)))
    print("FIND: 4 %s" % str(Chef.find(obj, obj, 4)))

    print("UPDATE: 4 DO NOTHING %s" % Chef.update(obj, 4, 'class', 'python'))
    print("PY:    %s" % obj.toPY())

    print("UPDATE: 4 %s" % str(Chef.update(obj, 4, 'code', 'foobar')))
    print("UPDATE: 4 %s" % str(Chef.update(obj, 4, 'class', 'parallel')))
    print("PY:    %s" % obj.toPY())
    return
