...
# SPL-228769: Use timeout default from web.conf[settings]splunkdConnectionTimeout
#             when calling simpleRequest and streamingRequest
rest_splunkd_connection_timeout = SPLUNKD_CONNECTION_TIMEOUT
try:
    splunkdConnectionTimeout = int(getWebConfKeyValue('splunkdConnectionTimeout'))
    rest_splunkd_connection_timeout = max(rest_splunkd_connection_timeout, splunkdConnectionTimeout)
except Exception as e:
    logger.error("Could not access or parse settings stanza of web.conf. Error=%s" % e)
finally:
    logger.debug("splunkdConnectionTimeout=%s" % rest_splunkd_connection_timeout)
...
class BaseRestHandler(object):
    ...
    def handle_VIEW(self):
        ...
        try:
            ...
            logger.error('BaseRestHandler - No such file or directory=%s' % path)
            logger.exception(e)
            self.response.status = 404
            strerror = e.strerror if hasattr(e, "strerror") else "No such file or directory"
            self.response.write(strerror)
        except Exception as e:
...
def enforcePathSecurityPolicy(path, sessionKey):
    ...
    if not enableSplunkWebClientNetloc and (sessionKey is None or not isSplunkSessionKey(sessionKey)):
        ...
        if (not parsedPathUrlScheme or not parsedPathUrlNetloc) or (parsedPathUrlScheme and parsedPathUrlScheme != parsedSplunkdUrl.scheme) or (parsedPathUrlNetloc and parsedPathUrlNetloc != parsedSplunkdUrl.netloc):
...
def getSSLClientContextFromServerConf():
    """Create an SSLContext that allows override of [sslConfig] with [pythonSslClientConfig] settings, if available."""
    # this should only be called on Python 3, and SSLHelper is only import on Python 3
    ...
    # Python 3.7: merged_stanza = {**sslconfig_stanza, **pysslconfig_stanza}
    # Python 3.9: merged_stanza = sslconfig_stanza | pysslconfig_stanza
    merged_stanza = dict()
    ...
    ctx = splunk.ssl_context.SSLHelper().createSSLContextFromSettings(
        sslConfJSON=merged_stanza,
        serverConfJSON=sslconfig_stanza,
        isClientContext=True)

    logger.debug("verify_mode={} check_hostname={} CA_certs={}".format(
        ctx.verify_mode.name, ctx.check_hostname, ctx.get_ca_certs()))
...
def simpleRequest(path, sessionKey=None, getargs=None, postargs=None, method='GET', raiseAllErrors=False,
                  ...
                  timeout=None, jsonargs=None, token=False, keepTrailingSlash=False, headers=None):
    """
    ...
    if timeout is None or timeout < rest_splunkd_connection_timeout:
        timeout = rest_splunkd_connection_timeout
    ...
    # TODO: this probably doesn't work in windows
        ...
        if keepTrailingSlash:
            uri = makeSplunkdUri() + path.lstrip('/')
        else:
    ...
    ctx = None
    # only Python 3.x httplib2 is patched to accept a SSLContext param
    if sys.version_info >= (3, 0):
        host = parsedUri.hostname
        port = parsedUri.port
        islocalhost = host == 'localhost' or host == '127.0.0.1' or host == '::1'
        mgmt_port = int(getWebConfKeyValue("mgmtHostPort").rsplit(':', 1)[1])
        if not (islocalhost and port == mgmt_port):
            ctx = getSSLClientContextFromServerConf()
            # warn if certificate validation is enabled without hostname verification
            if ctx.verify_mode == ssl.CERT_REQUIRED and ctx.check_hostname == False:
                logger.warn("simpleRequest SSL certificate validation is enabled without hostname verification."
                            " See: server.conf/[pythonSslClientConfig]/sslVerifyServerName.")
    ...
    try:
        ...
        for aTry in range(tries):
            # SSL context available for Python 3.x httplib2 only
            if ctx:
                h = httplib2.Http(timeout=timeout, proxy_info=None, context=ctx)
                if ctx.verify_mode == ssl.CERT_REQUIRED and ctx.check_hostname == True:
                    try:
                        # if hostname is an IP address, just leave it as is and check for IP in CN/SAN
                        ipaddress.ip_address(parsedUri.hostname)
                    except:
                        # replace hostname with FQDN
                        fqdn = socket.getfqdn(parsedUri.hostname)
                        if parsedUri.hostname != fqdn:
                            fqdnUrl = parsedUri._replace(
                                netloc=parsedUri.netloc.replace(parsedUri.hostname, fqdn))
                            uri = fqdnUrl.geturl()
                            msg = ("simpleRequest hostname validation is enabled, "
                                   "replacing hostname={} with FQDN={}, URL={}.")
                            logger.info(msg.format(parsedUri.hostname, fqdn, uri))
            else:
        ...
        msg = 'SSL error communicating with splunkd, error="{}", path={}'.format(e, path)
        logger.error(msg)
...
def streamingRequest(path, sessionKey=None, getargs=None, postargs=None, method='GET', timeout=None):
    ...
    if sys.version_info >= (3, 0):
        ...
        if not (islocalhost and port == mgmt_port):
            ctx = getSSLClientContextFromServerConf()
            # warn if certificate validation is enabled without hostname verification
            if ctx.verify_mode == ssl.CERT_REQUIRED and not ctx.check_hostname:
                logger.warn("streamingRequest SSL certificate validation is enabled without hostname verification."
                            " See: server.conf/[pythonSslClientConfig]/sslVerifyServerName.")

    try:
        if ctx:
            if ctx.verify_mode == ssl.CERT_REQUIRED and ctx.check_hostname:
                try:
                    # if hostname is an IP address, just leave it as is and check for IP in CN/SAN
                    ipaddress.ip_address(parsedUri.hostname)
                except:
                    ...
                    if parsedUri.hostname != fqdn:
                        fqdnUrl = parsedUri._replace(
                            netloc=parsedUri.netloc.replace('://'+parsedUri.hostname, '://'+fqdn, 1))
                        uri = fqdnUrl.geturl()
                        msg = ("streamingRequest hostname validation is enabled, "
                               "replacing hostname={} with FQDN={}, URL={}.")
                        logger.info(msg.format(parsedUri.hostname, fqdn, uri))
                        parsedUri = urllib_parse.urlsplit(uri)
        else:
            ctx = ssl._create_unverified_context()
        conn = httplib.HTTPSConnection(host, port, getWebKeyFile(), getWebCertFile(), timeout=timeout, context=ctx) if isssl else httplib.HTTPConnection(host, port, timeout=timeout)
        conn.connect()
