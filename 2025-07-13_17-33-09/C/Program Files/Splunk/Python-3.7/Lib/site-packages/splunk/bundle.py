...
class MainTest(unittest.TestCase):
    ...
    def test_emptyValueWrite(self):
        ...
        conf = getConf('web', namespace='search', sessionKey=self.sessionKey)
        stanza = conf['test']
        self.assertTrue('emptyKey' not in stanza, '"emptyKey" key was written when it was not expected to')
    
    def test_remote_hostpath(self):
        conf = getConf('web', namespace='search', sessionKey=self.sessionKey)
        self.assertTrue(isinstance(conf, Conf), "The optional argument hostPath works when ignored")
        
        conf = getConf('indexes', namespace='search', sessionKey=self.sessionKey, hostPath=splunk.getLocalServerInfo())
        self.assertTrue(isinstance(conf, Conf), "The optional argument hostPath works when used")
        
        confName = 'testconf_%s' % round(time.time())
        newConf = createConf(confName, namespace="testing", sessionKey=self.sessionKey, hostPath=splunk.getLocalServerInfo())
        self.assertTrue(isinstance(newConf, Conf), "The optional argument hostPath works when used in createConf")
