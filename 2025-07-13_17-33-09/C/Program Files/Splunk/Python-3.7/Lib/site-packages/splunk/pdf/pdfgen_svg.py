...
def getSvgImageFromString(svgString, fontManager, sessionKey=None):
    """ replace any instances of &nbsp since it cannot be handled by lxml: SPL-196469 """
    parsedString = svgString.replace('&nbsp;', '&#160;')
    """ returns a Platypus Flowable object that will draw the SVG that's given
        in the string svgString """
    logger.debug("getSvgImageFromString> svgString: " + str(parsedString))

    try:
        svgRenderer = SVGRenderer(parsedString, fontManager, sessionKey=sessionKey)
        drawing = svgRenderer.getDrawing()
    except Exception as e:
        errorMsg = "Error parsing SVG. Exception=\"%s\" svgString=\"%s\"" % (str(e), parsedString)
...
class SVGRenderer(object):
    ...
    #
    #
    def __init__(self, svgString, fontManager, sessionKey=None):
    ...
    def _opacityMapping(self, attributes, svgAttributes):
        ...
        for opacityOverrideKey in self._opacityOverrideMap:
            if opacityOverrideKey in svgAttributes and re.match('^\s*rgba', svgAttributes[opacityOverrideKey]):
                if self._opacityOverrideMap[opacityOverrideKey] in attributes:
    ...
    def renderElementPath(self, element, group, isClipPath=0):
        ...
        path = shapes.Path(isClipPath=isClipPath, autoclose="svg", **attributes)
    ...
    def renderElementImage(self, element, group, isClipPath=0):
        ...
        image = image_utils.PngImage(**attrs, sessionKey=self._sessionKey)
