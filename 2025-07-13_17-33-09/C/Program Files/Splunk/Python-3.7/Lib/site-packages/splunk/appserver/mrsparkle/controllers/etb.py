...
def getMessagePriorityIndex(message):
    message_priority_index = float("inf")
    try:
        message_type = re.search('class="(.*?)"', message).group(1)
        message_priority_index = CVALIDLIST.index(message_type)
    except Exception:
        try:
            message_type = re.search('<title>(.*?)</title>', message).group(1).lower()
            message_priority_index = CVALIDLIST.index(message_type)
        except Exception:
            logger.warn('Unknown message type in message: %s' % message)

    return message_priority_index

def errorCMP(x,y):
    return cmp(getMessagePriorityIndex(y),getMessagePriorityIndex(x))
...
class ETBController(BaseController):
    ...
    def index(self, **kwargs):
        ...
        # if user modified eventtype and didn't press preview button, use his; otherwise generate it
        if args['edited']:
...
def eventTypeExists(args, eventTypeName=None):
    ...
    for stanzaname in props:
        ...
        if search.strip().lower() == eventtype.strip().lower():
            ...
            # just be nice and don't warn if the default search wasn't modified
...
class MainETBTest(unittest.TestCase):
    def setUp(self):
        self.cp_session = getattr(cherrypy, 'session', None)

    def tearDown(self):
        if self.cp_session:
            setattr(cherrypy, 'session', self.cp_session)
        else:
            if hasattr(cherrypy, 'session'): delattr(cherrypy, 'session')
...
class ETBMessagePriorityTest(unittest.TestCase):
    def test_invalid_message(self):
        message = 'non-valid-message-html'
        self.assertEqual(getMessagePriorityIndex(message), float('inf'))

    def test_invalid_message_class_and_title(self):
        message = '<p class="not-a-message-type"><svg style=""><title>not valid</title></svg></p>'
        self.assertEqual(getMessagePriorityIndex(message), float('inf'))

    def test_message_type_in_class(self):
        message = '<p class="warn">warning</p>'
        self.assertEqual(getMessagePriorityIndex(message), CVALIDLIST.index(CWARN))

    def test_message_type_in_title_tag(self):
        message = '<svg style="fill:#dc4e41;padding-right:16px;padding-left:16px" focusable="false" height="24px" width="24px" viewBox="0 0 1375 1500" aria-hidden="false" preserveAspectRatio="xMidYMid" xmlns="http://www.w3.org/2000/svg" class="inline_SVG_d13579264976c537_847fc6 main_SVG_d13579264976c537_847fc6"><title>Error</title><path d="M187.5 61.5h1000c103.553 0 187.5 83.947 187.5 187.5v1000c0 103.553-83.947 187.5-187.5 187.5h-1000C83.947 1436.5 0 1352.553 0 1249V249C0 145.447 83.947 61.5 187.5 61.5zm400.79 413.083l32.145 257.167c4.908 39.264 34.086 74.685 69.815 91.187 36.612-16.018 64.87-50.826 69.914-91.187l32.146-257.167C799.18 419.623 759.582 374 703.7 374h-26.8c-55.908 0-95.555 45.033-88.61 100.583zm101.293 644.209c63.283 0 114.584-51.301 114.584-114.584 0-63.282-51.301-114.583-114.584-114.583-63.282 0-114.583 51.3-114.583 114.583s51.3 114.584 114.583 114.584z"></path></svg>'
        self.assertEqual(getMessagePriorityIndex(message), CVALIDLIST.index(CERROR))


if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(MainETBTest)
    unittest.TextTestRunner(verbosity=2).run(suite)

    suite = unittest.TestLoader().loadTestsFromTestCase(ETBMessagePriorityTest)
    unittest.TextTestRunner(verbosity=2).run(suite)
