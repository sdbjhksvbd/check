...
from splunk.rest.splunk_web_requests import get_splunk_web_optimizations_settings
...
def get_user_theme():
    try:
        sessionKey, user = cherrypy.session['sessionKey'], cherrypy.session['REMOTE_USER']
        user_entity = en.getEntity('authentication/users', user, sessionKey=sessionKey)
        theme = user_entity['theme'] or 'enterprise'
        return theme
...
def getDashboardV2TemplateUri():
    return '/view/splunk-dashboard-studio:/templates/dashboard.html'

def v1p0_dashboard_exists():
    v1_template_path = 'share/splunk/search_mrsparkle/templates%s' % getDashboardV1TemplateUri()
    return splunk_home_file_exists(v1_template_path)
...
def getServerInfoPayload():
    ...
    server_info['entry'][0]['content']['isServerInfoAvailable'] = cherrypy.config.get('server_info_fetch', False)
    server_info['entry'][0]['content']['isConverged'] = cherrypy.config['isConverged']
    return server_info
...
def redactSensitiveData(data, keys, value='--redacted--'):
...
def make_url(target, _qs=None, translate=True, relative=False, __app_cache=None, encode=True, validate=True, appendcachebust=False):
    ...
    constructed_url = target+qargs
    # disallow double leading forward slash (ref: SPL-151752)
    if constructed_url.startswith('//'):
        return None
...
def get_setting_from_config(setting):
    return cherrypy.config[setting] if setting in cherrypy.config else None
...
def push_version():
    ...
    """
    now = datetime.now()
    last_push_version_read = get_setting_from_config('_last_push_version_read') or now
    cherrypy.config['_last_push_version_read'] = last_push_version_read
    seconds_since_last_load = (now - last_push_version_read).total_seconds()
    if seconds_since_last_load > 30:
        try:
            f = open(make_absolute('var/run/splunk/push-version.txt'), 'r')
            num = int(f.read(20))
            f.close()
            cherrypy.config['_push_version'] = num
            cherrypy.config['_last_push_version_read'] = datetime.now()
        except:
            last_push_version = get_setting_from_config('_push_version')
            if last_push_version is None:
                cherrypy.config['_push_version'] = 0
    return get_setting_from_config('_push_version') or 0
...
def splunk_to_cherry_cfg(conf, stanza):
    ...
    # Why an array for a single value?
    # There are other URI based attributes like "rss_endpoint", "static_endpoint", "root_endpoint" 
    # which may need the same sanitization
    uri_based_web_conf_attributes = ["embed_uri"]
    ...
    if conf == "web" and stanza == "settings":
        ...
        # settings with URI values need to extract domain only 
        for setting in uri_based_web_conf_attributes:
            if setting in cfg:
                try:
                    raw_uri = urllib_parse.urlparse(cfg[setting])
                    # get only root URL of the domain
                    if raw_uri.scheme and raw_uri.hostname: 
                        extracted_root_url = raw_uri.scheme + '://' + raw_uri.hostname
                        cfg[setting] = extracted_root_url
                    else:
                        raise ValueError('Invalid entry for setting : %s' % setting)
                except ValueError as e:
                    logger.error('unable to parse %s as URL: %s' % (setting, e))
                    cfg[setting] = ''
...
def extract_trusted_domains(dashboards_csp_settings, trusted_domain_prefixes, add_http_prefix=False):
    """
    extracts the trusted domains from dashboards_csp_settings[trusted_domain_prefixes.*]
    """
    domains = []
    for key in dashboards_csp_settings:
        keyPrefixMatch = False
        for prefix in trusted_domain_prefixes:
            if (key.startswith(prefix)):
                keyPrefixMatch = True
                break
        if keyPrefixMatch is True and dashboards_csp_settings[key] is not None:
            domain = dashboards_csp_settings[key]
            # Add http:// to anything that looks like a domain (*.com, *.example.com, example.com, www.example.com/some/path)
            if add_http_prefix and re.search(r'^((\*|\w+)\.)?((\*|[\w-]+)\.)\w+', domain, re.IGNORECASE) is not None:
                domain = 'http://' + domain
            domains.append(domain)

    return domains

def extract_csp_header(required_values, dashboards_csp_settings, required_directives, trusted_domain_prefixes, add_http_prefix=False, csp_header_size_limit=6500):
    domains = required_values.copy()
    domains += (extract_trusted_domains(dashboards_csp_settings,
                trusted_domain_prefixes, add_http_prefix))

    csp_values_string = ' '.join(domains)
    csp_header = '; '.join('%s %s' % (name, csp_values_string)
                           for name in required_directives)
    
    # If the response header is too big, the page will fail to load entirely so
    # we should limit the size of the CSP header to avoid that.
    # Testing revealed that page load failures occur when CSP header was around
    # 7500 characters, so setting csp_header_size_limit to 6500 characters by
    # default should give a decent buffer.
    # Using shorten will drop any domains that exceed the limit.
    return textwrap.shorten(csp_header, width=csp_header_size_limit, placeholder='')
...
def encode_ansi_escape_start(string):
    """
    Return string after encoding ansi escape start
    ANSI escapes always start with \x1b, or \e, or \033
    They are just various ways of inserting the byte 27 into a string.
    """
    try:
        if string:
            output_string = string.replace(chr(27), '%1B')
            return output_string
    except Exception as e:
        logger.info('Exception thrown, %s', e)
    return string

cache_use_app_bar_performance_optimizations = {}
cache_use_app_bar_ttl = 60
def should_use_app_bar_performance_optimizations(app):
    now_time = time.time()
    cached_value = cache_use_app_bar_performance_optimizations.get(app)
    if bool(cached_value) and (now_time - cached_value['time']) < cache_use_app_bar_ttl:
        return cached_value['value']

    def wrapped_fn(appName):
        splunk_web_optimizations_settings = get_splunk_web_optimizations_settings()
        optimize_app_bar = splunk_web_optimizations_settings["enable_app_bar_performance_optimizations"]
        bypass_optimization_apps = splunk_web_optimizations_settings["bypass_app_bar_performance_optimizations_apps"]
        if not optimize_app_bar:
            return False
        
        if bool(appName) and appName.lower() in bypass_optimization_apps.lower():
            return False
        return True
    result = wrapped_fn(app)
    cache_use_app_bar_performance_optimizations[app] = { 
        'value': result,
        'time': time.time()}
    return result
...
class MakeURLSuite(unittest.TestCase):
    def test_ensure_double_forward_slash_not_allowed(self):
        original_url = '//google.com'
        expected_url = None
        actual_url = make_url(original_url, translate=False, relative=False, encode=False, validate=False)
        self.assertEqual(expected_url, actual_url)
    def test_ensure_double_forward_slash_not_allowed_translate(self):
        original_url = '//google.com'
        expected_url = '/en-US/google.com'
        actual_url = make_url(original_url, translate=True, relative=False, encode=False, validate=False)
        self.assertEqual(expected_url, actual_url)

class extractCSPHeader(unittest.TestCase):
    def setUp(self):
        self.required_domains = ["'self'", "data:", "blob:"]
        self.required_directives = ["img-src"]
        self.prefixes = ["dashboards_trusted_domain.", "internal.dashboards_trusted_domain."]
        self.trusted_domains1 = {
            'enable_dashboards_external_content_restriction': 'true',
            'dashboards_trusted_domain.domain1': 'www.example.com',
            'dashboards_trusted_domain.domain2': '*.google.com',
            'internal.dashboards_trusted_domain.domain1': 'docs.splunk.com',
            'internal.dashboards_trusted_domain.domain2': 'splunkbase.splunk.com',

        }
        self.trusted_domains2 = {
            'notvalidprefix': 'something'
        }
        self.trusted_domains3 = {
            'enable_dashboards_external_content_restriction': 'true',
            'dashboards_trusted_domain.domain1': None,
            'dashboards_trusted_domain.domain2': None,
            'dashboards_trusted_domain.domain3': 'www.example.com',
            'internal.dashboards_trusted_domain.domain1': 'docs.splunk.com',
        }
        self.trusted_domains4 = {
            'enable_dashboards_external_content_restriction': 'true',
            'dashboards_trusted_domain.domain1': '*.com',
            'dashboards_trusted_domain.domain2': 'example.com',
            'dashboards_trusted_domain.domain3': '*.example.com',
            'dashboards_trusted_domain.domain4': 'www.example.com',
            'dashboards_trusted_domain.domain5': 'www.example.com/some/path/',
            'dashboards_trusted_domain.domain6': 'https://*.example.com',
            'dashboards_trusted_domain.domain7': '*',
            'dashboards_trusted_domain.domain8': "'none'",
            'internal.dashboards_trusted_domain.domain1': 'docs.splunk.com',
        }
        self.trusted_domains5 = {
            'dashboards_trusted_domain.domain1': 'www.should.be.included.com',
            'dashboards_trusted_domain.domain2': '*.this.entire.domain.should.be.dropped.com'
        }

    def test_only_defaults(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.trusted_domains2, self.required_directives, self.prefixes)
        expected_csp_header = "img-src 'self' data: blob:"
        self.assertEqual(actual_csp_header, expected_csp_header)

    def test_custom_values(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.trusted_domains1, self.required_directives, self.prefixes)
        expected_csp_header = "img-src 'self' data: blob: www.example.com *.google.com docs.splunk.com splunkbase.splunk.com"
        self.assertEqual(actual_csp_header, expected_csp_header)
    
    def test_undefined_values(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.trusted_domains3, self.required_directives, self.prefixes)
        expected_csp_header = "img-src 'self' data: blob: www.example.com docs.splunk.com"
        self.assertEqual(actual_csp_header, expected_csp_header)
    
    def test_http_prefix(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.trusted_domains4, self.required_directives, self.prefixes, True)
        expected_csp_header = "img-src 'self' data: blob: http://*.com http://example.com http://*.example.com http://www.example.com http://www.example.com/some/path/ https://*.example.com * 'none' http://docs.splunk.com"
        self.assertEqual(actual_csp_header, expected_csp_header)

    def test_csp_header_truncate(self):
        actual_csp_header = extract_csp_header(self.required_domains, self.trusted_domains5, self.required_directives, self.prefixes, False, 60)
        expected_csp_header = "img-src 'self' data: blob: www.should.be.included.com"
        self.assertEqual(actual_csp_header, expected_csp_header)

    def test_extract_trusted_domains(self):
        actual_trusted_domains = extract_trusted_domains({ 'allowed.domain': 'domain1', 'not-allowed.domain': 'domain2'}, ['allowed.'], False)
        self.assertEqual(actual_trusted_domains, ['domain1'])


class EncodeAnsiEscapeStart(unittest.TestCase):
    def test_encode_ansi_escape_start(self):
        string = "\033[32mthis is GREEN\033[0m"
        self.assertEqual(encode_ansi_escape_start(string), '%1B[32mthis is GREEN%1B[0m')

    def test_encode_ansi_escape_start_1(self):
        string = "\u001b[32mthis is GREEN\u001b[0m"
        self.assertEqual(encode_ansi_escape_start(string), '%1B[32mthis is GREEN%1B[0m')

    def test_encode_ansi_escape_start_2(self):
        string = "\x1b[32mthis is GREEN\x1b[0m"
        self.assertEqual(encode_ansi_escape_start(string), '%1B[32mthis is GREEN%1B[0m')

    def test_encode_ansi_escape_print(self):
        """
        Below URL contains ANSI escape codes which will invoke print dialog if it is not encoded when opening the
        log file
        """
        string = """/en-US/app/launcher/home/injection/]8;;http://evil.com/]1337;AddAnnotation=PEW PEW PEW
        PWND
        PWND\n]9;

        haxx0rd by [insert handle here][i"""

        output_string = """/en-US/app/launcher/home/injection/%1B]8;;http://evil.com/%1B]1337;AddAnnotation=PEW PEW PEW
        PWND
        PWND\n%1B]9;

        haxx0rd by [insert handle here]%1B[i"""

        self.assertEqual(encode_ansi_escape_start(string), output_string)

    def test_encode_ansi_escape_start_exception(self):
        self.assertEqual(encode_ansi_escape_start(True), True)
...
if __name__ == '__main__':
    ...
    make_url_suite = unittest.TestLoader().loadTestsFromTestCase(MakeURLSuite)
    unittest.TextTestRunner(verbosity=2).run(make_url_suite)

    suite = unittest.TestLoader().loadTestsFromTestCase(EncodeAnsiEscapeStart)
    unittest.TextTestRunner(verbosity=2).run(suite)
