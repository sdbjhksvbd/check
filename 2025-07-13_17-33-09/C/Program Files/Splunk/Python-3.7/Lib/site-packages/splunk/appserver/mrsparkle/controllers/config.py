...
class ConfigController(BaseController):
    ...
    def does_host_match_header(
        self,
        origin_header,
        host_header,
    ):
        parsed_origin = urllib_parse.urlsplit(origin_header)
        parsed_host = urllib_parse.urlsplit('//' + host_header)
        def hosts_match():
            origin_hostname = parsed_origin.hostname
            host_hostname = parsed_host.hostname
            return origin_hostname != '' and origin_hostname == host_hostname
        def ports_match():
            origin_port = parsed_origin.port
            if not origin_port:
                # get default port
                origin_port_dict = { 'https': 443, 'http': 80 }
                origin_scheme = parsed_origin.scheme
                origin_port = origin_scheme in origin_port_dict and origin_port_dict[origin_scheme]
            host_port = parsed_host.port or 80
            return origin_port == host_port
        return hosts_match() and ports_match()
    ...
    def index(self, autoload=False, namespace=None, asDict=False, embed=False, oid=None, **kwargs):
        ...
        if cherrypy.session.get('sessionKey'):
        ...
        if autoload:
            ...
            if self.does_host_match_header(origin_header, host_header):
