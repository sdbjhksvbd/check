...
def didYouMean(command):
    ...
    commands = ["search", "login", "logout", "start", "stop", "restart", "status", "add", "edit", "list", "import", "export", "clean", "summary", "enable", "disable", "display", "train", "test", "validate", "anonymize", "find", "help", "diag", "dispatch", "getconf", "install", "migrate", "package", "recover", "refresh", "reload", "remove", "set", "show", "spool", "upgrade", "bootstrap", "rolling-restart", "abort-rolling-restart"]
...
cList.addCmd(SplunkCmd("abort-rolling-restart", "cluster-peers", None, None))
              ...
              if command in run_local or '%s:%s' % (command, subCmd) in run_local or '*:%s' % (subCmd) in run_local:
                 ...
                 try:
                   ...
                   if comm.isWindows:
                     ...
                     logger.warning("Failed cli cmd %s" % command)
                     if comm.debugMode:
