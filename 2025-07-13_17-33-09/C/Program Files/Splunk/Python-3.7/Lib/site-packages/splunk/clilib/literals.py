...
CLUSTER_LONG="""
        ...
        robust to single node failure. A cluster would typically have a single cluster manager server and several peer servers. When one
        ...
        Parameters:
                 ...
                 mode                    Controls the mode of the current instance in the cluster
                                         Acceptable values for which include [manager|peer|searchhead|disabled]
                 cxn_timeout             Low level timeout, in seconds, for establishing connection between cluster nodes
                                          (can be configured only at splunk instances configured as the cluster manager)
                 ...
                 replication_factor      Controls the number of copies of each bucket maintained in the cluster
                                          (can be configured only at the cluster manager)
                 ...
                 heartbeat_timeout       Controls the hearbeat for the cluster
                                          (can be configured only at splunk instances configured as the cluster manager)
                 restart_timeout         Controls the amount of time the cluster manager waits for the peer to readd itself after a restart
                                          (can be configured at the cluster manager)
                 replication_port        Controls the port dedicated for replication data, at the peers.
                                          (must be configured at the peer)
                 manager_uri             Controls the URI of the cluster manager to which this peer node connects
                 ...
                 max_peer_rep_load       Controls the max number of concurrent replications that peer can take
                                           part in as target
                 secret                  Controls the secret key between the cluster manager and the peer. Needed if the secret key
                                          is configured at the cluster manager. Appears in the server.conf as pass4SymmKey
                 use_batch_mask_changes  Controls how bucket mask changes are processed (In batch or individually) in a cluster.
                                          (can be configured only at the cluster manager)

        Syntax:

        Commands that can be used on the cluster manager to view and edit various clustering configurations:

                  [list|edit] cluster-config

                  list [manager-info|cluster-generation|cluster-peers|cluster-buckets]
         ...
         Commands that can be used on the cluster peer to view and edit various clustering configurations:
        ...
        Examples:
          ...
          (For Cluster Searchhead)
                    ...
                    ./splunk edit cluster-manager
                    ./splunk list cluster-manager
                    ./splunk remove cluster-manager
...
OFFLINE_PEER="""
          ...
          Used to shutdown the peer in a way that doesn't affect existing searches. The cluster manager rearranges the primary peers for buckets, and actually
     ...
     Flags:
        ...
        enforce-counts  If this flag is used, the cluster is completely fixed up before this peer is taken down.
                        ...
                        Without this flag, the cluster manager will simple rearrange the primaries and timeout after 5 minutes(by default),
                        The amount of time the cluster manager waits for the peer is configurable using the "restart_timeout" parameter using
...
PACKAGE_LONG="""
     ...
     Syntax:
        ...
        package app [-merge-local-meta <t/f>] [-exclude-local-meta <t/f>]   packages the app and returns its uri
     ...
     Optional Parameters:
        ...
        -merge-local-meta       value   merges local.meta with default.meta file.
                                        Choices are: t/f, true/false, 0/1. Defaults to f.

        -exclude-local-meta     value   excludes local.meta from the app package.
                                        Choices are: t/f, true/false, 0/1. Defaults to f.
     ...
     Examples:
       ...
       ./splunk package app stubby -merge-local-meta false

       ./splunk package app stubby -exclude-local-meta t
