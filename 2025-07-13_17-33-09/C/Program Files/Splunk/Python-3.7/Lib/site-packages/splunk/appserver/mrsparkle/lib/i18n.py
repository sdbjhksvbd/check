...
INTERNAL_APPS = ['gettingstarted', 'launcher', 'learned', 'legacy', 'sample_app', 'search', 'splunkdeploymentmonitor', 'splunkforwarder', 'splunklightforwarder', 'user-prefs', 'splunk_datapreview', 'dmc', 'splunk_instrumentation', 'splunk_gdi']

def get_ISO8609_MICROTIME(tzOffset=None):
    if tzOffset is None:
        tzOffset = format_local_tzoffset()
    return '%Y-%m-%dT%H:%M:%S.%Q' + tzOffset
...
def find_trans_keys_for_file_direct(filename):
    ...
    result = []
    try:
        f = open(potfile, 'r', encoding="utf-8")
    except IOError:
        if app_name:
...
def find_trans_keys_for_file(filename):
    ...
    if filename.startswith(app_prefix):
        ...
        app_name = filename[len(app_prefix):].split(os.path.sep, 2)[1]
        # SPL-240674. Ignore windows app names with ':'.
        if os.name == 'nt' and re.compile(r":").search(app_name):
            return []
...
def generate_js(filename, locale=None):
    """Generate the JS block that gets prepended to the .js file.  Also used to help generate the primary modules.js file"""
    if not locale:
        locale = cherrypy.request.lang
...
def extract_xml(fileobj, keywords, comment_tags, options):
    ...
    root = et.parse(fileobj, parser=parser)
    root = root.getroot()
...
def extract_nav(fileobj, keywords, comment_tags, options):
    ...
    nav = et.fromstring(fileobj.read(), parser=parser)
