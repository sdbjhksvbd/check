...
def _sanitize_non_internal_routes(route_file, route_class):
    """
    Verifies a file name related to a @route(...) is coming directly from Splunk
    core. Apps could theoretically define routes in private apps which
    could contain PII. Since we collect data on usage of routes in CherryPy, it
    is important that we only send data related to known core routes. If the file
    is not internal, return None for the file and class.

    @type  route_file: string
    @param route_file: The file path related to a route.

    @type  route_class: string
    @param route_class: The class related to a route.

    @rtype: tuple(route_file<str|None>, route_class<str|None>)
    @return: A tuple containing the route's file and class. If the file is not
             internal, return None for the file and class.
    """
    if 'splunk/appserver/mrsparkle/' not in route_file:
        return (None, None)
    rel_route_file = route_file.split('splunk/appserver/mrsparkle/')[1]
    return (rel_route_file, route_class)
...
def route(route=None, methods=None, leave_exposed=False):
    ...
    def decorator(fn):
        ...
        calling_class = calling_frame[3]
        (calling_file, calling_class) = _sanitize_non_internal_routes(calling_file, calling_class)
        fn.routes.append(Route(fn, route, defaults, methods, calling_file, calling_class))
        if leave_exposed:
...
class Route(object):
    ...
    def __init__(self, target, route_def, defaults={}, methods=None, calling_file=None, calling_class=None):
        ...
        if routes:
            ...
            def default(self, *path, **kw):
                ...
                for route in self.routes:
                    ...
                    if match is False:
                        continue
                    post_telemetry_metric({
                        'type': 'route.load',
                        'data': {
                            'route': route.route_str,
                            'file_path': route.calling_file,
                            'class': route.calling_class
                        }
                    })
