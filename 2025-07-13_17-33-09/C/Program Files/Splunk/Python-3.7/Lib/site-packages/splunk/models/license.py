...
class Pool(SplunkAppObjModel):
    ...
    peers               = ListField()
    peers_usage_bytes   = DictField(is_mutable=False)
    slaves              = ListField()
...
class SelfConfig(SplunkAppObjModel):
    ...
    resource = 'licenser/localpeer'
    resource_default = 'licenser/localpeer/license'

    connection_timeout                  = IntField(is_mutable=False)
    guid                                = ListField(is_mutable=False)
    features                            = DictField(is_mutable=False)
    last_manager_contact_attempt_time   = EpochField(is_mutable=False)
    last_manager_contact_success_time   = EpochField(is_mutable=False)
    ...
    manager_guid                        = Field(is_mutable=False)
    manager_uri                         = Field()
    master_guid                         = Field(is_mutable=False)
    master_uri                          = Field()
    receive_timeout                     = IntField(is_mutable=False)
    send_timeout                        = IntField(is_mutable=False)
    peer_name                           = Field(api_name='peer_id', is_mutable=False)
    peer_label                          = Field(is_mutable=False)
...
class Message(SplunkAppObjModel):
    ...
    peer_name       = Field(api_name='peer_id')
