...
def initVersionInfo(force=False, sessionKey=None):
    ...
    try:
        ...
        logger.info('Successfully retrieved serverInfo from splunkd')
        ...
        isConverged = splunk.util.normalizeBoolean(serverInfo.get('isConverged'))
        ...
        success = False
        isSessionKeyDefined = sessionKey is not None
        logger.error('Unable to read in product version information; isSessionKeyDefined=%s error=%s' % (isSessionKeyDefined, e))
    ...
    cherrypy.config['isConverged'] = isConverged

    if success:
        cherrypy.config['server_info_fetch'] = success
    elif 'server_info_fetch' in cherrypy.config:
        del cherrypy.config['server_info_fetch']

    logger.info(
        'Splunk appserver version=%(version_label)s build=%(build_number)s isFree=%(is_free_license)s isTrial=%(is_trial_license)s productType=%(product_type)s instanceType=%(instance_type)s' % cherrypy.config
    )
