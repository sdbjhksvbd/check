...
def verifyName(certDict, subjectAltNamesToCheck=None,
    ...
    if commonNamesToCheck:
        ...
        # If the common name is in the allow list of common names, then the CN
        # has passed
...
class SSLHelper(object):
    ...
    def createSSLContext(self,
        ...
        """
            ...
            Arguments:
                ...
                allowed host names to compare against a certificate's Common
                ...
                allowed host names to compare against a certificate's
                ...
                validateServerName (boolean) - sets SSLContext.check_hostname.
                If SSLContext.check_hostname is set to true, the Python 3 ssl
                library automatically sets verify_mode to CERT_REQUIRED.

                validateCRL (boolean) - enable CRL validation by setting
                ssl.VERIFY_CRL_CHECK_CHAIN flag.
        ...
        if self.isCCMode() or validateCRL:
        ...
        ctx.check_hostname = validateServerName
    ...
    def createSSLContextFromSettings(self, sslConfJSON=None,
        ...
        return self.createSSLContext(sslVersions=sslSettings['sslVersions'],
                                     ...
                                     validateServerName=sslSettings[
                                         'validateServerName'],
                                     validateCRL=sslSettings[
                                         'validateCRL'])
    ...
    def getSSLConfSettings(self, sslConfJSON, serverConfJSON, isClientContext=True):
        ...
        settings['validateServerName'] = False
        settings['validateCRL'] = False
        if settings['validatePeerCert'] == VALIDATE_SERVER:
            validateServerName = sslConfJSON.get('sslVerifyServerName')
            if validateServerName != None and validateServerName.lower() == 'true':
                settings['validateServerName'] = True
            certStatusValidation = sslConfJSON.get('certificateStatusValidationMethod')
            if certStatusValidation != None and certStatusValidation.lower() == 'crl':
                settings['validateCRL'] = True
