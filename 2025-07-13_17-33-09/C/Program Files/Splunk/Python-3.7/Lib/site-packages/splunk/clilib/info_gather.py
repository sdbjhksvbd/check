...
RULE 1: DO NOT MODIFY DIAG WITHOUT COMMUNICATION!
        Communicate with the diag maintainer
...
PYTHON_SSL_CONFIG_STANZA = 'pythonSslClientConfig'
...
def system_info_write(system_info, output):
    # python3 safe way to use system_info
    if sys.version_info >= (3, 0) and isinstance(output, bytes):
        output = output.decode(errors='replace')
    return system_info.write(output)
...
def is_loopback(family, host):
    # use ipaddress lib for Python 3
    assert sys.version_info < (3, 0)

    loopback_checker = {
        socket.AF_INET: lambda x: struct.unpack('!I', socket.inet_aton(x))[0] >> (32-8) == 127,
        socket.AF_INET6: lambda x: x == '::1' or x == '0:0:0:0:0:0:0:1'
    }
    return loopback_checker[family](host)

def isLocalhostSplunkd(url):
    try:
        mgmt_host_port = splunk.clilib.cli_common.getWebConfKeyValue('mgmtHostPort')
        mgmt_port = int(mgmt_host_port.rsplit(':', 1)[1])
    except Exception as e:
        msg = 'Failed to get Splunkd management port from web.conf, error="{}"'
        logger.error(msg.format(e))
        mgmt_port = 8089

    logger.debug("Localhost Splunkd management port={}".format(mgmt_port))
    try:
        parsed_url = urllib_parse.urlparse(url)
        addrs = socket.getaddrinfo(
            parsed_url.hostname, parsed_url.port, 0, 0, socket.IPPROTO_TCP)
        for family, _, _, _, sockaddr in addrs:
            host = sockaddr[0]
            # ipaddress lib is only available in Python 3
            if sys.version_info >= (3, 7):
                ip = ipaddress.ip_address(host)
                if ip.is_loopback and parsed_url.port == mgmt_port:
                    logger.debug("{} is the localhost Splunkd".format(url))
                    return True
            else:
                if is_loopback(family, host) and parsed_url.port == mgmt_port:
                    logger.debug("{} is the localhost Splunkd".format(url))
                    return True

        logger.debug("{} is not the localhost Splunkd".format(url))
        return False
    except Exception as e:
        msg = 'Unable to determine if {} is the localhost Splunkd, error="{}"'
        logger.error(msg.format(url, e))
        return False

def getSSLConfigStanzas():
    server_conf = get_server_conf()
    sslconfig_stanza = server_conf.get(SSL_CONFIG_STANZA, {})
    pysslconfig_stanza = server_conf.get(PYTHON_SSL_CONFIG_STANZA, {})
    merged_stanza = dict()
    merged_stanza.update(sslconfig_stanza)
    merged_stanza.update(pysslconfig_stanza)
    if 'sslRootCAPath' not in sslconfig_stanza:
        sslconfig_stanza['sslRootCAPath'] = sslconfig_stanza.get('caCertFile')
    return merged_stanza, sslconfig_stanza

def getSSLContext(url=None):
    """
    ...
    try:
        ...
        # SPL-223115: disable cert validation when connecting to localhost splunkd
        if isLocalhostSplunkd(url):
            return ssl._create_unverified_context()

        pysslconfig_stanza, sslconfig_stanza = getSSLConfigStanzas()
        sslHelper = splunk.ssl_context.SSLHelper()
        return sslHelper.createSSLContextFromSettings(
            sslConfJSON=pysslconfig_stanza,
...
def ping(protocol_host_port, cm=False):
    ...
    try:
        try:
            urllib_request.urlopen(req, timeout=5, context=getSSLContext(protocol_host_port)) # ignore return value
        except TypeError as e:
            urllib_request.urlopen(req, timeout=5) # ignore return value
    except urllib_error.HTTPError as error:
        msg = "When accessing /services for status, got error %s" % (error,)
...
def read_rest_endpoint(url, diag_path, cm=False):
    ...
    try:
        with tempfile.NamedTemporaryFile(mode="w", prefix='splunkdiag_rest', delete=False) as results_output_file:
            ...
            try:
                try:
                    rest_f = urllib_request.urlopen(req, timeout=5, context=getSSLContext(url))
                except TypeError as e:
...
def interactive_restsession_setup(protocol_host_port, clustering):
    ...
    if not clustering and not local_splunkd_protocolhostport:
        ...
        for i in range(password_retries):
            ...
            try:
                try:
                    resp = urllib_request.urlopen(url, params, timeout=5, context=getSSLContext(url))
                except TypeError as e:
...
def splunkVersion():
    ...
    if output:
        system_info_write(system_info, output)
...
LOG_FILE_REGEX = re.compile('^.*\\.log(\\.\\d+)?$')

def get_non_log_paths(dir, file_list):
    return list(filter(lambda file : not LOG_FILE_REGEX.match(file), file_list))

def try_copy_sub_dir_to_diag(dir_path, diag_path, ignore_files):
    try:
        add_dir_to_diag(dir_path, diag_path, ignore_files)
    except shutil.Error as copy_errors:
        msg = "Some problems were encountered copying '%s' files:\n%s"
        logger.warn(msg % (dir_path, str(copy_errors)))
...
def copy_dispatch_dir():
     ...
     for job in os.listdir(dispatch_dir):
         try:
             ...
             # copy remote_logs and files in the job's dir, skip all the .gz stuff
             for f in os.listdir(job_dir):
                src_file = os.path.join(job_dir, f)
                # don't capture the customer's data, or zero byte status indicator files.
                if f.endswith('.gz') or f.startswith('results') or f.endswith(".token") or f.endswith('.zst') or f.startswith('lookup'):
                    continue
                if not os.path.isdir(src_file):
                    add_file_to_diag(src_file, os.path.join("dispatch", job, f))
                elif f in dirs_to_collect:
                    try_copy_sub_dir_to_diag(src_file, os.path.join("dispatch", job, f), get_non_log_paths)
         except OSError as e:
             # report fail, but continue along
...
def warn_if_logcfg_has_odd_paths():
    ...
    client_events_path = os.sep.join(["${SPLUNK_HOME}", "var", "log", "client_events"])
        ...
        with open(filename) as f:
            for line in f:
                if not "fileName=" in line:
                    continue
                if log_path not in line and introspection_path not in line and watchdog_path not in line and client_events_path not in line:
...
def copy_logs(options):
    ...
    try:
        client_events_logdir = os.path.join(SPLUNK_HOME, 'var', 'log', 'client_events')
        if not os.path.isdir(client_events_logdir):
            logger.warn("No client_events logdir found, continuing...")
            return
        add_dir_to_diag(client_events_logdir, "client_events", ignore= size_filt)
    except shutil.Error as copy_errors:
        msg = 'Problems copying client_events logs:\n\t' + str(copy_errors)
        logger.warn(msg)
...
class FilteredAuditLog(FilteredStream):
    ...
    def log_stats(self, filename):
        ...
        if not joined_bits:
            auxlogger.info("While filtering %s, no rules matched.",
                           rootless_filename)
        else:
            auxlogger.info("While filtering %s, rule match counts: %s",
...
def _setup_filters_passwords():
    server_conf = get_server_conf()
    # encrypt_fields was moved from shclustering to general, but try first to read from shclustering in case confs were not yet migrated
    shcluster_stanza = server_conf.get('shclustering')
    field_encrypt_info = None
    if shcluster_stanza:
        field_encrypt_info = shcluster_stanza.get('encrypt_fields')
    if not field_encrypt_info:
        general_stanza = server_conf.get('general')
        if not general_stanza:
            msg = "Could not find general stanza in server.conf, will not be able to auto-redact passwords"
            logger.error(msg)
            return
        field_encrypt_info = general_stanza.get('encrypt_fields')
        if not field_encrypt_info:
            msg = "Cannot read 'encrypt_fields' setting from general stanza in server.conf, will not be able to auto-redact passwords"
            logger.error(msg)
            return
...
def get_dir_listing(dir_path, recursive=False):
    ...
    if sys.version_info < (3, 0) and isinstance(cmd, unicode):
        ...
        cmd_bytes =  cmd
    if sys.version_info >= (3, 0) and isinstance(cmd_output, str):
...
def deleteTree(path):
    ...
    def handle_readonly_filedir_errors(func_called, path, exc):
        ...
        if func_called == os.rmdir:
            os.rmdir(path)
        elif func_called == os.remove or func_called == os.unlink:
            os.remove(path)
        else:
            raise
...
def local_getopt(file_options, cmd_argv=sys.argv):
    ...
    upload_group.add_option("--request_timeout", type="int", metavar="request_timeout",
                            help="Optional set the timeout in seconds")
...
def discover_apps():
    ...
    slave_app_dir = os.path.join(get_splunk_etc(), splunk.clilib.cli_common.getAppDir())
    ...
    if plain_v_slave:
        logger.warn(" Apps exist in both the apps and the peer-apps dir: " +
                     ",".join(plain_v_slave))
        warned = True
...
def main():
    # We want all logged messages to access our custom in-memory StringIO buffer;
...
def remote_diag_status(options):
    ...
    try:
        ...
        try:
            diagstatus_f = urllib_request.urlopen(diagstatus_req, context=getSSLContext(diagstatus_url))
        except TypeError as e:
...
def remote_diag(options, log_buffer):
    ...
    with open(our_tarname, "wb") as out_f:
        ...
        try:
            diagdata_f = urllib_request.urlopen(diagdata_req, context=getSSLContext(diagdata_url))
        except TypeError as e:
...
class FileWrappingProgressBar(object):
    ...
    def __init__(self, readable_obj, bar_size=59, read_max=None, options=None):
            ...
            def unix_getkey():
                ...
                try:
                    ...
                    fcntl.fcntl(stdin_fd, fcntl.F_SETFL, saved_flags |
                                os.O_NONBLOCK)
                    try:
                        # Encountering this bug in python3, but
...
def upload_slice_to_splunkcom(path, options, slicebegin=None, sliceend=None,
    ...
    proxyFactory = FileWrappingProgressBar
    try:
        proxyFactory = options.proxyFactory
    except AttributeError:
        pass

    fileproxy = proxyFactory(f, read_max=read_max, options=options)
...
def make_splunkcom_request(url, options, params={}, method='GET', body=None,
    ...
    auth_encoded = str(base64.b64encode(auth_string).strip().decode())

    req.add_header("Authorization", "Basic %s" % auth_encoded)
    try:
        timeout = vars(options).get("request_timeout", 60)
        rest_f = urllib_request.urlopen(req, timeout=timeout)
...
def fetch_case_list(options):
    ...
    try:
        ...
        logger.error("Case listing api '%s' did not return a json document.", url)
        return []
