...
def log_request_headers(debug=False):
    ...
    h = []
    redacted_cookie = ''
    """SPL-194357 SPL-209785: prevent the full values for cookie, x-splunkd,
    x-splunk-form-key in web_service.log from the cherrpy error log. Redact entire or all
    but the last few characters in the string.
   
    All values will either be '[REDACTED]' or will print with the last 4 characters with the
    the prefix '[REDACTED]' to indicate that these are intentionally obscured
    in web_service.log (using COOKIE as example below):

    COOKIE: cookie/token=[REDACTED]<last 4 characters>
    
    To manually verify that this is working, run the search query:
    'index=_internal sourcetype=splunk_web_service "Request Headers" | head 1'
    after receiving a 500 internal service request failure and observe the
    values under the 'COOKIE:' parameter.
    """
    for k, v in cherrypy.serving.request.header_list:
        if k in ['X-SPLUNKD','X-SPLUNK-FORM-KEY']:
            h.append('  %s: [REDACTED] ' % k)
        elif k == 'COOKIE':
            c = v.split()
            for s in c:
                redacted_cookie += (s[:s.find('=')] + '=[REDACTED]' + s[-5:] + ' ')
            h.append('  %s: ' % (k) + redacted_cookie)
        else: 
            h.append('  %s: %s' % (k, v))
    cherrypy.log('\nRequest Headers:\n' + '\n'.join(h), 'HTTP')
