...
class InternalFunctionsTest(unittest.TestCase):
    ...
    def test_format_optdict(self):
        ...
        opts = {'Î±Î²Î³': True, 'Ã¡': False}
        orig_opts = opts.copy()
        # check if giving unicode keys is fine
        check_against(ttk._format_optdict(opts), {'-Î±Î²Î³': True, '-Ã¡': False})
        ...
        # ignore an option
        amount_opts = len(ttk._format_optdict(opts, ignore=('Ã¡'))) / 2
        self.assertEqual(amount_opts, len(opts) - 1)

        # ignore non-existing options
        amount_opts = len(ttk._format_optdict(opts, ignore=('Ã¡', 'b'))) / 2
        self.assertEqual(amount_opts, len(opts) - 1)
    ...
    def test_format_mapdict(self):
        ...
        opts = {'Ã¼Ã±Ã­Ä‡Ã³dÃ¨': [('Ã¡', 'vÃ£l')]}
        result = ttk._format_mapdict(opts)
        self.assertEqual(result, ('-Ã¼Ã±Ã­Ä‡Ã³dÃ¨', 'Ã¡ vÃ£l'))
    ...
    def test_script_from_settings(self):
        ...
        configdict = {'Î±Î²Î³': True, 'Ã¡': False}
        self.assertTrue(
            ttk._script_from_settings({'name': {'configure': configdict}}))

        mapdict = {'Ã¼Ã±Ã­Ä‡Ã³dÃ¨': [('Ã¡', 'vÃ£l')]}
        self.assertTrue(
...
class TclObjsToPyTest(unittest.TestCase):
    ...
    def test_unicode(self):
        adict = {'opt': 'vÃ¤lÃºÃ¨'}
        self.assertEqual(ttk.tclobjs_to_py(adict), {'opt': 'vÃ¤lÃºÃ¨'})
    ...
    def test_multivalues(self):
        ...
        adict['opt'] = (MockStateSpec('a', 'b'), 'vÃ¡lÅ©Ã¨')
