define(
    ...
    function($, _, module, JobModel, ResultModel, Base, DataSummaryModal, time_utils, splunkUtil) {
        return Base.extend({
            ...
            render: function() {
                var wasSummaryButtonFocused = $("a.btn-data-summary").is(':focus');
                ...
                if(wasSummaryButtonFocused) {
                    this.$("a.btn-data-summary").focus();
                }
                return this;
            },
            template: '\
                <% if (showDataSummary) { %>\
                    <a href="#" data-test="data-summary-button" class="btn btn-data-summary"\
                        title="<%- _("View sources, source types and hosts.").t() %>"><%- _("Data Summary").t() %></a>\
                <% } %>\
            '
        });
    }
