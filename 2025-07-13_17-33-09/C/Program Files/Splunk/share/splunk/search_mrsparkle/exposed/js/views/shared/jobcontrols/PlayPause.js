define(
    ['jquery', 'underscore', 'module', 'views/Base', 'util/splunkd_utils', 'bootstrap.tooltip'],
    function($, _, module, Base, splunkd_utils /* bootstrap tooltip */) {
    return Base.extend({
        moduleId: module.id,
        ...
        render: function() {
            ...
            if(isPaused && currentMode !== 'unpause') {
                this.$el.data('mode', 'unpause').html('<i class="icon-play"></i><span class="hide-text" aria-hidden="true">' + _("Resume").t() + '</span>');
            ...
            if(!isPaused && currentMode !== 'pause') {
                this.$el.data('mode', 'pause').html('<i class="icon-pause" aria-hidden="true"></i><span class="hide-text" aria-hidden="true">' + _("Pause").t() + '</span>');
