        ...
        return Base.extend({
            ...
            initialize: function() {
                ...
                var isAlert = (this.options.documentType === ReportModel.DOCUMENT_TYPES.ALERT),
                    isStreaming = this.options.alertType === 'streaming',
                ...
                if (isStreaming) {
                    includeControls.push(
                        new SyntheticCheckboxControl({
                            modelAttribute: 'action.email.include.smaDefinition',
                            model: this.model.document.entry.content,
                            label: _('Streaming Definition').t()
                        })
                    );
                } else if (isAlert) {
                    includeControls.push(
                ...
                includeControls.push(
                    new SyntheticCheckboxControl({
                        modelAttribute: 'action.email.allow_empty_attachment',
                        model: this.model.document.entry.content,
                        label: _('Allow Empty Attachment').t()
                    })
                );
