define(
    ...
    return Base.extend({
        ...
        render: function() {
            var progress = _("Starting search point job...").t(),
            ...
            if (eventCount === 0) {
                ...
                } else if (modelToRender.entry.content.get("eventSearch") === '') {
                    isEvents = false;
            ...
            modelToRender.isTableNew = this.model.isTableNew;
            modelToRender.isTableEditing = this.model.isTableEditing;
        ...
        template: '\
            <% if (!model.isNew()) { %>\
                <% if (model.isDone()) { %>\
                    ...
                    <% if (isEvents) { %>\
                        <% if (model.isTableNew || model.isTableEditing) { %>\
                            <span class="bold"><%= splunkUtil.sprintf(i18n.ungettext("%s event", "%s events", eventCount), \'Previewing <span class="number">\' + eventCount + \'</span>\') %></span>\
                        <% } else { %>\
                            <span class="bold"><%= splunkUtil.sprintf(i18n.ungettext("%s event", "%s events", eventCount), \'<span class="number">\' + eventCount + \'</span>\') %></span>\
                        <% } %>\
                    <% } else { %>\
