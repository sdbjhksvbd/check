...
class WorkloadManagementPageContainer extends React.Component {
    constructor(props) {
        ...
        this.props.admissionRules.on('sync', this.updateAdmissionRulesTable);
        this.props.status.on('sync', this.updateStatus);
    }

    getDefaultState = () => ({
        title: _('Workload Management').t(),
        description: _(`Apply configurations for admission rules, workload pools, 
        workload placement & monitoring rules.`).t(),
        ...
        admissionRules: this.props.admissionRules.prepareAdmissionRules(),
        ...
        isAdmissionEnabled: this.props.policy.isEnabled(),
        eligibleToEnableAdmission: this.props.user.hasCapability('edit_workload_policy'),
        ...
        handleAdmissionEnableDisableClick: this.handleAdmissionEnableDisableClick,
        ...
        handleAdmissionModalOpen: this.handleAdmissionModalOpen,
    ...
    getMissingTablesMessage = () => {
        ...
        if (noPools && noRules) {
            missingTables = _('workload pools or workload rules are defined').t();
        } else if (noPools) {
            missingTables = _('workload pools are defined').t();
        } else if (noRules) {
            missingTables = _('workload rules are defined').t();
        }
        ...
        const status = _.isEmpty(missingTables) ? '' : splunkUtil.sprintf(_('No %s. ').t(), missingTables);
        const action = (_.isEmpty(addButtons) || _.isEmpty(newTables)) ? '' :
        splunkUtil.sprintf(_(' Click the %s to create new %s.').t(), addButtons, newTables);
    ...
    getDefaultRuleUpdateModalState = () => ({
        ...
        getAdmissionActions: this.props.admissionRules.getAdmissionActions(),
        workload_rule_type: '',
        changed: false,
        backendErrorMsg: '',
    })
    ...
    handleTabBarChange = (e, { selectedTabId }) => {
        if (selectedTabId === 'admission') {
            this.setState({
                tabBarState: 'admission',
            });
        } else if (selectedTabId === 'pools') {
            this.setState({
                tabBarState: 'pools',
            });
        } else {
    ...
    handleAdmissionEnableDisableClick = (e, { value }) => {
        if (value === true) {
            const modalTitle = _('Disabling Workload Management').t();
            this.props.enableDisableAdmission.disable().done(() => {
                this.props.policy.fetch().done(() => {
                    this.setState({
                        isAdmissionEnabled: this.props.policy.isEnabled(),
                    });
                    this.handleMessageModalOpen(
                        modalTitle,
                        _('Successfully disabled admission control').t(),
                        'success',
                        true,
                    );
                });
            }).fail((response) => {
                let modalErrorMsg = _('Error disabling admission control').t();
                if (response.responseJSON && response.responseJSON.messages &&
                    response.responseJSON.messages.length > 0) {
            ...
            const modalTitle = _('Enabling Admission Control').t();
            this.props.enableDisableAdmission.enable().done(() => {
                this.props.policy.fetch().done(() => {
                    this.setState({
                        isAdmissionEnabled: this.props.policy.isEnabled(),
                    });
                    if (this.props.policy.isEnabled()) {
                        this.handleMessageModalOpen(
                            modalTitle,
                            _('Successfully enabled admission control.').t(),
                            'success',
                            true,
                        );
                    } else {
                ...
                let modalErrorMsg = _('Error enabling admission control.').t();
                if (response.responseJSON && response.responseJSON.messages &&
                    response.responseJSON.messages.length > 0) {
                    const messageObj = response.responseJSON.messages[0];
                    modalErrorMsg = splunkUtil.sprintf(_('%s: %s').t(), messageObj.type, messageObj.text);
    ...
    handleCategoryUpdateModalTextChange = (e, { value, name }) => {
        if ((name === 'cpu_weight') && !_.isUndefined(value)) {
            const categoryName = this.state.categoryUpdateModalState.model === undefined ?
                '' : this.state.categoryUpdateModalState.model.entry.get('name');
            this.validateFields([{
                name,
                value,
            }, {
    ...
    handleRuleUpdateModalOpenEdit = (e, { value }) => {
        ...
        let admissionModalOpenConst = false;
        let ruleUpdateModalOpenConst = false;
        const obj = {
            model: ruleModel,
            edit: true,
            ...
            user_message: ruleModel.getUserMessage(),
        ...
        if (ruleModel.entry.content.get('action') === 'filter') {
            admissionModalOpenConst = true;
            obj.workload_rule_type = 'search_filter';
        } else {
            ruleUpdateModalOpenConst = true;
        }
        this.setState({
            admissionModalOpen: admissionModalOpenConst,
            ruleUpdateModalOpen: ruleUpdateModalOpenConst,
    ...
    handleAdmissionModalOpen = () => {
        const obj = {
            title: _('New Admission Rule').t(),
            workload_rule_type: 'search_filter',
            action: 'filter',
        };
        this.setState({
            admissionModalOpen: true,
            ruleUpdateModalState: _.extend(this.getDefaultRuleUpdateModalState(), obj),
        });
    }
    ...
    validateRuleFields = (fields, stateObj) => {
        ...
        _.each(fields, (field) => {
            ...
            switch (field.name) {
                ...
                case 'predicate':
                    ...
                    break;
                case 'user_message':
                    if (!_.isEmpty(field.value) && field.value.length > 140) {
                        obj.userMessageErrorMsg = _('User message cannot exceed 140 characters.').t();
                    } else {
                        delete obj.userMessageErrorMsg;
                    }
                    break;
                case 'order':
                    if (obj.workload_rule_type !== 'search_filter' &&
                        (_.isEmpty(field.value.match(/^[0-9]+$/)) || field.value === '0')) {
                ...
                case 'workload_pool':
                    if (obj.workload_rule_type !== 'search_filter' &&
                        (_.isEmpty(field.value) && (obj.action === 'move' || obj.action === ''))) {
        ...
        obj.userMessageError = !_.isEmpty(obj.userMessageErrorMsg);
    ...
    handleRuleUpdateModalSubmit = () => {
        ...
        this.validateRuleFields([{
            ...
            name: 'user_message',
            value: this.state.ruleUpdateModalState.user_message,
        }, {
            name: 'action',
        ...
        const data = {
            ...
            user_message: this.state.ruleUpdateModalState.user_message,
        ...
        let ruleCollection = this.props.rules;
        if (this.state.ruleUpdateModalState.workload_rule_type &&
            this.state.ruleUpdateModalState.workload_rule_type === 'search_filter') {
            data.workload_rule_type = 'search_filter';
            ruleCollection = this.props.admissionRules;
        }
        ...
        ruleCollection.updateRule(model, data).done(() => {
            this.props.admissionRules.fetch({
                data: {
                    workload_rule_type: 'search_filter',
                },
            });
    ...
    handleRuleDeleteModalOpen = (e, { value }) => {
        ...
        if (obj.ruleModel.entry.content.get('action') === 'filter') {
            obj.ruleModel.entry.content.set({ workload_rule_type: 'search_filter' });
        }
        this.setState({
...
WorkloadManagementPageContainer.propTypes = {
    ...
    admissionRules: PropTypes.shape({
        models: PropTypes.arrayOf(PropTypes.shape({})),
        on: PropTypes.func,
        fetch: PropTypes.func,
        prepareAdmissionRules: PropTypes.func,
        getAdmissionActions: PropTypes.func,
        updateRule: PropTypes.func,
        deleteRule: PropTypes.func,
    }).isRequired,
    policy: PropTypes.shape({
        isAdmissionEnabled: PropTypes.func,
    ...
    enableDisableAdmission: PropTypes.instanceOf(Backbone.Model).isRequired,
