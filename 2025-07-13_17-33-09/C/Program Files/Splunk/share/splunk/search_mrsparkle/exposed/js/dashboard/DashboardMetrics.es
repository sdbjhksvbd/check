...
import { getDefaultSimpleXMLVersion } from 'util/dashboard_utils';
/**
 *
...
export function sendDashboardMetrics({ model, state, deferreds, managers,
  VisualizationRegistry, SearchManager, isValidPivotSearch }) {
    ...
    Promise.all(_.values(deferreds)).then(() => {
        getAndSendData(model, state, managers,
  VisualizationRegistry, SearchManager, isValidPivotSearch);
    });
}

export function getAndSendData(model, state, managers,
  VisualizationRegistry, SearchManager, isValidPivotSearch) {
    const dataPromise = captureDashboardMetrics(model, state, managers,
        VisualizationRegistry, SearchManager, isValidPivotSearch);
    const dependencies = requirejs.getInternalDependencies();
    const enableRiskyCommandCheckDashboard = window.$C.ENABLE_RISKY_COMMAND_CHECK_DASHBOARD;

    return dataPromise.then((data) => {
        const obj = { ...data, dependencies, enableRiskyCommandCheckDashboard };
        window._splunk_metrics_events.push({ type: 'dashboard.load', data: obj });
    });
}
...
export function getDashboardAttrs(committedState, page, scheduledView, fieldsetState) {
    ...
    const version = settings.version || getDefaultSimpleXMLVersion();
