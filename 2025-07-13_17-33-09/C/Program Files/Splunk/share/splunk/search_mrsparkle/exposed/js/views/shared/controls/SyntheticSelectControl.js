...
function(
     ...
     * be set to the label. Defaults to false.
     * @param {boolean} [options.shouldTranslateSelectedItemLabel] if true the translation for the selected item label
     * will be fetched by the i18n utils. Defaults to true.
     */
    return Control.extend(/** @lends views.SyntheticSelectControl.prototype */{
        ...
        initialize: function() {
            var defaults = {
                ...
                shouldTranslateSelectedItemLabel: true
            };
        ...
        keydown: function (e) {
            ...
            if (e.keyCode === keyboard_utils.KEYS['DOWN_ARROW']) {
                ...
                if (e.shiftKey) {
                    ...
                    // [a11y] When Tab on the last element,
                    // close popdown and focus the next interactive element.
                    e = $.Event( 'keypress', { which: keyboard_utils.KEYS['TAB'] } );
                    this.children.popdown.hide();
                    $focused.trigger(e);
                    handled = true;
                }
        ...
        render: function() {
            ...
            if (selectedItem) {
                if (selectedItem.label || selectedItem.value) {
                    var selectedValueLabel = this.options.shouldTranslateSelectedItemLabel ?
                        _(selectedItem.label || selectedItem.value).t() :
                        selectedItem.label || selectedItem.value;
                    var formattedLabel = _(this.options.label).t() + " " + selectedValueLabel;
        ...
        template: '\
            ...
            <a class="dropdown-toggle <%- options.toggleClassName %>" aria-haspopup="true" href="#"\
             ...
             <% if (options.useLabelAsTitle) { %>title="<%- label %>"<% } %>>\
                ...
                <span class="link-label"><%- label %></span><span class="caret" aria-hidden="true"></span>\
            </a>\
