define(['jquery', 'underscore', 'module', 'views/shared/Modal','uri/route','views/shared/documentcontrols/dialogs/permissions_dialog/Master','views/dashboards/table/controls/SchedulePDF','models/services/ScheduledView'],
    ...
    return Modal.extend({
        ...
        render: function() {
            ...
            var isUDFDashboard = this.model.dashboard.isUDFDashboard();
            var message;
            if (isUDFDashboard) {
                message = _('You may now view your dashboard, change additional settings, or edit.').t();
            }
            else {
                message = _('You may now view your dashboard, change additional settings, or edit the panels.').t();
            }
            this.$(Modal.BODY_FORM_SELECTOR).append(_.template(this.messageTemplate, {
                dashboardLink: link,
                canChangePerms: canChangePerms,
                canSchedule: canSchedule,
                isUDFDashboard: isUDFDashboard,
                message: message
            }));
        ...
        messageTemplate:'<p><%- message %></p>' +
                        '<p><% if(canChangePerms || canSchedule){ %>' +
                                ...
                                '<ul>' +
                                    ...
                                    '<% if(canSchedule && !isUDFDashboard) { %><li><a href="#" class="schedule-pdf"><%- _("Schedule PDF Delivery").t() %><% } %></a></li>' +
                                '</ul>' +
                            '<% } %>' +
                        '</p>'
    });
