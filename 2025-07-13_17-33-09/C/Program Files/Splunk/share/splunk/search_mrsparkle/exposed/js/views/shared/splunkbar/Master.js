...
define([
    ...
    'views/shared/global_banner/ReactAdapterMaster',
...
function(
    ...
    var View = BaseView.extend({
        ...
        initialize: function() {
            ...
            this.globalBannerDfd = $.Deferred();
            this.cntRetries = 0;
            if (this.model.user) {
                if (this.model.user.entry.content.has('realname')) {
            ...
            if (this.model.globalBanner) {
                if (this.model.globalBanner.entry && this.model.globalBanner.entry.content &&
                    this.model.globalBanner.entry.content.has('global_banner.message')) {
                    this.globalBannerDfd.resolve();
                } else {
                    this.model.globalBanner.on('sync', function() {
                        this.globalBannerDfd.resolve();
                    }, this);
                }
            }
        ...
        renderGlobalBanner: function() {
            this.children.globalBanner = new GlobalBannerReactAdapter({
                model: {
                    globalBanner: this.model.globalBanner
                }
            });
            this.children.globalBanner.render().prependTo(this.$el);
        },
        render: function() {
                ...
                $.when(this.userDfd, this.appsDfd).then(function() {
                    ...
                    $.when(this.webConfDfd, this.appsVisibleDfd).then(function() {
                        this.isFreeDfd.fail(function() {
                            ...
                            if (this.model.user.canListHealth() || this.model.user.canListHealthSubset()) {
                    ...
                    $.when(this.webConfDfd, this.globalBannerDfd).done(function() {
        ...
        restartMessagePolling: function(interval) {
            this.collection.messages.stopPolling();
            this.collection.messages.startPolling({delay: interval, uiInactivity: true, stopOnError: false});
        },
    ...
    },
    {
        create: function(options){
            ...
            if (!options.model.globalBanner) {
                options.model.globalBanner = new GlobalBannerModel();
                options.model.globalBanner.fetch();
            }
