...
function(
    ...
    return Base.extend({
        ...
        listElementPartial: _.template('\
            <li>\
                <a href="#" data-earliest="<%- model.entry.content.get("earliest_time") || "" %>"\
                    data-latest="<%- model.entry.content.get("latest_time") || "" %>"\
                    aria-label="<%- _(timePresets).t() + " " + _(model.entry.content.get("label")).t() %>" >\
                        <%- _(model.entry.content.get("label")).t() %>\
                </a>\
            </li>\
        '),
        template: '\
            <% if (realTimePresets && realTimePresets.length) { %>\
                <ul class="unstyled presets-group">\
                    ...
                    <% _.each(realTimePresets, function(model) { %>\
                        <%= listElementPartial({model: model, timePresets: "Real-time"}) %>\
                    <% }); %>\
                </ul>\
            ...
            <% if (hasPeriodPresets && hasLastPresets) { %>\
                    <ul class="unstyled presets-group">\
                        ...
                        <% _.each(periodPresets, function(model) { %>\
                            <%= listElementPartial({model: model, timePresets: "Relative"}) %>\
                        <% }); %>\
                    </ul>\
                    <ul class="unstyled presets-group">\
                ...
                <ul class="unstyled presets-group">\
                    ...
                    <% _.each(otherPresets, function(model) { %>\
                        <% if (!(isAllTime(model) && !options.showAllTime)) { %>\
                            <%= listElementPartial({model: model, timePresets: "Other"}) %>\
                        <% } %>\
                    <% }); %>\
                </ul>\
            <% } %>\
