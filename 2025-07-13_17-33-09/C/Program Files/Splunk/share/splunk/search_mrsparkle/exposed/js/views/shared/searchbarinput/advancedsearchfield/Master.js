define(
    ...
    function($, _, module, Ace, ModeSpl, Base, AutoCompletion, UserModel, SearchHistoryModel, ResultsModel, keyboard_utils, general_utils, Printer, userAgent, splunkUtils, rum, css) {
        return Base.extend({
            moduleId: module.id,
            className: 'search-field-wrapper',
            initialize: function() {
                Base.prototype.initialize.apply(this, arguments);
                this.model.splCommandHistory = {
                    searchHistoryResults: new ResultsModel(),
                    searchHistoryJob: new SearchHistoryModel(),
                    currentSearch: "",
                ...
                this.firstUseOfHistoryCommand = true;
            ...
            startListening: function() {
                ...
                this.listenTo(this.model.splCommandHistory.searchHistoryJob, 'done', this.fetchSearchHistoryResults);
                this.listenTo(this.model.splCommandHistory.searchHistoryResults, 'sync', this.updateCommands);
            },
            ...
            setupEditorListeners: function() {
                ...
                var historyUpCommand = {
                    name: "historyUp",
                    bindKey: {win: 'Alt-p', mac: 'Ctrl-p'},
                    exec: this.navigateHistoryUp.bind(this),
                    readOnly: false,
                };

                var historyDownCommand = {
                    name: "historyDown",
                    bindKey: {win: 'Alt-n', mac: 'Ctrl-n'},
                    exec: this.navigateHistoryDown.bind(this),
                    readOnly: false,
                };

                this.editor.commands.addCommand(historyUpCommand);
                this.editor.commands.addCommand(historyDownCommand);
            ...
            _onFormSubmit: function(options) {
                ...
                if (this.options.submitEmptyString || search) {
                    ...
                    if (this.model.splCommandHistory.alreadyFetched) {
                        this.model.splCommandHistory.commands.unshift(this.getSearchFieldValue());  //push only if commands are fetched i.e. user has executed Ctr+P at some point.
                    }
                    this.firstUseOfHistoryCommand = true;
                }
            },
            ...
            fetchSearchHistoryResults: function() {        
                var id = this.model.splCommandHistory.searchHistoryJob.entry.links.get('results');
                if (id) {
                    this.model.splCommandHistory.searchHistoryResults.set({id: id});
                    this.model.splCommandHistory.searchHistoryResults.safeFetch({
                        data: {
                            offset: 0,
                            count: 100,
                            search: '| search'
            ...
            updateCommands: function (searchHistory) {              
                if(!_.isEmpty(searchHistory.results.models)) {
                    this.model.splCommandHistory.commands = searchHistory.results.models.map(function (element) {
                        // sometimes searchQuery is of the format "search index="_audit"
                        // slicing off the "search\s"
                        var str = "search";
                        var searchFromRest = !_.isEmpty(element.get(str)) ? element.get(str)[0] : '';
                        return searchFromRest.indexOf(str) !== 0 ? searchFromRest : searchFromRest.slice(str.length).trim();                       
                    }.bind(this));
                }
                this.setNextCommand();
            },
            ...
            startSearchHistoryJob: function() {
                if (!this.model.splCommandHistory.searchHistoryJob.isNew()) {
                    this.clearSearchHistory();
                }
                $.when(this.model.splCommandHistory.searchHistoryJob.startJob(this.model.application,
                    {data: {
                        earliest_time: '-7d@d',
                        provenance: 'UI:Search'
                    }})).then(function() {                 
                    this.model.splCommandHistory.searchHistoryJob.startPolling();
                }.bind(this));
            },

            clearSearchHistory: function() {
                this.model.splCommandHistory.searchHistoryJob.clear();
                this.model.splCommandHistory.searchHistoryResults.fetchAbort();
                this.model.splCommandHistory.searchHistoryResults.clear();
            }, 

            navigateHistoryUp: function() {
                rum.mark('SPL command history key used', { timeout: 50 });
                if(!this.model.splCommandHistory.alreadyFetched) {
                    this.startSearchHistoryJob();
                    this.model.splCommandHistory.alreadyFetched = true;
                } else {
                    this.setNextCommand();
                }               
            },

            navigateHistoryDown: function() {
                rum.mark('SPL command history key used', { timeout: 50 });
                if(this.firstUseOfHistoryCommand) return;
                this.setPreviousCommand();
            },

            setNextCommand: function() {               
                var historyArray = this.model.splCommandHistory.commands;
                var currentIndex = this.model.splCommandHistory.currentIndex;

                if(_.isEmpty(historyArray) || currentIndex >= historyArray.length-1) return;

                if(this.firstUseOfHistoryCommand) {
                    this.model.splCommandHistory.currentSearch = this.getSearchFieldValue();
                    this.firstUseOfHistoryCommand = false;
                    currentIndex = -1;
                }                       
                this.setSearchString(historyArray[++currentIndex]);
                this.model.splCommandHistory.currentIndex = currentIndex;
            },

            setPreviousCommand: function() {
                var historyArray = this.model.splCommandHistory.commands;                        
                var currentIndex = this.model.splCommandHistory.currentIndex;                      
                if(!_.isEmpty(historyArray) && currentIndex > 0) {
                    this.setSearchString(historyArray[--currentIndex]);
                    this.model.splCommandHistory.currentIndex = currentIndex;
                } else {
                    currentIndex = -1;
                    this.setSearchString(this.model.splCommandHistory.currentSearch);
                    this.model.splCommandHistory.currentIndex = currentIndex;
                    this.firstUseOfHistoryCommand = true;
                }
            },
