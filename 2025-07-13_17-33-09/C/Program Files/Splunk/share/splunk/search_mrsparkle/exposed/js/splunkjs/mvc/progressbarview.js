        ...
        render: function() {
            ...
            if(this.model.messages.has('errors') || this.model.messages.has('warnings')) {
                if(!this.$error) {
                    ...
                    this.$error.find('a.error-indicator').attr('aria-label', _('Click to see the error details.').t());
                }
                this.$error.find('.error-list').html(this.errorStatusTemplate(_.extend({
                    _:_,
                    errors: null,
                    warnings: null
                }, this.model.messages.toJSON())));
