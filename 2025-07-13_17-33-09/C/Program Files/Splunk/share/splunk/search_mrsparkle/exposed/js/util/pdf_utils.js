define(['underscore', 'jquery', 'splunk.util', 'util/console', 'models/services/configs/AlertAction', 'models/search/Job', 'splunk.config'], function(_, $, splunkUtil, console, AlertAction, Job, splunkConfig) {
    ...
    /**
     * Downloads the PDF version of a dashboard|report
     * @param xml the xml content of the dashboard
     ...
     */
    var downloadReportFromXML = function (xml, app, view, params, viewType) {
        var baseURL = splunkUtil.make_full_url("splunkd/__raw/services/pdfgen/render");
        var form = $('<form method="POST" target="_blank" rel="noopener noreferrer"></form>').attr('action', baseURL);
        ...
        $('<input/>').attr({ type: 'hidden', name: inputType, value: view}).appendTo(form);
