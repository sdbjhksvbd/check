define([
    ...
    return BaseController.extend({
        initialize: function(options) {
            ...
            this.createDeprecatedMessage();
            this.setupProfiler();
        },
        createDeprecatedMessage: function() {
            var latestSimpleXMLVersionURL = urlParser.setURLFlag(location.href, 'xmlv', '1.1', false);
            this.collection.dashboardMessages.add(new BaseModel({
                id: 'error-deprecated-dashboard',
                level: 'error',
                text: _('This dashboard view is deprecated and will be removed in future versions of Splunk software.').t(),
                linkText: _('Open the updated view of this dashboard.').t(),
                link: latestSimpleXMLVersionURL,
                linkExternal: true,
                dismissable: false
            }));
        },
        setupProfiler: function() {
            this.profiler = Profiler.get('Dashboard', {dashboard_name: this.model.view && this.model.view.entry.get('name')}).module(module.id);
        ...
        checkAddExtensionWarnings: function() {
            if (this.extensionLoader.hasNonDefaultExtensionScript()) {
        ...
        getDashboardMetricsOptions: function() {
            // DashboardTelemetry lacks context on VisualizationRegistry collections; if VisualizationRegistry is imported
            // from inside the dashboards-telemetry app, it will error out saying VisualizationRegistry has not been populated yet.
            // Importing SearchManager from inside the DashboardTelemetry app runs into a similar problem, so current fix
            // is to pass in the modules as arguments when calling the DashboardMetrics script from dashboards-telemetry.
            // Additionally, this ensures that DashboardTelemetry does not depend on the splunk-web-core library.
            return {
                model: {
                ...
                isValidPivotSearch: GeneralUtils.isValidPivotSearch
            };
        },
        sendDashboardMetrics: function() {
            var options = this.getDashboardMetricsOptions();
            DashboardMetrics.sendDashboardMetrics(options);
        },
        ...
        applyCustomExtension: function() {
            ...
            // load all custom js scripts defined in the dashboard
            this.scriptLoadDeferreds = this.scripts.map(function(js) {
                return this.extensionLoader.loadScriptExtension(this.model.view.entry.acl.get('app'), locale, root, js);
            }.bind(this));
            $.when(this.scriptLoadDeferreds).then(function() {
                this.sendDashboardMetrics();
            }.bind(this));

            // load all stylesheet scripts defined in the dashboard
