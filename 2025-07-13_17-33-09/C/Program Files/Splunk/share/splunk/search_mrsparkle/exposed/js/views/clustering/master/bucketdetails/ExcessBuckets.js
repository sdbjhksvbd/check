...
define([
    ...
    function(
        ...
        var ExcessBucketsView = BaseView.extend({
            ...
            removeExcessBuckets: function($button, indexName) {
                ...
                $.post(SplunkdUtils.fullpath(url))
                    ...
                    .error(function(response) {
                        var errorMsg = response.responseJSON.messages[0].text || _('Server Error').t();
                        this._confirmDialogModel.trigger('error', errorMsg);
                    }.bind(this));
            },
            ...
            getExcessBucketCount: function ($el) {
                // update excess bucket count
                try {
                    var $thisTab = $el ? $el.find('.excess-bucket-count-text') : $('.excess-bucket-count-text');
                    if (!_.isEmpty($thisTab)) {
                        $thisTab[0].textContent = this.collection.reduce(function (accumalator, element) {
                            if (!(element.entry && element.entry.content)) return;
                            var bc = element.entry.content.get('buckets_with_excess_copies') || 0;
                            return accumalator + parseInt(bc);
                        }, 0);
                    }  else {
                            throw 'Could not find excess bucket count element on UI';
                    }
                } catch (error) {
                    console.error(error);
                }
