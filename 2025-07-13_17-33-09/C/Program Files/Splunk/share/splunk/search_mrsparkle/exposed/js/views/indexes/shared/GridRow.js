    ...
    return BaseView.extend({
        ...
        isRemoteIndex: function() {
            ...
            return !!this.model.entity.entry.content.get("remotePath") ||
                   (this.model.entity.entry.content.get('isS2Mode') &&
                   splunkUtils.normalizeBoolean(this.model.entity.entry.content.get('isS2Mode')));
        },
        ...
        formatBucketLocationName: function(bucketLocation, bucketFolder) {
            if (bucketFolder) {
                return bucketLocation + '/' + bucketFolder;
            }
        ...
        formatArchiveRetention: function (archiveRetention) {
            // Convert archive retention to a map representation { day: 1, year: 2 }
            var dateMap = {
                years: 0,
                days: 0,
            };
            // Archive retention is stored in days. If archiveRetention is >= 365,
            // we store the value divided by 365 as years and the remainder as days
            // in the dateMap. Otherwise the value we only store days in the dateMap.
            if (archiveRetention >= 365) {
                dateMap.years = Math.floor(archiveRetention/365);
                dateMap.days = archiveRetention % 365;
            } else {
                dateMap.days = archiveRetention;
            }

            var dateStr = '';
            if (dateMap.years > 0) {
                if (dateMap.years === 1) {
                    dateStr += splunkUtils.sprintf(_('%s year').t(), dateMap.years);
                } else {
                    dateStr += splunkUtils.sprintf(_('%s years').t(), dateMap.years);
                }
            }
            if (dateMap.days > 0) {
                if (dateMap.years) {
                ...
                if (dateMap.days === 1) {
                    dateStr += splunkUtils.sprintf(_('%s day').t(), dateMap.days);
                } else {
                    dateStr += splunkUtils.sprintf(_('%s days').t(), dateMap.days);
                }
            }
            return dateStr;
        },
        render: function () {
            ...
            var bucketFolder = this.model.entity.entry.content.get("archiver.selfStorageBucketFolder");
            ...
            var html = _(this.template).template({
                ...
                archiveBucketExists: this.formatBucketLocationName(bucketLocation, bucketFolder),
                ...
                formatArchiveRetention: this.formatArchiveRetention,
                user: this.model.user,
        ...
        columns: [
            {
                ...
                visible: function() {
                    return this.model.controller.get('mode') === 'local' ||
                           this.collection.appLocals.isArchiverAppInstalled() ||
                           this.collection.entities.some(function(entity) {
                               return entity.entry.content.get('remotePath') ||
                                      (entity.entry.content.get('isS2Mode') &&
                                      splunkUtils.normalizeBoolean(entity.entry.content.get('isS2Mode')));
                            });
                }
            }, {
                ...
                visible: function() {
                    return this.model.controller.get('singleInstance') &&
                           !this.collection.appLocals.isArchiverAppInstalled() &&
                           this.collection.entities.every(function(entity) {
                               return !entity.entry.content.get('remotePath') &&
                                      (!entity.entry.content.get('isS2Mode') ||
                                      !splunkUtils.normalizeBoolean(entity.entry.content.get('isS2Mode')));
                            });
                ...
                id: 'selfStorageLocation',
                title: _('Self Storage').t(),
                visible: function() {
                    return ((this.model.controller.get('mode') === 'cloud') && this.collection.appLocalsUnfilteredAll.findByEntryName('dynamic-data-self-storage-app'));
                }
            }
