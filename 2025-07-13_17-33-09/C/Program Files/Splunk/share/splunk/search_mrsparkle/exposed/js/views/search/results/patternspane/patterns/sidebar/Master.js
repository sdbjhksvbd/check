    ...
    function($, _, module, ReportModel, Base, EventTypeDialog, AlertDialog, EstimatedEvents, route, i18n, splunkUtil){
        return Base.extend({
            ...
            instructionsTemplate: '\
                <a href="<%- docHref %>" target="_blank" rel="noopener noreferrer" class="documentation btn-pill" title="<%- _("Documentation").t() %>"><%- _("Documentation").t() %> <i class="icon-external"></i></a>\
                ...
                <div class="pattern-details-single">\
                    ...
                    <% if (search) { %>\
                        <div class="info-group">\
                            <div class="subtitle" tabindex=0><%- _("Estimated Events").t() %></div>\
                        ...
                        <div class="info-group">\
                            <div class="subtitle" tabindex=0><%- _("Search").t() %></div>\
                            <div class="event-type" tabindex=0><%- search %></div>\
                            <% if (eventtypeable) { %>\
                        ...
                        <% if (includeTerms.length) { %>\
                            <div class="info-group">\
                                <div class="subtitle" tabindex=0><%- _("Included Keywords").t() %></div>\
                                <div class="terms-group">\
                                    <% _(includeTerms).each(function(term) { %>\
                                        <div tabindex=0><%- term %></div>\
                                    <% }) %>\
                                </div>\
                            </div>\
                        <% } %>\
                        <% if (excludeTerms.length) { %>\
                            <div class="info-group">\
                                <div class="subtitle" tabindex=0><%- _("Excluded Keywords").t() %></div>\
                                <div class="terms-group">\
