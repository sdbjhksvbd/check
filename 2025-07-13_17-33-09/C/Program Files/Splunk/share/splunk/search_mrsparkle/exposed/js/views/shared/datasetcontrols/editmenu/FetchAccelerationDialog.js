        ...
        return Modal.extend({
            ...
            initialize: function (options) {
                ...
                this.on('hidden', function () {
                    // DialogBase setFocus()
                    this.children.accelerationDialog.setFocus();
                }, this);
            },
            ...
            accelerationWarningTemplate: '\
                <% if (datasetType === "table") { %>\
                    ...
                    <a class="external datasets-help-link" href="<%- accelerationHelpLink %>" target="_blank" rel="noopener noreferrer"><span><%- _("Learn more.").t() %></span></a>\
                <% } else { %>\
                    <span class="acceleration-warning"><%= _("You can only accelerate data models that include at least one event-based dataset or one search-based dataset that does not use reporting commands.").t() %></span>\
                <% } %>\
            ',
