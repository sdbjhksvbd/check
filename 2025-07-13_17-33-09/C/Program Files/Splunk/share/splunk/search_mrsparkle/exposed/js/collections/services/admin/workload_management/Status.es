...
export default SplunkDsBaseCollection.extend({
    ...
    getGeneral(attr) {
        if (_.isUndefined(this.findByEntryName('workload-management-status'))) {
            if (attr === 'error_message') return _('error returning message').t();
            return false;
        }
        return this.findByEntryName('workload-management-status').entry.content.get('general')[attr];
    },
    getErrorMessage() {
    ...
    getDropdownOptions(includeEmptyOption = false) {
        ...
        if (this.isEnabled()) {
            this.populatePoolOptions(
                this.workloadPoolOptions,
                this.findByEntryName(
                    'workload-management-status',
                ).entry.content.get('workload-category:search')['workload-pools'],
    ...
    getDefaultSearchPoolName() {
        return this.findByEntryName(
            'workload-management-status',
        ).entry.content.get('workload-category:search').search.default_category_pool;
    },
    getDefaultMiscPoolName() {
        return this.findByEntryName(
            'workload-management-status',
        ).entry.content.get('workload-category:misc').misc.default_category_pool;
    },
    poolIdExists(poolID) {
        if (_.isUndefined(this.findByEntryName('workload-management-status')) ||
            _.isEmpty(this.findByEntryName(
                'workload-management-status',
            ).entry.content.get('workload-category:search')['workload-pools'])) {
            return undefined;
        }
        return _.has(this.findByEntryName(
            'workload-management-status',
            ).entry.content.get('workload-category:search')['workload-pools'], poolID) ||
            _.has(this.findByEntryName(
                'workload-management-status',
            ).entry.content.get('workload-category:ingest')['workload-pools'], poolID);
    },
