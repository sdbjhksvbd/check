define(
    ...
    function($, _, Backbone, SplunkDBaseModel, splunkUtils, splunkdUtils, console) {
        var commaSeparatedAlphaNumericRegex = /^(\s*[\w. *-]+\s*)(,\s*[\w. *-]+\s*)*$/;
        ...
        return SplunkDBaseModel.extend({
            initialize: function(options) {
                ...
                var cloudEndpoint = options.isSingleInstanceCloud ? onPremEndpoint :
                    'cluster_blaster_transforms/sh_metric_transforms_manager';
                this.url = this.isCloud ? cloudEndpoint : onPremEndpoint;
            },
            defaults: {
                'blacklist_dimensions': undefined,
                'whitelist_dimensions': undefined,
                ...
                'field_names': [
                    {
                        ...
                        msg: _('Measures field in \'Metrics\' tab must include at least one value.').t()
                    },
                    {
                        required: function (val, attr, computed) {
                            return computed[attr] === '';
                        },
                        pattern: commaSeparatedAlphaNumericRegex,
                        msg: _('Values provided for the Measures field on the \'Metrics\' tab must be composed of alphanumeric characters, dot characters (.), or wildcard characters (*). Multiple values must be comma-separated.').t()
                    }
                ],
                'blacklist_dimensions': [
                    {
                        ...
                        pattern: commaSeparatedAlphaNumericRegex,
                        msg: _('Values provided for the Blacklist field on the \'Metrics\' tab must be composed of alphanumeric characters, dot characters (.), or wildcard characters (*). Multiple values must be comma-separated.').t()
                    }
                ],
                'whitelist_dimensions': [
                    {
                        ...
                        pattern: commaSeparatedAlphaNumericRegex,
                        msg: _('Values provided for the Whitelist field on the \'Metrics\' tab must be composed of alphanumeric characters, dot characters (.), or wildcard characters (*). Multiple values must be comma-separated.').t()
                    }
                ]
            },
            isParsing: function() {
            ...
            sanitizeAttributes: function() {
                ...
                _.each(this.attributes, function(value, key) {
                    if (_.indexOf(validAttributes, key) < 0) {
                        this.unset(key);
                    } else if (key === 'whitelist_dimensions' || key === 'blacklist_dimensions' || key === 'field_names') {
            ...
            save: function(key, val, options) {
                ...
                if (_.isUndefined(this.get('whitelist_dimensions'))) {
                    this.set({'whitelist_dimensions':''});
                }
