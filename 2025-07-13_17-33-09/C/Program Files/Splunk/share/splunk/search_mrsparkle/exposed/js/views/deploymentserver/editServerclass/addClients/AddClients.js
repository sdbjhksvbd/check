    ...
    function(
            ...
            initialize: function() {
                ...
                this.children.blacklist = new ControlGroup({
	                ...
	                controlOptions: {
                        ...
                        modelAttribute: 'filter',
                        additionalClassNames: 'filter-text-area regex-box',
		...
		var html = this.compiledTemplate({backButtonUrl: this.getBackButtonUrl(), sectionTitle: this.getSectionTitle(), nameLabel: this.getNameLabel(), docUrl: docUrl, filtersDocUrl: filtersDocUrl, _:_, serverClass: this.model});
            ...
            handleSaveClicked: function() {
                ...
                // SPL-133310: fixing error messages overlap when trying dedup whitelists and blacklists
                // all blacklist from serverClass model is being reset to null before any steps
                // however, iterating over blackListLength won't yeild the right results as
                // there are instances where blackListLength would be 0 and blacklists on other indices would still exist
                var allBlacklistIndicesIfAny = this.getAllBlacklistIndicesFromServerClassModel();
                for (i = 0; i < allBlacklistIndicesIfAny.length; i++){
                   this.model.serverClass.entry.content.set("blacklist."+allBlacklistIndicesIfAny[i], null);
                }
            ...
            getReturnTo: function(model) {
                model = model || this.model.serverClass;
                if (HtmlCleaner.isBadUrl(this.options.return_to)) {
                    return route.manager(this.options.application.get('root'), this.options.application.get('locale'), this.options.application.get('app'), 'error');
                } else {
            ...
            getAllBlacklistIndicesFromServerClassModel: function(){
                var allKeysFromServerClassAttributes = Object.keys(this.model.serverClass.entry.content.attributes);

                var blacklistIndices = [];
                for(var i=0; i<allKeysFromServerClassAttributes.length; i++){
                    if (allKeysFromServerClassAttributes[i].startsWith("blacklist.")){
                        blacklistIndices.push(allKeysFromServerClassAttributes[i].split('.')[1]);
                    }
                }
