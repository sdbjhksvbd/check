        ...
        */
        ...
        events: $.extend({}, Modal.prototype.events, {
            'click .save.modal-btn-primary': function(e) {
                ...
                if (this.model.inmem.entry.content.get('action.summary_index._type') === 'event') {
                    this.model.inmem.entry.content.set({
                        'action.summary_index._metric_dims': '',
                    });
                }
        ...
        initialize: function(options) {
            ...
            this.children.summarize = new ControlGroup({
                ...
                help: _('Create summaries of event or metric data. ').t() +
                      '<a href="' + _.escape(summaryIndexingHelpLink) + '" target="_blank" rel="noopener noreferrer">' + _("Learn More").t() + ' <i class="icon-external"></i></a>'
            });


            if (_.isEmpty(this.model.inmem.entry.content.get('action.summary_index._type'))) {
                this.model.inmem.entry.content.set({
                    'action.summary_index._type': 'event',
                });
            }
            this.children.indexFilter = new ControlGroup({
                controls: [new SyntheticRadioControl({
                    model: this.model.inmem.entry.content,
                    modelAttribute: 'action.summary_index._type',
                    items: [
                        { label: _('Event').t(), icon: 'event', value: 'event'},
                        { label: _('Metric').t(), icon: 'metric', value: 'metric'}
                    ]
                })],
                label: _('Select the index type').t(),
                tooltip: _('Select Metric to convert the results of this scheduled searchâ€”whether events or metricsâ€”into a summary of metric data points. ' +
                    'Summary metric indexes provide better metrics search performance and can be powerfully visualized through the Splunk Analytics Workspace').t()
            });

            this.children.dimensions = new ControlGroup({
                controls: [new TextControl({
                    model: this.model.inmem.entry.content,
                    modelAttribute: 'action.summary_index._metric_dims',
                    placeholder: _('Separate field names with commas').t()
                })],
                label: _('Exclude from measures').t(),
                help: _('The metric summarization process converts all fields with numeric values into measures. Here, you can optionally identify numeric fields that should instead be converted into dimensions.').t()
            });
            ...
            this.children.summaryIndexes = new ControlGroup({
                ...
                help: _('Choose an index that matches the index type you have selected. Only indexes you can write to are listed.').t()
            });
            ...
            this.model.inmem.entry.content.on('change:action.summary_index._type', function() {
                if (this.model.inmem.entry.content.get('action.summary_index._type') === 'metric') {
                    this.children.dimensions.$el.show();
                } else {
                    this.children.dimensions.$el.hide();
                }
            }, this);
        },
        ...
        filterToExistingFields: function() {
            ...
            var defaultFields = ['action.summary_index._metric_dims', 'action.summary_index._type',
                                 'action.summary_index._name', 'action.summary_index.inline',
        ...
        showSummaryOptions: function() {
            ...
            this.children.indexFilter.$el.show();
            this.children.addFields.$el.show();
        },

        hideSummaryOptions: function() {
            this.children.dimensions.$el.hide();
            this.children.indexFilter.$el.hide();
        ...
        render: function() {
            ...
            $.when(this.intentionsParserDeferred).then(function(){
                ...
                if (isScheduled) {
                    ...
                    this.children.indexFilter.render().appendTo(this.$(Modal.BODY_FORM_SELECTOR));
                    this.children.dimensions.render().appendTo(this.$(Modal.BODY_FORM_SELECTOR));
