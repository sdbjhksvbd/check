define(
    ...
    function($, _, splunkUtils, splunkConfig, generalUtils) {
        ...
        var getParentRiskyError = function(manager) {
            if (!manager) {
                return false;
            }
            if (manager.parent) {
                // if already find the risky error from the current postprocess manager, then no need to continue
                if (manager.containsRiskyCommand) {
                    return true;
                }
                return getParentRiskyError(manager.parent);
            }

            var riskyError = manager.lastError && manager.lastError.err && manager.lastError.err.data.messages;
            return messagesContainsOneOfTypes(riskyError, [RISKY_COMMAND]);
        };
        ...
        return {
            ...
            getParentRiskyError: getParentRiskyError,
