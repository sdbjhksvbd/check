...
function(
    ...
    return BaseView.extend({
        ...
        update: function(){
            ...
            this.$('#siteRepFactor').html(this._extractFactorValue(this.model.clusterConfig.entry.content.get('site_replication_factor')));
            this.$('#siteSearchFactor').html(this._extractFactorValue(this.model.clusterConfig.entry.content.get('site_search_factor')));
            this.$('#genId').html(this.model.masterGeneration.entry.content.get('generation_id'));
            this.$('a.cluster-push')[noPeers?'hide':'show']();
        },

        _extractFactorValue: function (factor){
            // trim {}
            return factor?factor.replace(/[{}]/g,'').trim():null;
        },
        render: function(){
            var html = this.compiledTemplate({
                ...
                siteReplicationFactor: this._extractFactorValue(this.model.clusterConfig.entry.content.get('site_replication_factor')),
                siteSearchFactor: this._extractFactorValue(this.model.clusterConfig.entry.content.get('site_search_factor')),
