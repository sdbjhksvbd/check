...
define([
    ...
    var DataModel = SplunkDBase.extend({
        ...
        /**
         * Returns a boolean to validate model name to make sure it exists
         * and is in the proper format.
         * @param modelName
         */
        validateModel: function(modelName, owner) {
            var path = decodeURIComponent(modelName).replace(owner, "");
            if (path === undefined || path === "" || !path.match(/^[\w\-\/\d_]+$/)) {
               return false;
            }
            return true;
        },
        ...
        getSourceGUID: function() {
           return this.entry.content.acceleration ? (this.entry.content.acceleration.get("source_guid")) : "";
        },
