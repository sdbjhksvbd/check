define(
    ...
    function($, _, module, PopTartView, FVTags, SplunkDModel, TagDialog, generalUtils, util, route, HtmlCleaner) {
        return PopTartView.extend({
            ...
            */
            ...
            uri: function(uri) {
                uri = HtmlCleaner.isBadUrl(uri) ? '' : uri;
                if(uri.indexOf('/') === 0) {
            ...
            link: function(content, target) {
                ...
                if (uri.indexOf('http://') === -1 && uri.indexOf('https://') === -1) {
                    $form.append($('<input/>').attr({
                        'type': 'hidden',
                        'name': 'splunk_form_key',
                        'value': util.getFormKey()
                    }));
                }
                $('body').append($form);
            ...
            search: function(content, target){
                ...
                (target === '_self') ? (window.location.href = url): window.open(url, '_blank', 'noopener,noreferrer');
                return false;
            },    
