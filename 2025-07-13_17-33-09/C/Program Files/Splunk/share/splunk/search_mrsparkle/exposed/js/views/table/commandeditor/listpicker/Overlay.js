    ...
    return ListPicker.extend({
        ...
        _show: function(animate) {
            ...
            this.$el.css({
                ...
                'border-top': this.options.marginTop ? this.options.border : this.options.marginTop
            }); 
        ...
        render: function() {
            if (this.options.name) {
                this.$el.attr('data-test-name', this.options.name);
            }

            ListPicker.prototype.render.apply(this, arguments);
            // show the filter input
            if (this.options.required && !this.hasSelection() && (this.items.length !== 0)) {
                this.$('.list-picker-filter').addClass('show').focus();
            } 
        ...
        template:  ListPicker.prototype.template +'<div class="list-picker-buttons" >\
                <a class="list-picker-back btn"><%- _("Back").t() %></a>\
                <% if (options.multiselect) { %><a class="list-picker-ok <%- !hasValue && options.required ? "btn disabled" : "btn btn-primary" %>"><%- _("Next").t() %></a><% } %>\
            </div>'
    }, {
        PAGINATOR_TYPES: {
