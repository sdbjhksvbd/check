...
import Confirmation from 'views/shared/ModalConfirmation/Master';
...
export default Modal.extend({
    ...
    initialize(options = {}) {
        ...
        if (options.isNew) {   // new mode
            ...
            if (!this.isArchiverAppInstalled && !this.isS2 && !this.model.controller.get('singleInstance')) {
            ...
            const timestampResolution = this.model.entity.entry.content.get('metric.timestampResolution');

            this.addEditIndexModel = new this.options.archiverModelClass({ // eslint-disable-line new-cap
                id: this.model.entity.entry.get('name'),
                name: this.model.entity.entry.get('name'),
                isNew: false,
                datatype: this.model.entity.entry.content.get('datatype'),
                'metric.timestampResolution': timestampResolution,
            ...
            this.children.confirmDialog = new Confirmation({
                isAlert: true,
                title: _('Storage Type Confirmation').t(),
                buttonRightLabel: _('Yes').t(),
                closeOnEscape: true,
                showCloseButton: true,
                text: _('Switching away from Splunk Archive will delete all ' +
                    'archived data for this index. Are you sure you want to do that? ').t(),
            });

            this.children.confirmDialog.on('success', () => {
                this.onSubmit();
            });

            this.children.confirmDialog.on('shown', () => {
                this.$el.addClass('index-modal-backdrop');
            });

            this.children.confirmDialog.on('hidden', () => {
                this.$el.removeClass('index-modal-backdrop');
            });
        }
        ...
        */
        ...
        this.children.timestampResolution = new ControlGroup({
            controlType: 'SyntheticRadio',
            controlOptions: {
                modelAttribute: 'metric.timestampResolution',
                model: this.addEditIndexModel,
                items: [
                    {
                        label: 'Seconds',
            ...
            label: _('Timestamp Resolution').t(),
            help: _('Specify the timestamp precision of the metrics in this index. ' +
                'Indexes with millisecond timestamp precision have reduced search performance.').t(),
        });
    ...
    events: $.extend({}, Modal.prototype.events, {
        'click .btn-primary'() { // eslint-disable-line object-shorthand
            const currentStorageOptionArchive = (this.model.stateModel.get('dynamicStorageOption') ===
            this.options.ARCHIVE);
            if (this.isArchiveIndex() && !currentStorageOptionArchive && this.children.confirmDialog) {
                this.children.confirmDialog.render().show();
            } else {
                this.onSubmit();
            }
        },
    ...
    isS2() {
        return this.collection.entities.some(entity =>
               entity.entry.content.get('remotePath') ||
               (entity.entry.content.get('isS2Mode') &&
               splunkutil.normalizeBoolean(entity.entry.content.get('isS2Mode'))));
    },

    isArchiveIndex() {
        if (this.model.controller.get('singleInstance')) {
            return false;
        }
        return (this.model.entity.entry.content.get('archiver.coldStorageProvider') ===
        this.addEditIndexModel.getColdStorageProvider());
    },

    toggleVisibility() {
        this.isMetrics = this.addEditIndexModel.get('datatype') === 'metric' ||
            this.addEditIndexModel.get('datatype') === 'rollup';
        if (this.isMetrics) {
            this.children.timestampResolution.show();
        } else {
            this.children.timestampResolution.hide();
        }
    },
    ...
    onSubmit() {
        ...
        const frozenTimePeriodInSecsValue = this.addEditIndexModel.get('frozenTimePeriodInDays') *
            (60 * 60 * 24);
        const datatypeValue = this.addEditIndexModel.get('datatype');
        const metricResolutionValue = this.addEditIndexModel.get('metric.timestampResolution');
        ...
        if (this.model.controller.get('singleInstance') && !this.isS2 && !this.isArchiverAppInstalled) {
            this.modelToUpdate.entry.content.set({
                ...
                'metric.timestampResolution': metricResolutionValue,
    ...
    showSelectedStorageView() {
        ...
        const archiveWarning = _('Switching away from Splunk Archive will delete all ' +
            'archived data for this index').t();
        if (storageOption) {
            switch (storageOption) {
                case this.options.ARCHIVE:
                    this.children.flashMessagesView.flashMsgHelper.removeGeneralMessage('archiveWarn');
                ...
                case this.options.SELF_STORAGE:
                    if (this.isArchiveIndex()) {
                        this.children.flashMessagesView.flashMsgHelper.addGeneralMessage('archiveWarn', {
    ...
    render() {
        if (!this.el.innerHTML) {
            ...
            this.children.timestampResolution.render().appendTo(this.$('.timestamp-resolution-placeholder'));
    ...
    template: `
        ...
        <div class='timestamp-resolution-placeholder'></div>
