define(
    ...
    function(
        ...
        return BaseView.extend({
            ...
            isExpanded: function() {
                return this.model.state.get(this.rowExpanded) ? true : false;
            },
            ...
            toggleArrow: function(open) {
                var $arrow = this.$('td.expands');
                var $arrowIcon = this.$('td.expands > a > i').removeClass('icon-triangle-right-small icon-triangle-down-small');
                $arrowIcon.addClass((open) ? 'icon-triangle-down-small':  'icon-triangle-right-small');
                $arrow.attr('aria-expanded', open); // Arg "open" is passed as either true or false
            },
            render: function() {
            ...
            template: '<!--Line breaks intentionally removed to fix obscure IE9 bug with tables-->' +
                '<% if (allowRowExpand) { %>' +
                '<td tabindex="0" class="expands <%- colorClass %> <% if (isPreviewEvent) { %> disabled <% } %>" aria-expanded="<%- expanded %>">' +
                    '<a aria-label="<%- _("Search Events Viewer Section").t() %>"><i class="icon-triangle-<%- expanded ? "down" : "right" %>-small"></i></a>' +
                '</td>' +
                '<% } %>' +
                ...
                '<td class="col-icon" <% if (!isList || !iconPath) { %> style="display:none" <%}%> tabindex="0">' +
                   '<em class="icon"><img src="<%= iconPath %>" alt="<%- _("Column icon").t() %>"/></em>' +
                '</td>' +
                '<td class="event"></td>'
        });
    }
