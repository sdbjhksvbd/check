    ...
    function ($,
        ...
        return BaseView.extend({
            ...
            initialize: function (options) {
                ...
                this.model.input.on('change:ui.name', function() {
                    var scriptPath = this.model.input.get('ui.name');
                    var fullScriptPath = scriptPath.replace('$SPLUNK_HOME', this.splunkHomePath);
                    this.model.input.set({'fullScriptPath': fullScriptPath});
                }.bind(this));
            ...
            faqList: function() {
                return [
                {
                    ...
                    '<a class="external" href="' + arguments[0] + '" target="_blank" rel="noopener noreferrer">' + _("Learn More").t() + '</a>'
                },
                {
                ...
                },
                {
                    ...
                    answer: splunkUtil.sprintf(_('Use a special combination of text (known as the cron notation) to specify start and interval times for the scripted input to run. %s to learn how.').t(), '<a class="external" href="' + arguments[1] + '" target="_blank" rel="noopener noreferrer">' + _("See this page").t() + '</a>')
                }
                ];
            },
