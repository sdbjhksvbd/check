...
import { callGetPrivateAppGloablFlag } from './Utils';
...
const ACTION_SUCCESS_MESSAGE = _(' (version %s) has been successfully %s.').t();
...
const APP_VALIDATION_COMPLETE_TITLE = _('App Vetting Complete').t();
...
class UploadedAppsPageContainer extends React.Component {
    constructor(props) {
        ...
        callGetPrivateAppGloablFlag()
        .then((response) => {
            this.setState({
                privateAppGlobal: response,
            });
        })
        .catch(() => {
            this.setState({
                privateAppGlobal: false,
            });
        });
    }
    ...
    getDefaultState = () => ({
        ...
        vettingProgressModalOpen: false,
        vettingErrorModalOpen: false,
        onAcknowledgementModalOpen: this.handleAcknowledgementModalOpen,
        onAcknowledgementModalClose: this.handleAcknowledgementModalClose,
        onVettingProgressModalOpen: this.handleVettingProgressModalOpen,
        onVettingProgressModalClose: this.handleVettingProgressModalClose,
        onVettingErrorModalOpen: this.handleVettingErrorModalOpen,
        onVettingErrorModalClose: this.handleVettingErrorModalClose,
    ...
    handleRequestUpload = () => {
        ...
        .done((xhrArgs, responseMessage) => {
            ...
            this.vettingTask = new TaskModel();
            this.vettingTask.entry.set('name', taskId);

            const pollVettingTask = () => {
                this.vettingTask.beginPolling()
                .done(() => {
                    this.props.packages.fetch().done(() => {
                        const currentPackage = this.props.packages.models.find(
                            model => model.id === packageId);
                        this.packageModel = currentPackage;
                        const vettingStatus = this.vettingTask.entry.content.get('stages')[0].payload.packageStatus;
                        if (vettingStatus === 'approved') {
                            const warnings = this.vettingTask.entry.content
                                .get('stages')[0].payload.statusData.info.warning;
                            this.setState({
                                action: LABELS.Install,
                                vettingProgressModalOpen: false,
                                acknowledgementModalOpen: true,
                                viewReportLink: currentPackage.getFullLink('report'),
                                status: vettingStatus,
                                vettingWarningCount: warnings,
                                acknowledgementModalTitle: APP_VALIDATION_COMPLETE_TITLE,
                            });
                        } else {
                            const failures = this.vettingTask.entry.content
                                .get('stages')[0].payload.statusData.info.failure;
                            this.setState({
                                vettingProgressModalOpen: false,
                                vettingErrorModalOpen: true,
                                vettingErrorCount: failures,
                                viewReportLink: currentPackage.getFullLink('report'),
                                status: vettingStatus,
                                action: LABELS.Close,
                            });
            ...
            if (this.state.privateAppGlobal) {
                pollVettingTask();
            }
            ...
            this.setState({
                ...
                responseMessage,
                packageId,
                packageName: this.state.file.name,
                buttonLabel: LABELS.Close,
                vettingProgressModalOpen: this.state.privateAppGlobal,
                uploadDialogOpen: !this.state.privateAppGlobal,
            });
    ...
    handleAcknowledgementModalOpen = (packageModel) => {
        this.packageModel = packageModel;
        this.setState({
            action: LABELS.Install,
            packageId: packageModel.id,
            packageName: packageModel.getApp().label,
            acknowledgementModalOpen: true,
        });
    }

    handleAcknowledgementModalClose = () => {
        this.setState({
            acknowledgementModalOpen: false,
    ...
    handleInstallPackage = () => {
        ...
        .done((xhrArgs) => {
            ...
            this.setState({
                action: LABELS.Install,
                actionDialogOpen: !this.state.privateAppGlobal,
                installProgressModalOpen: this.state.privateAppGlobal,
                acknowledgementModalOpen: false,
                acknowledgementModalTitle: '',
                vettingWarningCount: '0',
                status: 'info',
            ...
            const pollInstallTask = () => {
                ...
                .done(() => {
                    this.setState({
                        ...
                        actionDialogTitle: _('App Installation Complete').t(),
                        buttonLabel: LABELS.Close,
                        taskStatus: STATES.SUCCESS,
                        actionDialogOpen: true,
                        installProgressModalOpen: false,
                        appName: this.packageModel.getApp().label,
                    });
