define(function(require, exports, module) {
    ...
    var SearchManager = BaseManager.extend( /** @lends splunkjs.mvc.SearchManager.prototype */{
        ...
        initialize: function(options) {
            ...
            this.listenTo(this, "search:error", function(msg, err, job) {
                this.lastError = { message: msg, err: err, job: job };
                return true;
            });
        ...
        handleCancelOnUnload: function() {
            this.stopListeningDOM($(window), 'beforeunload', this._cancelOnUnload);
            if (this.get('cancelOnUnload') && !this.get('cache')) {
                this.listenToDOM($(window), 'beforeunload', this._cancelOnUnload);
            }
        },
        ...
        _cancelOnUnload: function() {
            ...
            if (job && !job.isDone()) {
                // firefox seems to kill the session before the query is sent if it is async
                // while chrome and edge both only work if it is async
                job.cancel({
                    async: !userAgent.isFirefox(),
                });
            }
        },
        ...
        _createOptionsForSearchJob: function() {
            ...
            if (options.search) {
                ...
                if (this.settings.get('check_risky_command')) {
                    options.check_risky_command = true;
                }
            }
                        ...
                        if (splunkDUtils.messagesContainsOneOfTypes(err.data.messages, [splunkDUtils.RISKY_COMMAND])) {
                            if (dashboardUtils.isHiddenSearch(that.get('parentElement'))) {
                                if (dashboardUtils.isHiddenSearchRiskyCommandDisabled()) {
                                    ...
                                    job.error.set("messages", err.data.messages);
                                    that.showRiskyWarningDialog(job);
                                }
                            } else {
                                that.trigger("search:error", _("Action required: Click the icon in the top right corner to review the query before running.").t(), err, job);
                            }
                        }
                        else {
    ...
    function MockSdkJob(sdkService, sid) {
        ...
        this._sdkJob.error = null;
        this._sdkJob.setError = function(err) {
            this._sdkJob.error = err;
        }.bind(this);
        // add off event to match Backbone model to avoid failures when switching UI mode to source mode
        this._sdkJob.off = function() {};
        this._sdkJob.getRiskyCommands = function() {
            if (!this._sdkJob.error) {
                return [];
            }
            var messages = this._sdkJob.error.data.messages,
                riskyCommandMessage = _.findWhere(messages, {type: splunkDUtils.RISKY_COMMAND});

            //TODO: Currently the error message is "Found potentially risky commands:<space separated list of commands> "
            // The error response also has a RiskyCommands object that is just the space separated list of command.
