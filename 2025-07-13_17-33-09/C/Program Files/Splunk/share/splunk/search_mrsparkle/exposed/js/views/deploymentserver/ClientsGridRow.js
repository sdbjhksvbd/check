        ...
        return BaseView.extend({
           ...
           renderMoreServerclassesLink: function() {
            ...
            render: function() {
                ...
                var applicationList = "";
                // SPL-188436 Fixed the display of deployed and undeployed apps
                var app_names = _.keys(applications);
                if (app_names.length > 0) {
                    var deployedApps = _.filter(app_names, function (app) {
                        return applications.hasOwnProperty(app) && applications[app]['result'] === 'Ok';
                    });
                    var undeployedApps = _.filter(app_names, function (app) {
                        return applications.hasOwnProperty(app) && applications[app]['result'] != 'Ok';
                    });
                    applicationList = deployedApps.concat(undeployedApps).slice(0, 3).join(', ');
                }
