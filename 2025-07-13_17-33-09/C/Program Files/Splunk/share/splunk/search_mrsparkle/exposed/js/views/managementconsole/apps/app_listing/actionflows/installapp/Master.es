...
export default DMCBaseDialog.extend({
    ...
    primFn() {
        ...
        return this.model.app.install(this.appVersion);
    },
    ...
    getSuccessChildren() {
        ...
        if (!_.isNull(releaseNotesURI)) {
            childrenArr.push(`<a target="_blank" rel="noopener noreferrer" href="${releaseNotesURI}"
            class="btn btn-secondary successbtn releasenotes">${_('Read Release Notes').t()} 
    ...
    '<% _.each(licenses, function(licenseGroup, licenseName) { %>' +
        '<div class="license-group">' +
            '<p class="rights"><%- _("The").t() %> ' +
                '<a target="_blank" rel="noopener noreferrer" class="license-link" ' +
        ...
        '</div>' +
    '<% }) %>' +
    '<a target="_blank" rel="noopener noreferrer" class="license-agreement" id="cloud-terms-conditions" href="' +
