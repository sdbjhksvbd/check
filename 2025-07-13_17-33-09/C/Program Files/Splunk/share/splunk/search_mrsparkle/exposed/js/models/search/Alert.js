define(
    ...
    function(
        ...
        var Model = ReportModel.extend({
            ...
            validateAlertID: function(id) {
                if (id.match(/^[\d:.\/]+$/) || id.includes("://") || HtmlCleaner.isBadUrl(decodeURIComponent(id))){
                    return false;
                }
                    return true;
            },
                ...
                switch(type) {
                    ...
                    case 'custom':
                        ...
                        if (isRealtime) {
                            switch(timeParse.unit) {
                                case 'm':
                                    return splunkUtil.sprintf(i18n.ungettext('Custom. "%(alertCondition)s" in %(timeAmount)s minute.', 'Custom. "%(alertCondition)s" in %(timeAmount)s minutes.', timeParse.amount), {alertCondition: _.escape(alertCondition), timeAmount: timeParse.amount});
                                case 'h':
                                    return splunkUtil.sprintf(i18n.ungettext('Custom. "%(alertCondition)s" in %(timeAmount)s hour.', 'Custom. "%(alertCondition)s" in %(timeAmount)s hours.', timeParse.amount), {alertCondition: _.escape(alertCondition), timeAmount: timeParse.amount});
                                case 'd':
                                    return splunkUtil.sprintf(i18n.ungettext('Custom. "%(alertCondition)s" in %(timeAmount)s day.', 'Custom. "%(alertCondition)s" in %(timeAmount)s days.', timeParse.amount), {alertCondition: _.escape(alertCondition), timeAmount: timeParse.amount});
                            }
                        } else {
                            return splunkUtil.sprintf(_('Custom. "%s".').t(), _.escape(alertCondition));
                        }
                        break;
                }
            },
            getAlertTriggerActions: function () {
                var alertActions = [];
                var tempAlert = this.entry.content;
                if (splunkUtil.normalizeBoolean(tempAlert.get('alert.track'))) {
                    alertActions.push('Add to Triggered Alerts');
                }
                if (tempAlert.get('action.email')) {
                    alertActions.push('Send Email');
                }
                if (tempAlert.get('action.script.filename')) {
                    alertActions.push('Run a Script');
                }
                if (splunkUtil.normalizeBoolean(tempAlert.get('action.lookup'))) {
                    alertActions.push('Lookup Options');
                }
                if (splunkUtil.normalizeBoolean(tempAlert.get('action.logevent'))) {
                    alertActions.push('Log Event');
                }
                if (splunkUtil.normalizeBoolean(tempAlert.get('action.webhook'))) {
                    alertActions.push('WebHook');
                }
                if (splunkUtil.normalizeBoolean(tempAlert.get('action.outputtelemetry'))) {
                    alertActions.push('Output to Telemetry');
                }
                return alertActions;
            }
        });
