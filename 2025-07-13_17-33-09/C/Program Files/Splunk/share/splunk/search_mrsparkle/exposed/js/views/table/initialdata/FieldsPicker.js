        ...
        return BaseView.extend({
            ...
            initialize: function() {
                ...
                if (this.model.ast && !this.model.ast.isTransforming() && !this.model.ast.getFromCommandObjectPayloads().length) {
                    staticItems = [{ value: '_time', isDisabled: true, type: 'timestamp' }, { value: '_raw', type: 'raw' }];
                }
                ...
                this.children.fieldsPicker = new ListPicker({
                    ...
                    items: _.map(this.collection.columns.models, function(item) {
                        return { value: item.get('name'), type: item.get('type') };
                    }, this),
                    // _time and _raw are always at the top of the list
                    staticItems: staticItems,
                    header: _('Select existing fields').t(),
                    multiselectMessage: _('Filter fields').t(),
                    allMessage: _("all fields").t(),
            ...
            events: {
                'mouseenter .list-picker-list > li > a': function(e) {
                    e.preventDefault();
                    this.handleFieldHover(e);
                },
                'mouseleave .list-picker-list > li > a': function(e) {
                    e.preventDefault();
