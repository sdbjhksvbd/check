define([
    ...
    'splunkjs/mvc/components/UrlWarningModal',
    ...
    'util/console',
    'util/react_render'
], function (React, $, _, mvc, BaseManager, BaseSplunkView, UrlWarningModal, Settings, TokenUtils, Utils, SplunkUtil, Eval, console, reactRender) {

    var EventHandler = BaseManager.extend({
        componentIdSetting: 'viewid',
        initialize: function (options) {
            this.$urlWarningModal = $('<div class="url-warning-modal"/>');
            if (!options.event) {
        ...
        executeAction: function (action, data, evt) {
            ...
            switch (action.type) {
                ...
                case "link":
                    ...
                    if (EventHandler.isAbsoluteURL(url) && !sessionStorage.getItem(url)) {
                        this._showUrlWarningModal(url).then(function() {
                            return EventHandler.linkTo(action.value, action.target || target, data, newWindow);
                        }).catch(function() {
                            // do nothing if modal is canceled
                        });
                    } else {
                        return EventHandler.linkTo(action.value, action.target || target, data, newWindow);
                    }
        ...
        _showUrlWarningModal: function(url) {
            return new Promise(function (resolve, reject) {
                var previouslyFocusedElement = document.activeElement;
                var removeUrlWarningModal = function() {
                    if (this.unmountUrlWarningModal) {
                        this.unmountUrlWarningModal();
                        this.unmountUrlWarningModal = null;
                    }
                    previouslyFocusedElement.focus();
                };
                var onConfirm = function() {
                    resolve();
                    removeUrlWarningModal.call(this);
                };
                var onRequestClose = function() {
                    reject();
                    removeUrlWarningModal.call(this);
                };
                var onAddUrlToAllowList = function() {
                    sessionStorage.setItem(url, true);
                };
                var urlWarningModal = React.createElement(UrlWarningModal, {
                    url,
                    onConfirm: onConfirm.bind(this),
                    onRequestClose: onRequestClose.bind(this),
                    onAddUrlToAllowList: onAddUrlToAllowList.bind(this),
                });
                this.unmountUrlWarningModal = reactRender(urlWarningModal, this.$urlWarningModal[0]);
            }.bind(this));
        }
    }, {
