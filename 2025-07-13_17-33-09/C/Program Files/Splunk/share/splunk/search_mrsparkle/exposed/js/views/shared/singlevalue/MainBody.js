        ...
        return BaseView.extend({
            ...
            reflow: function() {
                ...
                if (generalUtil.valuesAreNumericAndFinite([scaledMainBodyPadding])) {
                    var transformAttr = this.transformToFit(scaledMainBodyPadding, this.$el[0].getBBox().width);
                    this.$el.attr('transform', transformAttr);
                }
            },

            transformToFit: function(scaledMainBodyPadding, innerWidth) {
                var parentWidth = this.model.presentation.get('svgWidth');
                if (parentWidth < innerWidth) {
                    return 'translate(' + scaledMainBodyPadding + ',' + scaledMainBodyPadding + ') scale(0.4)';
                } else {
                    return 'translate(0,' + scaledMainBodyPadding + ')';
                }
            },
