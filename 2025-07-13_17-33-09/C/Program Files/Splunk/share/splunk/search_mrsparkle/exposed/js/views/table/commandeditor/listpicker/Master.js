...
function(
     ...
     *     {String} header (Optional) The header above the input list.
     *     {String} multiselectMessage (Optional) The text at the top of the list when multiselect is enabled.
     *     {String} allMessage (Optional) The label for all checkbox when multiselect is enabled.
    ...
    return BaseView.extend({
        ...
        initialize: function(options) {
            ...
            if (this.options.multiselect) {
                this.model.checked = new BaseModel({
                    'enabled': this.options.enabled
                });                
                this.children.checkbox = new SyntheticCheckboxControl({
                    label: this.options.allMessage,
                    model: this.model.checked,
                    modelAttribute: 'enabled',
                    value: '*',
                    checkboxClassName: 'btn select-all',
                    icon: 'icon-' + this.options.selectedIcon
                });
                this.listenTo(this.model.checked, 'change enabled', this.toggleAll);
            }
            ...
            this.children.input = new FindInput({
                model: this.model.state,
                rawSearch: filterModel,
                placeholder: this.options.multiselect ? this.options.multiselectMessage : this.options.selectMessage
            });
        ...
        events: {
            ...
            'click .list-picker-list-multi a:not(".disabled")': function(e) {
                ...
                var boolean = this.selectedValues.length === this.items.length;
                this.stopListening(this.model.checked, 'change enabled');
                this.children.checkbox.setValue(boolean);
                this.listenTo(this.model.checked, 'change enabled', this.toggleAll);
            },
            ...
            'keyup .search-input-container': function(e) {
                this._filterListDebounced();
            },

            'paste .search-input-container': function(e) {
                this._filterList();
            },

            'cut .search-input-container': function(e) {
                this._filterList();
            },

            'change .search-input-container': function(e) {
                this._filterList();
            },

            'click .search-input-container .control-clear': function(e) {
                this._filterList();
            }
        },
        ...
        _selectItem: function($el) {
            // undefined for the .select-all $el
            if (this.items[$el.data('index')] === undefined) {
        ...
        _renderList: function() {
            ...
            this.$('.list-picker-list').html(_(this.listTemplate).template({
                ...
                ICONS: ColumnModel.ICONS
            }));

            if (this.children.checkbox && this.items.length) {
                this.children.checkbox.render().insertBefore(this.$('.list-picker-list'));
            }

            this.$allListItems = this.$('.list-picker-list a').not('[data-value="*"]');
        ...
        addNewItem: function() {
            ...
            if (errorMessage) {
                ...
                this.collection.customAddedFieldPickerItems.push({ value: newItem, type: 'string' });
                this._updateItems();
                this.$('.list-picker-list').html(_(this.listTemplate).template({
                    items: this.items,
                    options: this.options,
        ...
        _filterList: function() {
            var words;
            var val = this.children.input.$input.val();
            ...
            if (this.options.deferred) {
                ...
                modelsReadyCallback = function() {
                    ...
                    if (this.children.addItem) {
                        this.children.addItem.activate({ deep:true }).render().prependTo(this.$('.list-picker-scroll'));
                    }
                    this._updateItems();
        ...
        render: function() {
            ...
            this.visibility();
            this.children.input.render().appendTo(this.$('.search-input-container'));
            this._updateSelected();
        ...
        template: '\
            <div class="list-picker-heading <%= options.multiselect ? "list-picker-heading-multi" : "" %>">\
                <% if (options.header) { %><h3 class="header"><%- options.header %></h3><% } %>\
            ...
            <% _.each(items, function(item, index, list) { %>\
                <li>\
                    <a href="#" class="<%= item.isDisabled ? "disabled" : "" %> <%= item.isItalics ? "italicize" : "" %>" data-value="<%- item.value %>" data-index="<%- index %>">\
                        <i class="icon-<%- options.selectedIcon %>"></i>\
                        <% if (item.type) { %>\
                            <i class="field-icon icon-<%- ICONS[item.type] %>"></i>\
                        <% } %>\
