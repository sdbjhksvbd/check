define([
    ...
    function(
        ...
        var STACK_SIZE_SEARCH = splunkUtils.sprintf(" | join type=outer _time [search index=_internal [`set_local_host`] source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | dedup _time stack | stats sum(stacksz) AS \"%s\" by _time] | fields - _timediff ", _("stack size").t());
        ...
        var POOL_WARNING = "| rest splunk_server=local /services/licenser/messages | where (category==\"license_window\" OR category==\"pool_over_quota\") AND create_time >= now() - (30 * 86400) | rename pool_id AS pool | eval warning_day=if(category==\"pool_over_quota\",\"(\".strftime(create_time,\"%B %e, %Y\").\")\",strftime(create_time-43200,\"%B %e, %Y\")) | fields pool warning_day | join outer pool [rest splunk_server=local /services/licenser/peers | mvexpand active_pool_ids | eval peer_name=label | eval pool=active_pool_ids | fields pool peer_name | stats values(peer_name) as \"members\" by pool] | join outer pool [rest splunk_server=local /services/licenser/pools | eval pool=title | eval quota=if(isnull(effective_quota),quota,effective_quota) | eval quotaGB=round(quota/1024/1024/1024,3) | fields pool stack_id, quotaGB] |" + splunkUtils.sprintf("stats first(pool) as \"%s\" first(stack_id) as \"%s\" first(members) as \"%s\" first(quotaGB) as \"%s\" values(warning_day) AS \"%s\" by pool | fields - pool", _("Pool").t(), _("Stack ID").t(), _("Current Members").t(), _("Current Quota (GB)").t(), _("Warning Days - (Soft)/Hard").t());
        var SLAVE_WARNING = splunkUtils.sprintf("| rest splunk_server=local /services/licenser/peers | mvexpand active_pool_ids | where warning_count>0 | eval pool=active_pool_ids | join type=outer pool [rest splunk_server=local /services/licenser/pools | eval pool=title | fields pool stack_id] | eval in_violation=if(warning_count>4 OR (warning_count>2 AND stack_id==\"free\"),\"%s\",\"%s\") | fields label, title, pool, warning_count, in_violation | fields - _timediff | rename label as \"%s\" title as \"%s\" pool as \"%s\" warning_count as \"%s\" in_violation AS \"%s\"", _("yes").t(), _("no").t(), _("Peer").t(), _("GUID").t(), _("Pool").t(), _("Hard Warnings").t(), _("In Violation?").t());

        var LICENSE_USAGE_NOSPLIT =                         BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | timechart span=1d sum(b) AS \"%s\" fixedrange=false", _("volume").t()) + STACK_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        var LICENSE_USAGE_NOSPLIT_POOL_SELECTED =           BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | timechart span=1d sum(b) AS \"%s\" fixedrange=false", _("volume").t()) + POOL_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        var LICENSE_USAGE_BY_POOL =                         BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | timechart span=1d sum(b) AS \"%s\" by pool fixedrange=false", _("volume").t()) + STACK_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        var LICENSE_USAGE_BY_POOL_POOL_SELECTED =           BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | timechart span=1d sum(b) AS \"%s\" by pool fixedrange=false", _("volume").t()) + POOL_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        var LICENSE_USAGE_BY_INDEXER =                      BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | eval peer_guid=peer | stats max(b) AS volume by peer_guid, _time | join type=outer peer_guid [rest splunk_server=local /services/licenser/peers | rename label AS peer_name title AS peer_guid | table peer_guid peer_name] | eval peer_name = if(isnotnull(peer_name),peer_name,\"GUID: \".peer_guid) | timechart span=1d max(volume) AS \"%s\" by peer_name fixedrange=false", _("volume").t()) + STACK_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        var LICENSE_USAGE_BY_INDEXER_POOL_SELECTED =        BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | eval peer_guid=peer | stats max(b) AS volume by peer_guid, _time | join type=outer peer_guid [rest splunk_server=local /services/licenser/peers | rename label AS peer_name title AS peer_guid | table peer_guid peer_name] | eval peer_name = if(isnotnull(peer_name),peer_name,\"GUID: \".peer_guid) | timechart span=1d max(volume) AS \"%s\" by peer_name fixedrange=false", _("volume").t()) + POOL_SIZE_SEARCH + " | foreach * [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)]";
        ...
        var LICENSE_USAGE_NOSPLIT_PCT =                     BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(stacksz) AS stacksz by peer, pool, _time | stats sum(b) AS volumeB max(stacksz) AS stacksz by _time | eval pctused=round(volumeB/stacksz*100,2) | timechart span=1d max(pctused) AS \"%s\" fixedrange=false", _("% used").t());
        var LICENSE_USAGE_NOSPLIT_PCT_POOL_SELECTED =       BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(poolsz) AS poolsz by peer, pool, _time | stats sum(b) AS volumeB max(poolsz) AS poolsz by _time | eval pctused=round(volumeB/poolsz*100,2) | timechart span=1d max(pctused) AS \"%s\" fixedrange=false", _("% used").t());
        var LICENSE_USAGE_BY_POOL_PCT =                     BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(stacksz) AS stacksz by peer, pool, _time | stats sum(b) AS volumeB max(stacksz) AS stacksz by pool, _time | eval pctused=round(volumeB/stacksz*100,2) | timechart span=1d max(pctused) AS \"%s\" by pool fixedrange=false", _("% used").t());
        var LICENSE_USAGE_BY_POOL_PCT_POOL_SELECTED =       BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(poolsz) AS poolsz by peer, pool, _time | stats sum(b) AS volumeB max(poolsz) AS poolsz by pool, _time | eval pctused=round(volumeB/poolsz*100,2) | timechart span=1d max(pctused) AS \"%s\" by pool fixedrange=false", _("% used").t());
        var LICENSE_USAGE_BY_INDEXER_PCT =                  BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(stacksz) AS stacksz by peer, pool, _time | stats sum(b) AS volumeB max(stacksz) AS stacksz by peer, _time | eval pctused=round(volumeB/stacksz*100,2)| eval peer_guid=peer | join type=outer peer_guid [rest splunk_server=local /services/licenser/peers | rename label AS peer_name title AS peer_guid | table peer_guid peer_name] | eval peer_name = if(isnotnull(peer_name),peer_name,\"GUID: \".peer_guid) | timechart span=1d max(pctused) AS \"%s\" by peer_name fixedrange=false", _("% used").t());
        var LICENSE_USAGE_BY_INDEXER_PCT_POOL_SELECTED =    BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b latest(poolsz) AS poolsz by peer, pool, _time | stats sum(b) AS volumeB max(poolsz) AS poolsz by peer, _time | eval pctused=round(volumeB/poolsz*100,2) | eval peer_guid=peer | join type=outer peer_guid [rest splunk_server=local /services/licenser/peers | rename label AS peer_name title AS peer_guid | table peer_guid peer_name] | eval peer_name = if(isnotnull(peer_name),peer_name,\"GUID: \".peer_guid) | timechart span=1d max(pctused) AS \"%s\" by peer_name fixedrange=false", _("% used").t());
        var LICENSE_USAGE_BY_SOURCETYPE_PCT =               BASE_USAGE_SEARCH + " | join _time pool type=outer [search index=_internal source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(stacksz) AS stacksz by peer, pool, _time | stats max(stacksz) AS stacksz by _time, pool] | stats sum(b) AS bytes_used max(stacksz) AS stacksz by st, _time | timechart span=1d max(eval(round(bytes_used/stacksz*100,2))) by st fixedrange=false";
        var LICENSE_USAGE_BY_SOURCETYPE_PCT_POOL_SELECTED = BASE_USAGE_SEARCH + " | join _time pool type=outer [search index=_internal source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(poolsz) AS poolsz by peer, pool, _time | stats max(poolsz) AS poolsz by pool, _time] | stats sum(b) AS bytes_used max(poolsz) AS poolsz by st, _time | timechart span=1d max(eval(round(bytes_used/poolsz*100,2))) by st fixedrange=false";
        ...
        var LICENSE_USAGE_BY_HOST_PCT_POOL_SELECTED =       BASE_USAGE_SEARCH + " | join _time pool type=outer [search index=_internal source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(poolsz) AS poolsz by peer, pool, _time | stats max(poolsz) AS poolsz by pool, _time] | stats sum(b) AS bytes_used max(poolsz) AS poolsz by h, _time | timechart span=1d max(eval(round(bytes_used/poolsz*100,2))) by h fixedrange=false";
        ...
        var LICENSE_USAGE_BY_SOURCE_PCT_POOL_SELECTED =     BASE_USAGE_SEARCH + " | join _time pool type=outer [search index=_internal source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(poolsz) AS poolsz by peer, pool, _time | stats max(poolsz) AS poolsz by pool, _time] | stats sum(b) AS bytes_used max(poolsz) AS poolsz by s, _time | timechart span=1d max(eval(round(bytes_used/poolsz*100,2))) by s fixedrange=false";
        ...
        var LICENSE_USAGE_BY_INDEX_PCT_POOL_SELECTED =      BASE_USAGE_SEARCH + " | join _time pool type=outer [search index=_internal source=*license_usage.log* type=\"RolloverSummary\" earliest=-30d@d | eval _time=_time - 43200 | bin _time span=1d | stats latest(poolsz) AS poolsz by peer, pool, _time | stats max(poolsz) AS poolsz by pool, _time] | stats sum(b) AS bytes_used max(poolsz) AS poolsz by idx, _time | timechart span=1d max(eval(round(bytes_used/poolsz*100,2))) by idx fixedrange=false";

        var LICENSE_USAGE_NOSPLIT_MAXAVG =                  BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | stats sum(b) AS volume by _time | stats avg(volume) AS avgVolume max(volume) AS maxVolume  | eval maxVolumeGB=round(maxVolume/1024/1024/1024,3) | eval avgVolumeGB=round(avgVolume/1024/1024/1024,3) | rename avgVolumeGB AS \"%s\" maxVolumeGB AS \"%s\" | eval \"%s\" = \"\" | fields \"%s\", \"%s\", \"%s\"", _("average").t(), _("peak").t(), _("All Pools").t(), _("All Pools").t(), _("average").t(), _("peak").t());
        var LICENSE_USAGE_BY_POOL_MAXAVG =                  BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | stats sum(b) AS volume by pool, _time  | stats avg(volume) AS avgVolume max(volume) AS maxVolume by pool | eval maxVolumeGB=round(maxVolume/1024/1024/1024,3) | eval avgVolumeGB=round(avgVolume/1024/1024/1024,3) | rename avgVolumeGB AS \"%s\" maxVolumeGB AS \"%s\" pool as \"%s\" | fields \"%s\", \"%s\", \"%s\" | sort 5 - \"%s\"", _("average").t(), _("peak").t(), _("Pool").t(),  _("Pool").t(), _("average").t(), _("peak").t(), _("average").t());
        var LICENSE_USAGE_BY_INDEXER_MAXAVG =               BASE_SUMMARY_SEARCH + splunkUtils.sprintf(" | eval _time=_time - 43200 | bin _time span=1d | stats latest(b) AS b by peer, pool, _time | eval peer_guid=peer | stats sum(b) AS volume by peer_guid, _time | stats avg(volume) AS avgVolume max(volume) AS maxVolume by peer_guid | join type=outer peer_guid [rest splunk_server=local /services/licenser/peers | rename label AS peer_name title AS peer_guid | table peer_guid peer_name] | eval peer_name = if(isnotnull(peer_name),peer_name,\"GUID: \".peer_guid) | foreach *Volume [eval <<FIELD>>=round('<<FIELD>>'/1024/1024/1024, 3)] |  rename peer_name as \"%s\" avgVolume AS \"%s\" maxVolume AS \"%s\" | sort 5 - \"%s\" | fields \"%s\", \"%s\", \"%s\" | fields - _timediff, peer_guid", _("Indexer").t(), _("average").t(), _("peak").t(), _("average").t(), _("Indexer").t(), _("average").t(), _("peak").t());
        ...
        var UNKNOWN_VALUE_MSG = splunkUtils.sprintf(_("You have selected \"%s\" from the \"%s\" menu. If your license peers are running a version of Splunk lower than 6.0, they will not report per index usage and you will see their usage labeled as \"%s\".").t(), _("index").t(), _("Split by").t(), _("(UNKNOWN)").t());
