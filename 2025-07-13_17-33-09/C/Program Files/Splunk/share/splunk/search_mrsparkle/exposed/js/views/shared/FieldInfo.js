        ...
        return PopTartView.extend({
            ...
            events: {
                ...
                'click td.value > a': function(e) {
                    var $target = $(e.currentTarget),
                        data = $target.data(),
                        // Cannot use data.value here because a json payload is
                        // treated as an object and causes a 400 error in the
                        // intentions parser.
                        // Cannot use innerHTML because it escapes < and >
                        dataValue = $target.prop('innerText');
                    this.model.intentionsParser.clear({silent: true});
                    this.model.intentionsParser.fetch({
                        data: {
                            q: this.model.report.entry.content.get('search'),
