define(function(require, exports, module) {
     ...
     * <a href="http://dev.splunk.com/view/SP-CAAAEU6" target="_blank" rel="noopener noreferrer">Search results model</a>
    ...
    var ResultsModel = Backbone.Model.extend(/** @lends splunkjs.mvc.SplunkResultsModel.prototype */{
        ...
        fetch: function(options) {
            ...
            var resultsSid = manager.getSid && manager.getSid();            
            ...
            Backbone.ajax({
                ...
                error: function(response) {
                    ...
                    var text = message ? message.text : _("An error occurred while fetching data.").t();
                    if (splunkDUtils.messagesContainsOneOfTypes(err.data && err.data.messages, [splunkDUtils.RISKY_COMMAND])) {
                        text = _("Delayed: Dependent on successful preceding search.").t();
                    }
                ...
                success: function(data, status, res) {
                    ...
                    model._pendingFetchOptions = null;
                    model._isFetching = false;
                    var isSameData = _.isEqual(data, model._data);
                    model._data = data;

                    var manager = model.get('manager');
                    var currentSid = manager && manager.getSid && manager.getSid();
                    if (resultsSid && currentSid && resultsSid !== currentSid) {
                        // discard the results if a new job was started while fetching the results of the old one
                        return;
                    }

                    var isSameSearchJob = _.isEqual(model.lastSid, currentSid);    
                    model.lastSid = currentSid;
              
                    if (_success) {
                        _success(this, data, options);
                    }

                    var isPostSearch = manager && !!manager.parent;
                    
                    // SPL-209481: should not trigger data event if it is same data returned from the same search job but not for post search
                    if (!isSameData || !isSameSearchJob || isPostSearch) {
