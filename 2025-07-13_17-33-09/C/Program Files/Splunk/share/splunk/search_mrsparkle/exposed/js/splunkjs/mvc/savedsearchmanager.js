                    ...
                    if (splunkDUtils.messagesContainsOneOfTypes(err.data.messages, [splunkDUtils.RISKY_COMMAND])) {
                        if (dashboardUtils.isHiddenSearch(that.get('parentElement'))) {
                            if (dashboardUtils.isHiddenSearchRiskyCommandDisabled()) {
                                ...
                                dispatchJob.error.set("messages", err.data.messages);
                                that.showRiskyWarningDialog(dispatchJob);
                            }
                        } else {
                            that.trigger("search:error", _("Action required: Click the icon in the top right corner to review the query before running.").t(), err, dispatchJob);
                        }
                    } else {
