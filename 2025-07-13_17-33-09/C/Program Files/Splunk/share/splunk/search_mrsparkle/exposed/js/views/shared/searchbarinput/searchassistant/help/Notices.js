        ...
        return Base.extend({
            ...
            getSavedSearchesLinks: function() {
                ...
                _.each(savedSearches, function(savedSearch) {
                    ...
                    savedSearchLinks.push('<a href="' + searchRoute + '" class="suggested-search" title="' + _.escape(search) + '">' + _.escape(savedSearch.name) + '</a> <a class="suggested-search_secondary secondary-link" href="' + searchRoute + '" target="_blank" rel="noopener noreferrer"><i class="icon-external"></i></a>');
                }, this);
            ...
            template: '\
                <% if (error) { %>\
                    <div class="alert alert-error"><i class="icon-alert"></i><%- error %></div>\
                <% } else { %>\
                    <% _.each(commandErrors, function(commandError) { %>\
                       <div class="alert alert-warning"><i class="icon-alert"></i><%- sprintf(_("Note: Unknown command \'%s\'. Did you mean \'%s\'?").t(), commandError.unknown, commandError.suggestion[0]) %></div>\
                    <% }); %>\
                    <% _.each(fieldErrors, function(fieldError) { %>\
                       <div class="alert alert-warning"><i class="icon-alert"></i><%- sprintf(_("Note: Did you mean the %s field?").t(), fieldError.suggestion[0]) %></div>\
                    <% }); %>\
                    <% _.each(considers, function(consider) { %>\
                       <div class="alert alert-warning"><i class="icon-alert"></i><%- sprintf(considerStrings[consider.type], consider) %></div>\
                    <% }); %>\
                    <% if (savedSearchesLinks.length > 0) { %>\
                        <% if (savedSearchesLinks.length === 1) { %>\
                            <div class="alert alert-info"><i class="icon-alert"></i><%= sprintf(_("Note: Your search looks similar to the savedsearch %s").t(), savedSearchesLinks[0]) %></div>\
                        <% } else { %>\
                            <div class="alert alert-info"><i class="icon-alert"></i><%= sprintf(_("Note: Your search looks similar to the savedsearches %s and %s").t(), savedSearchesLinks[0], savedSearchesLinks[1]) %></div>\
                        <% } %>\
                    <% } %>\
                <% } %>\
            '
