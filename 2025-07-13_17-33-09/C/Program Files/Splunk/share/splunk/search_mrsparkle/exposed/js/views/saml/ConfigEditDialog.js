...
define([
    ...
    'uri/route',
    './components/ScriptSecureArguments/Master'
],
        ...
        return Modal.extend({
            ...
            events: $.extend({}, Modal.prototype.events, {
                'click .btn-primary': function(e) {
                    if (!this.model.saml.get('ui.scriptFunctions') ||
                        this.model.saml.get('ui.scriptFunctions').length == 0) {
                        this.model.saml.set('ui.scriptFunctions', '');
                    }
                ...
                'click .script-header': function(e) {
                    this.$('.script-header').toggleClass('icon-triangle-down');
                    this.$('.script-body').toggle();
                },
            ...
            initialize: function(options) {
                ...
                this.children.getUserInfoTtl = new ControlGroup({
                    controlType: 'Text',
                    className: 'saml-getUserInfoTtl control-group',
                    controlOptions: {
                        modelAttribute: 'ui.getUserInfoTtl',
                        model: this.model.saml,
                    },
                    tooltip: _("When you configure the authentication system to use SAML, " +
                             "it runs the 'getUserInfo' script function to retrieve information from the SAML " +
                             "identity provider when users perform ad-hoc operations such as working with " +
                             "tokens and saved searches.").t(),
                    label: _('Get User Info time-to-live').t(),
                    help: _("How long the auth system caches information that it retrieves with the 'getUserInfo' script.").t(),
                });

                this.children.scriptPath = new ControlGroup({
                    controlType: 'Text',
                    className: 'saml-scriptPath control-group',
                    controlOptions: {
                        modelAttribute: 'ui.scriptPath',
                        model: this.model.saml,
                    },
                    label: _('Script Path').t(),
                    help: _('Name and extension of the script to use. Must be placed into $SPLUNK_HOME/etc/auth/scripts.').t(),
                });

                this.children.scriptTimeout = new ControlGroup({
                    controlType: 'Text',
                    className: 'saml-scriptTimeout control-group',
                    controlOptions: {
                        modelAttribute: 'ui.scriptTimeout',
                        model: this.model.saml,
                    },
                    help: _("The maximum time the script can run before the auth system forcefully terminates it.").t(),
                    label: _("Script timeout").t()
                });
            ...
            render: function() {
                ...
                var scriptHelpLink = route.docHelp(rootUrl, locale, 'learnmore.SAML.getstarted');
                ...
                var scriptPath = this.model.saml.get('ui.scriptPath');
                var scriptSecureArguments = this.model.saml.get('ui.scriptSecureArguments');
                var showScriptedLogin = scriptPath || scriptSecureArguments;
                ...
                this.children.scriptPath.render().appendTo(this.$(".scriptPath-placeholder"));
                this.children.scriptTimeout.render().appendTo(this.$(".scriptTimeout-placeholder"));
                this.children.getUserInfoTtl.render().appendTo(this.$(".getUserInfoTtl-placeholder"));
                ...
                this.children.scriptSecureArguments = new ScriptSecureArguments({
                    model: this.model,
                    scrollContainer: this.$(Modal.BODY_SELECTOR)[0],
                });
                this.children.scriptSecureArguments.render().appendTo(this.$(".scriptSecureArguments-placeholder"));
                ...
                if (showScriptedLogin) {
                    this.$('.script-header').toggleClass('icon-triangle-down');
                } else {
                    this.$('.script-body').hide();
                }
                if (showAlias) {
                    this.$('.alias-header').toggleClass('icon-triangle-down');
                } else {
            ...
            dialogFormBodyTemplate: '\
                ...
                <p class="control-heading">\
                    ...
                    <a href="<%- helpLink %>" class="external learnMoreLink" target="_blank" rel="noopener noreferrer"><%= _("Learn More").t() %></a>\
                </p>\
                <br/>\
                <p class="control-heading">\
                 ...
                 <a href="<%- spMetadataLink %>" class="btn download-file-button" target="_blank" rel="noopener noreferrer"><%- _("Download File").t() %></a>\
                <p class="control-heading">\
                ...
                <div class="aqr-body">\
                    <div class="section-desc"><%- _("Optional. Used only for vendors that support AQRs, such as " +\
                                                    "IBM, CA, Ping Identity. Confirm with your provider. You must " +\
                                                    "enable either AQRs or authentication extensions for SAML user " +\
                                                    "tokens.").t() %></div>\
                ...
                <p class="control-heading script-header icon-triangle-right">\
                    <strong><%- _("Authentication Extensions").t() %></strong>\
                </p>\
                <div class="script-body">\
                    <div class="section-desc">\
                        <%- _("Optional. You must enable either AQRs or authentication extensions for SAML user tokens.").t() %>\
                        <a href="<%- scriptHelpLink %>" class="external learnMoreLink" target="_blank" rel="noopener noreferrer"><%= _("Learn More").t() %></a>\
                    </div>\
                    <div class="scriptPath-placeholder"></div>\
                    <div class="scriptTimeout-placeholder"></div>\
                    <div class="getUserInfoTtl-placeholder"></div>\
                    <div class="scriptSecureArguments-placeholder"></div>\
                </div>\
                <p class="control-heading alias-header icon-triangle-right" style="display: block;">\
