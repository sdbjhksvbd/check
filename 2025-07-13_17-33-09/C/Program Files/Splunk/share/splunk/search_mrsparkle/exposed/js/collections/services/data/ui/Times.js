define(
    ...
    function(Utils, TimeModel, SplunkDsBaseCollection) {
        var TimesCollection = SplunkDsBaseCollection.extend({
            ...
            setSettings: function (dialogOptions) {
                 this.filter(function (model) {
                    return model.isSettings();
                }).forEach(function (model) {
                    var content = {};
                    Object.keys(dialogOptions).forEach(function (cfg) {
                        content[cfg] = Utils.normalizeBoolean(dialogOptions[cfg]);
                    });
                    model.entry.content.set(content);
                });
            }
        });
