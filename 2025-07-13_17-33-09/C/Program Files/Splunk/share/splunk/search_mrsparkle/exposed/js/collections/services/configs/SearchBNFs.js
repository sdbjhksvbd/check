        ...
        return SplunkDsBaseCollection.extend({
            ...
            _getParsedSyntax: function(syntax, response) {
                ...
                var listItemRegex = '[\\w\\-_]+',
                    ...
                    dynamicListItemRegex = '(?:(?:\\(' + listItemRegex + '(?:\\|' + listItemRegex + ')*\\))|' + listItemRegex + ')<(?:int|num)>',
                ...
                if (listMatch) {
                    ...
                    // replace all functions that match (sma|ema|wma)<int> 
                    ...
                    // replace all functions that match (perc|p|exactperc|upperperc)<num>, 
                    // (perc)<num> or pert<num> with a regex for the function name
                    list = list.replace(/(\(?[\w\-_]+\)?|(?:\([\w\-_]+(?:\|[\w\-_]+)+)\))<num>/g, function(fullMatch, match, index) {
                        var functionsArray = match.replace(/[\(\)]/g,'').split('|');
                        var replaceText = '';
                        functionsArray.forEach(function(currentValue,index,arr) {
                            // Allow either int or float
                            var withRegex = currentValue+'\\d+\\.\\d+|' + currentValue + '\\d+';
                            replaceText += replaceText ?  "|" + withRegex : withRegex;
                        }, this);
                        return replaceText;
                    });
