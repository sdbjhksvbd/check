...
class HyperParserTest(unittest.TestCase):
    code = (
            ...
            '[x for x in\n'
            'for = False\n'
            'clich̩ = "this is a string with unicode, what a clich̩"'
            )
    ...
    def test_get_expression(self):
        ...
        p = get('12.6')
        self.assertEqual(p.get_expression(), 'clich̩')
    ...
    def test_eat_identifier_various_lengths(self):
        ...
        for length in range(1, 21):
            ...
            self.assertEqual(eat_id('̩' * length, 0, length), length)
            self.assertEqual(eat_id('a' + '2' * (length - 1), 0, length), length)
            self.assertEqual(eat_id('̩' + '2' * (length - 1), 0, length), length)
            self.assertEqual(eat_id('̩' + 'a' * (length - 1), 0, length), length)
            self.assertEqual(eat_id('̩' * (length - 1) + 'a', 0, length), length)
            self.assertEqual(eat_id('+' * length, 0, length), 0)
            self.assertEqual(eat_id('2' + 'a' * (length - 1), 0, length), 0)
            self.assertEqual(eat_id('2' + '̩' * (length - 1), 0, length), 0)
